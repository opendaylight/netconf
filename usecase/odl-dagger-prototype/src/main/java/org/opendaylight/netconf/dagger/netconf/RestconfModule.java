...
public interface RestconfModule {
    ...
    @Provides
    @Singleton
    static JaxRsEndpointConfiguration jaxRsEndpointConfiguration(final ConfigLoader configLoader) {
        final var config = configLoader.getConfig(RFC8040Configuration.class, "", Path.of(
            "org.opendaylight.restconf.nb.rfc8040.cfg"));
        return new JaxRsEndpointConfiguration(
            config.dataMissingIs404() ? ErrorTagMapping.ERRATA_5565 : ErrorTagMapping.RFC8040,
            PrettyPrintParam.of(config.prettyPrint()),
            Uint16.valueOf(config.maxFragmentLength()),
            Uint32.valueOf(config.heartbeatInterval()), config.restconf(),
            config.pingExecutorNamePrefix(), config.maxThreadCount());
    }
