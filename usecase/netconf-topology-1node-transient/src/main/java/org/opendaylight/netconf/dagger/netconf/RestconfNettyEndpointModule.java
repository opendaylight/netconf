...
import org.opendaylight.restconf.server.NettyLocationProvider;
...
public interface RestconfNettyEndpointModule {
    ...
    static LocationProvider locationProvider() {
        return new NettyLocationProvider();
    }
    ...
    static RFC8040Configuration rfcConfig(final ConfigLoader configLoader) {
        return configLoader.getConfig(RFC8040Configuration.class, "odl.netconf.prototype.rfc8040-configuration",
    ...
    /**
     * Implementation of OSGi RFC8040Configuration configuration used for components that are not initialized or managed
     * by the OSGi.
     *
     ...
     *     {+restconf} URI Template. The format of this string must be conform to the "path-rootless" ABNF rule
     *     of RFC3986 and is subject to decoding of any percent-encoded octets. Valid examples include, without
     *     the double quotes:
     *     <ul>
     *     <li>"restconf", which is the default value and results in the same expansion as RFC8040, i.e. the RESTCONF
     *        data resource being located at /restconf/data</li>
     *     <li>"foo/bar/baz", which results in the RESTCONF data resource being located at /foo/bar/baz/data</li>
     ...
     * @param httpWriteBufferLowWatermark Netty channel write buffer low watermark used for outbound backpressure.
     *     A channel becomes writable again when queued outbound bytes fall below this value.
     * @param httpWriteBufferHighWatermark Netty write buffer high watermark in bytes. A channel becomes unwritable when
     *     queued outbound bytes exceed this value. To prevent OutOfMemoryError (OOM) crashes, ensure that is
     *     significantly lower than the maximum memory allocated to the JVM (-Xmx and MaxDirectMemorySize). Setting this
     *     value too high defeats the backpressure mechanism and risks exhausting server RAM.
     * @param http3AltSvcMaxAge Max-Age (ma) value advertised in Alt-Svc header for HTTP/3 (h3). The advertised port is
     *     always bind-port. Set to 0 to disable Alt-Svc advertisement.
     ...
     *     remotely-initiated bidirectional streams. Locally-initiated stream limit is fixed to 262144 bytes.
     * @param http3InitialMaxStreamsBidirectional QUIC initial max number of bidirectional streams.
     */
    @ConfigurationProperties("odl.netconf.prototype.rfc8040-configuration")
    record RFC8040Configuration(
        ...
        @DefaultValue("16384")
        @Min(0)
        int httpWriteBufferLowWatermark,

        @DefaultValue("16384")
        @Min(0)
        int httpWriteBufferHighWatermark,
