...
public interface RestconfNettyEndpointModule {
     ...
     *                                      A channel becomes writable again when queued outbound bytes fall below
     *                                      this value.
     * @param httpWriteBufferHighWatermark  Netty write buffer high watermark in bytes. A channel becomes unwritable
     *                                      when queued outbound bytes exceed this value.
     *                                      To prevent OutOfMemoryError (OOM) crashes, ensure that is significantly
     *                                      lower than the maximum memory allocated to the JVM
     *                                      (-Xmx and MaxDirectMemorySize). Setting this value too high defeats
     *                                      the backpressure mechanism and risks exhausting server RAM.
