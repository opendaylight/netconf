...
#    echo "Match $i count $count"
done

#for i in `find . -name \*.class | xargs strings | sort -u | fgrep shaded/xerces | sed 's/.*xerces//' | sed 's/;.*//' | sort -u `; do
#    dir=`dirname $i`
#    name=`basename $i`
#    count=`find org/opendaylight/netconf/shaded/xerces$dir -name $name\* -printf . | wc -c`
#    echo "Match $i count $count"
#done
