...
public class NetconfCallHomeServer implements AutoCloseable, ServerKeyVerifier {
    ...
    static Logger LOG = LoggerFactory.getLogger(NetconfCallHomeServer.class);
    ...
    NetconfCallHomeServer(SshClient sshClient, CallHomeAuthorizationProvider authProvider, Factory factory, InetSocketAddress socketAddress) {
        ...
        acceptor = minaFactory.createAcceptor(clientSessions);
    }
    ...
    protected void doAuth(final ClientSession cSession) {
        try {
            ...
            authFuture.addListener(newSshFutureListener(cSession));
        } catch (IOException e) {
            LOG.error("Failed to authorize.", e);
        }
    }

    SshFutureListener<AuthFuture> newSshFutureListener(final ClientSession cSession)
    {
    ...
    public boolean verifyServerKey(ClientSession sshClientSession, SocketAddress remoteAddress, PublicKey serverKey) {
        ...
        if(authorization.isServerAllowed()) {
            // FIXME: Do we need to capture opened sessions?
            sessionFactory.create(sshClientSession, authorization,remoteAddress);
            return true;
        }
        return false;
    }
    ...
    public void bind() {
        // FIXME shouldn't we let the bind exception flow out?
        try {
            client.start();
