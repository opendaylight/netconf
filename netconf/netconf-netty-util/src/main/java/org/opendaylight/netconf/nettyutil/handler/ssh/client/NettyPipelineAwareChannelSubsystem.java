...
public class NettyPipelineAwareChannelSubsystem extends ChannelSubsystem {
    ...
    public NettyPipelineAwareChannelSubsystem(String subsystem, ChannelPipeline pipeline) {
        super(subsystem);
    ...
    protected void doWriteData(final byte[] data, final int off, final long len) throws IOException {
        ...
        if (!isClosing()) {
            pipeline.firstContext().fireChannelRead(Unpooled.copiedBuffer(data, off, (int) len));
            adjustWindow(len);
        }
    }
