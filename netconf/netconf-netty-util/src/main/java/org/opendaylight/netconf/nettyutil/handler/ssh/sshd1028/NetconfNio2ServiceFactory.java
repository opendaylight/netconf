...
public class NetconfNio2ServiceFactory extends Nio2ServiceFactory {
    ...
    public IoConnector createConnector(final IoHandler handler) {
        try {
            final Field fieldGroup = this.getClass().getSuperclass().getDeclaredField("group");
            fieldGroup.setAccessible(true);
            final AsynchronousChannelGroup group = (AsynchronousChannelGroup)fieldGroup.get(this);
            return autowireCreatedService(new NetconfNio2Connector(getFactoryManager(), handler, group));
        } catch (NoSuchFieldException | IllegalAccessException e) {
            LOG.error("NetconfNio2Connector cannot be instanciated. Cannot access the ChannelGroup from the " +
                "Nio2ServiceFactory. Creating default Nio2Connector instead.");
        }
