...
class Filter {
    ...
    Collection<YidFilter> getDataRootsFromFilter(XmlElement operationElement) throws DocumentedException {
        try {
    ...
    private Map<QName,List<XmlElement>> groupByRootElements(List<XmlElement> filters) throws MissingNameSpaceException {
    ...
    private YidFilter createYidFilter(XmlElement filter) throws DocumentedException {
        return new YidFilter(validator.validate(filter), filter);
    }

    private QName getFilterRootQName(XmlElement filter) throws MissingNameSpaceException {
        return QName.create(filter.getNamespace(), filter.getName());
    }

    @SuppressWarnings("illegalCatch")
    private XmlElement mergeGroupedElements(List<XmlElement> idFilters) throws Exception {
        if (idFilters.size() == 1) {
            return idFilters.get(0);
        }
        FilterCombiner combiner = new FilterCombiner();
        for (XmlElement idFilter : idFilters) {
    ...
    static class YidFilter {
        ...
        YidFilter(YangInstanceIdentifier path, @Nullable XmlElement filter) {
