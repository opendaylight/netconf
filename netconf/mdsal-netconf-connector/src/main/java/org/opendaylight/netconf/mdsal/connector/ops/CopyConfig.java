...
public class CopyConfig extends AbstractSingletonNetconfOperation {
    ...
    protected Element handleWithNoSubsequentOperations(final Document document, final XmlElement operationElement)
        ...
        // <copy-config>, unlike <edit-config>, replaces entire configuration,
        ...
        for (final XmlElement element : configElement.getChildElements()) {
            ...
            // Doing merge instead of put, because put breaks toplevel list test case:
    ...
    private void removePreviousConfiguration(final DOMDataReadWriteTransaction rwTx) throws DocumentedException {
        // FIXME
        // Removing root node does not work:
        // rwTx.delete(LogicalDatastoreType.CONFIGURATION, YangInstanceIdentifier.EMPTY);
        // Apply strategy failed to produce root node for modification MutableDataTree [...]
        // to
        // org.opendaylight.netconf.mdsal.connector.TransactionProvider.commitTransaction(TransactionProvider.java:86)

        // Also overwriting it with empty node:
        // rwTx.put(LogicalDatastoreType.CONFIGURATION, YangInstanceIdentifier.EMPTY,
        //     Builders.containerBuilder()
        //         .withNodeIdentifier(new YangInstanceIdentifier.NodeIdentifier(ROOT_DATA_QNAME)).build());
        // Instance identifier references null but data identifier is ImmutableContainerNode{
        // nodeIdentifier=(urn:ietf:params:xml:ns:netconf:base:1.0)data, value=[], attributes={}}
        // at org.opendaylight.controller.cluster.databroker.AbstractDOMBrokerWriteTransaction
        // .checkInstanceIdentifierReferencesData(AbstractDOMBrokerWriteTransaction.java:87)

        // So removing all the children:
        try {
            final Optional<NormalizedNode<?, ?>> rootOptional =
                rwTx.read(LogicalDatastoreType.CONFIGURATION, YangInstanceIdentifier.EMPTY).get();
            if (!rootOptional.isPresent()) {
                throw new NetconfDocumentedException("Failed to read root node, cannot execute <copy-config>",
                    ErrorType.APPLICATION, ErrorTag.OPERATION_FAILED, ErrorSeverity.ERROR);
            }
            final ContainerNode root = (ContainerNode) (rootOptional.get());
            ...
            LOG.warn("Read from datastore failed when trying to remove data before <copy-config> operation", e);
        }
    }
    ...
    private static Datastore extractTargetParameter(final XmlElement operationElement) throws DocumentedException {
        ...
        if (elementsByTagName.getLength() == 0) {
            final Map<String, String> errorInfo = ImmutableMap.of("bad-attribute", TARGET_KEY, "bad-element",
                OPERATION_NAME);
            throw new DocumentedException("Missing target element", ErrorType.PROTOCOL, ErrorTag.MISSING_ELEMENT,
