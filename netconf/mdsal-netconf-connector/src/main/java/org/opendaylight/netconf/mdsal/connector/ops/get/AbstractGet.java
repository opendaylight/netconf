...
public abstract class AbstractGet extends AbstractSingletonNetconfOperation {
    ...
    private static final Function<PathArgument, QName> PATH_ARG_TO_QNAME = PathArgument::getNodeType;
    ...
    private SchemaPath getSchemaPath(final YangInstanceIdentifier dataRoot) {
        return SchemaPath.create(
                Iterables.transform(dataRoot.getPathArguments(), PATH_ARG_TO_QNAME), dataRoot.equals(ROOT));
    }
     ...
     * datastore. Empty filter returns Optional.absent() which should equal an empty &lt;data/&gt; container
     * in the response. If filter is not present we want to read the entire datastore - return ROOT.
     *
