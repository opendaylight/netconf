...
public class TxChain implements DOMTransactionChain, TxListener {
     ...
     * or is cancelled.
     */
    private volatile boolean lastTxSuccessful = true;
    /**
    ...
    public AbstractWriteTx newWriteOnlyTransaction() {
        ...
        pendingWriteTx.addListener(this);
    ...
    public void close() {
        if (closed.compareAndSet(false, true)) {
            ...
            if (lastTxFinished.get() && lastTxSuccessful) {
    ...
    private void setLastFinishedTxStatus(final boolean successful) {
        this.lastTxFinished.set(successful);
        this.lastTxSuccessful = successful;
    }
