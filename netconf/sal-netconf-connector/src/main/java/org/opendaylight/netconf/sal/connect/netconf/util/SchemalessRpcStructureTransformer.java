...
class SchemalessRpcStructureTransformer implements RpcStructureTransformer {
     ...
     */
    @Override
    public Optional<NormalizedNode<?, ?>> selectFromDataStructure(DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?> data,
     ...
     */
    @Override
    public AnyXmlNode createEditConfigStructure(Optional<NormalizedNode<?, ?>> data,
    ...
    private void checkDataValidForPath(YangInstanceIdentifier dataPath, Element dataNode) {
        //if datapath is empty, consider dataNode to be a root node
        if (dataPath.equals(YangInstanceIdentifier.EMPTY)) {
            return;
        }
        XmlElement dataElement = XmlElement.fromDomElement(dataNode);
    ...
    private void checkKeyValuesValidForPath(XmlElement dataElement, YangInstanceIdentifier.PathArgument lastPathArgument) {
        ...
        for (QName qName : keyValues.keySet()) {
            final List<XmlElement> key = dataElement.getChildElementsWithinNamespace(qName.getLocalName(), qName.getNamespace().toString());
            if (key.isEmpty()) {
    ...
    private void setOperationAttribute(Optional<ModifyAction> operation, Document document, Element dataNode) {
        final QName operationQname = NetconfMessageTransformUtil.NETCONF_OPERATION_QNAME;
        final Attr operationAttribute = document.createAttributeNS(operationQname.getNamespace().toString(), operationQname.getLocalName());
        operationAttribute.setTextContent(toOperationString(operation.get()));
        dataNode.setAttributeNode(operationAttribute);
    }

    private static Element instanceIdToXmlStructure(final List<YangInstanceIdentifier.PathArgument> pathArguments, Element data) {
    ...
    private List<XmlElement> selectMatchingNodes(Element domElement, YangInstanceIdentifier path) {
