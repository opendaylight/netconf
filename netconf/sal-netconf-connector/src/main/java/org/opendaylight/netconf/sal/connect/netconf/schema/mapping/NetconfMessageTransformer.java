...
public class NetconfMessageTransformer implements MessageTransformer<NetconfMessage> {
    ...
    private static NotificationDefinition getMostRecentNotification(final Collection<NotificationDefinition> notificationDefinitions){
        final Iterator<NotificationDefinition> iterator = notificationDefinitions.iterator();
        NotificationDefinition mostRecent = iterator.next();
        while (iterator.hasNext())
        {
            final NotificationDefinition tempNotificationDefinition = iterator.next();
            if (mostRecent.getQName().getRevision().compareTo(tempNotificationDefinition.getQName().getRevision()) == -1)
            {
                mostRecent = tempNotificationDefinition;
            }
        }
        return mostRecent;
    }
