...
public final class KeepaliveSalFacade implements RemoteDeviceHandler {
    ...
    public void onDeviceConnected(final NetconfDeviceSchema deviceSchema,
        ...
        final var kaRpc = devRpc == null ? null : new KeepaliveDOMRpcService(devRpc);
        final var devAction = services.actions();
        // FIXME: wrap with keepalive
        final var kaAction = devAction;

        salFacade.onDeviceConnected(deviceSchema, sessionPreferences, new RemoteDeviceServices(kaRpc, kaAction));
    ...
    private final class KeepaliveTask implements Runnable, FutureCallback<DOMRpcResult> {
        ...
        private synchronized void sendKeepalive(final long now) {
            ...
            // FIXME: fall back to actions
            final var devRpc = localServices.rpcs();
            if (devRpc != null) {
