...
public abstract class AbstractWriteTx implements DOMDataWriteTransaction {
    ...
    public final ListenableFuture<RpcResult<TransactionStatus>> commit() {
        checkNotFinished();
        finished = true;
        final SettableFuture<RpcResult<TransactionStatus>> resultSettableFuture = SettableFuture.create();
        //use settable future to guarantee, that this callback is called before any other callbacks
        Futures.addCallback(performCommit(), new FutureCallback<RpcResult<TransactionStatus>>() {
            @Override
            public void onSuccess(@Nullable final RpcResult<TransactionStatus> result) {
                try {
                    ...
                    resultSettableFuture.set(result);
                }
            }
            ...
            public void onFailure(final Throwable t) {
                try {
                    ...
                    resultSettableFuture.setException(t);
                }
            }
        });
