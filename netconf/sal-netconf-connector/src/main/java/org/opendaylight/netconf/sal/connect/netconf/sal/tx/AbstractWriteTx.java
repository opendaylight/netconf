...
public abstract class AbstractWriteTx implements DOMDataWriteTransaction {
    ...
    protected CheckedFuture<Void, NetconfDocumentedException> listenableFutureToCheckedFuture(final ListenableFuture<DOMRpcResult> future, final String transaction) {
        final ListenableFuture<Void> transacAsVoid = Futures.transform(future, new Function<DOMRpcResult, Void>() {
            @Override
      ...
      return Futures.makeChecked(transacAsVoid, new Function<Exception, NetconfDocumentedException>() {
          @Override
