...
public class NetconfTopologyRPCProvider implements NetconfNodeTopologyService{
    ...
    private NetconfNode encryptPassword(AddNetconfNodeInput input){
        ...
        if(encrypt) {
            String encryptedPassword = loginPassword.getPassword();
            try {
                encryptedPassword = encryptionService.encrypt(loginPassword.getPassword());
            } catch (GeneralSecurityException e) {
                LOG.error("Unable to encrypt the password."+e.getMessage());
            }
    ...
    private void writeToConfigDS(NetconfNode node, NodeId nodeId, String topologyId,
        ...
        Futures.addCallback(future, new FutureCallback<Void>() {
            ...
            public void onFailure(Throwable exception) {
                LOG.error("add-netconf-node RPC: Unable to add netconf node." + exception.getMessage());
