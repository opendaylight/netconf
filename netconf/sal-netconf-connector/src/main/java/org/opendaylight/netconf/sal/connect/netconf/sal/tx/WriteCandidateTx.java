...
public class WriteCandidateTx extends AbstractWriteTx {
    ...
    private void lock() {
        ...
        operationsList.add(netOps.lockCandidate(lockCandidateCallback));
    }
    ...
    public synchronized ListenableFuture<RpcResult<TransactionStatus>> performCommit() {
        operationsList.add(netOps.commit(new NetconfRpcFutureCallback("Commit", id)));
        ListenableFuture<RpcResult<TransactionStatus>> txResult = transform(Futures.allAsList(operationsList));
    ...
    protected void editConfig(final YangInstanceIdentifier path,
        ...
        if (defaultOperation.isPresent()) {
            operationsList.add(netOps.editConfigCandidate(
                    editConfigCallback, editStructure, defaultOperation.get(), rollbackSupport));
        } else {
            operationsList.add(netOps.editConfigCandidate(editConfigCallback, editStructure, rollbackSupport));
        }
    }
