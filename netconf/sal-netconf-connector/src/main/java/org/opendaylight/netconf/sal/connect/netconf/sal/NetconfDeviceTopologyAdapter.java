...
public final class NetconfDeviceTopologyAdapter implements AutoCloseable {
    ...
    private boolean masterDiffers(final String masterAddress) {
        ...
        try {
            ccs = Preconditions.checkNotNull(
                   Preconditions.checkNotNull(
                    Preconditions.checkNotNull(
                     Preconditions.checkNotNull(txChain)
                     .newReadOnlyTransaction())
                    .read(
                          Preconditions.checkNotNull(LogicalDatastoreType.OPERATIONAL),
                          Preconditions.checkNotNull(
                           Preconditions.checkNotNull(
                            Preconditions.checkNotNull(
                             id.getTopologyBindingPath())
                            .augmentation(NetconfNode.class))
                           .child(ClusteredConnectionStatus.class))))
                   .get())
                  .orNull();
        } catch (InterruptedException | ExecutionException exc) {
            LOG.warn("Failure reading clustered connection status for {}, assume new master is present.", id, exc);
            return true;
        }
