...
public final class NetconfDeviceTopologyAdapter implements AutoCloseable {
    ...
    private boolean masterDiffers(final String masterAddress) {
        ...
        final ClusteredConnectionStatus ccs = txChain.newReadOnlyTransaction().read(LogicalDatastoreType.OPERATIONAL,
                id.getTopologyBindingPath().augmentation(NetconfNode.class).child(ClusteredConnectionStatus.class));
        // FIXME: Catch possible read exception and decide what to do with it.
        final String currentMasterAddress = ccs == null ? null : ccs.getNetconfMasterNode();
        if (currentMasterAddress != null && !masterAddress.equals(currentMasterAddress)) {
