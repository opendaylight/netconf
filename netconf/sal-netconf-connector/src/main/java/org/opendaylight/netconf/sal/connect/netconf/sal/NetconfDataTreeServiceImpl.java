...
public class NetconfDataTreeServiceImpl implements NetconfDataTreeService {
    ...
    @Override
    public synchronized ListenableFuture<Optional<NormalizedNode<?, ?>>> get(YangInstanceIdentifier path) {
        return netconfOps.getData(new NetconfRpcFutureCallback("Data read", id), Optional.ofNullable(path));
    }

    @Override
    public synchronized ListenableFuture<Optional<NormalizedNode<?, ?>>> getConfig(final YangInstanceIdentifier path) {
        return netconfOps.getConfigRunningData(
                new NetconfRpcFutureCallback("Data read", id), Optional.ofNullable(path));
    }

    @Override
    public synchronized ListenableFuture<Optional<NormalizedNode<?, ?>>> readData(LogicalDatastoreType store,
