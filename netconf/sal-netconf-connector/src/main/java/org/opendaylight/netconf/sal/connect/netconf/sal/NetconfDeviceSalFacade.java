...
public final class NetconfDeviceSalFacade implements RemoteDeviceHandler, AutoCloseable {
    ...
    @Override
    public synchronized void onDeviceReconnected(final NetconfDeviceSchema deviceSchema,
            final NetconfSessionPreferences netconfSessionPreferences, final NetconfNode node) {
        // FIXME: right, so this is multiplexing to write confing ... or dump node capabilities to config
        //        if things do not pan out.
        salProvider.getTopologyDatastoreAdapter().updateDeviceData(ConnectionStatus.Connecting,
            // FIXME: CONFIGURATION?!
            deviceSchema.capabilities(), LogicalDatastoreType.CONFIGURATION, node);
    }
