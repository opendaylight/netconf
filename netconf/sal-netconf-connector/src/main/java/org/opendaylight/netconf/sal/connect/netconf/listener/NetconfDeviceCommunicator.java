...
public class NetconfDeviceCommunicator implements NetconfClientSessionListener, RemoteDeviceCommunicator<NetconfMessage> {
    ...
    private static void validation(final int rpcMessageLimit) {
        Preconditions.checkArgument(rpcMessageLimit > 0, "The limit of concurrent messages must be greater than zero");
    }
    ...
    public ListenableFuture<RpcResult<NetconfMessage>> sendRequest(final NetconfMessage message, final QName rpc) {
        ...
        if (!semaphore.tryAcquire()) {
            LOG.warn("Limit of concurrent rpc messages was reached. Rpc reply message is needed. Discarding request.");
            sessionLock.unlock();
