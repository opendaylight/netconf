...
abstract class AbstractNetconfDataTreeService implements NetconfDataTreeService {
    ...
    static @NonNull AbstractNetconfDataTreeService of(final RemoteDeviceId id, final MountPointContext mountContext,
        ...
        if (netconfSessionPreferences.isCandidateSupported()) {
            ...
            throw new IllegalArgumentException("Device " + id.getName() + " has advertised neither :writable-running "
                + "nor :candidate capability. Failed to establish session, as at least one of these must be advertised.");
        }
    }
