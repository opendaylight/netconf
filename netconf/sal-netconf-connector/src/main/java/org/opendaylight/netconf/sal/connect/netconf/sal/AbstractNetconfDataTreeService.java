...
public abstract class AbstractNetconfDataTreeService implements NetconfDataTreeService {
    ...
    public synchronized ListenableFuture<? extends DOMRpcResult> delete(final LogicalDatastoreType store,
            final YangInstanceIdentifier path, final NormalizedNode data, Optional<ModifyAction> defaultOperation) {
        checkEditable(store);
        return editConfig(netconfOps.createEditConfigStructure(Optional.ofNullable(data),
                Optional.of(ModifyAction.DELETE), path), defaultOperation.orElse(null));
    }
    ...
    public synchronized ListenableFuture<? extends DOMRpcResult> remove(final LogicalDatastoreType store,
        ...
        checkEditable(store);
        return editConfig(netconfOps.createEditConfigStructure(Optional.ofNullable(data),
                Optional.of(ModifyAction.REMOVE), path), defaultOperation.orElse(null));
    }
