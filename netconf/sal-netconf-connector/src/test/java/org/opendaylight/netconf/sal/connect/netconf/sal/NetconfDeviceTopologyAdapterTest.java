...
public class NetconfDeviceTopologyAdapterTest {
    ...
    public void testFailedDevice() throws Exception {
        ...
        verify(writeTx, times(2)).put(any(LogicalDatastoreType.class), any(InstanceIdentifier.class), any(NetconfNode.class));
        adapter.close();
    ...
    @Test
    public void testDeviceUpdateAugmentedNode() throws Exception {
        ...
        NormalizedNode<?, ?> augmenNode = ImmutableLeafNodeBuilder.create().withValue(dataTestId).withNodeIdentifier(new YangInstanceIdentifier.NodeIdentifier(nTestleafQname)).build();

        DOMDataWriteTransaction writeTx =  cDOMDataBroker.newWriteOnlyTransaction();
        writeTx.put(LogicalDatastoreType.OPERATIONAL, pathToAugmentedLeaf, augmenNode);
        writeTx.submit();

        adapter.updateDeviceData(true, new NetconfDeviceCapabilities());

        Optional<NormalizedNode<?, ?>> t1 = cDOMDataBroker.newReadOnlyTransaction().read(LogicalDatastoreType.OPERATIONAL, pathToAugmentedLeaf).checkedGet();

        assertEquals("Augmented node data should be inserted.", true, t1.isPresent());
        assertEquals("Augmented data should be the same as before update node.", dataTestId, t1.get().getValue());
    }
