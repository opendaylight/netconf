...
public class FieldsAwareReadOnlyTxTest {
    ...
    public void testReadWithFields() {
        ...
        verify(rpc).invokeRpc(eq(NetconfMessageTransformUtil.NETCONF_GET_CONFIG_QNAME), any(ContainerNode.class));

        readOnlyTx.read(LogicalDatastoreType.OPERATIONAL, YangInstanceIdentifier.empty());
        verify(rpc).invokeRpc(eq(NetconfMessageTransformUtil.NETCONF_GET_QNAME), any(ContainerNode.class));
    }
}