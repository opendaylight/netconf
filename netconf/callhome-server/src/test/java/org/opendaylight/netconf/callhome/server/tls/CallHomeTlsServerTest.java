...
public class CallHomeTlsServerTest {
    ...
    void integrationTest() throws Exception {
        ...
        // call-home server (tls client) having certificates for clients 1,2 only
        final var serverCtx = SslContextBuilder.forClient()
            .keyManager(keyManagerFor(serverCert))
            .trustManager(trustManagerFor(clientCert1, clientCert2)).build();

        // call-home clients (tls servers)
        ...
        final var authProvider = new CallHomeTlsAuthProvider() {
            ...
            public @Nullable String idFor(@NonNull PublicKey publicKey) {
                // identify client 1 only
                return clientCert1.keyPair.getPublic().equals(publicKey) ? "id" : null;
            }
...
//            .registerCapabilitiesListener(any(NetconfMonitoringService.CapabilitiesListener.class));
        ...
        final var contextMgr = new CallHomeTlsSessionContextManager(authProvider, statusRecorder) {
            ...
            public CallHomeTlsSessionContext createContext(final String id, final Channel channel) {
                return new CallHomeTlsSessionContext(id, channel, clientSessionListener);
            }
        };
        ...
        try {
            final var client1 = TLSServer.connect(
            ...
            final var client2 = TLSServer.connect(
                netconfTransportListener, bootstrapFactory.newBootstrap(), tcpConnectParams,
                channel -> clientCtx2.newHandler(channel.alloc())).get(TIMEOUT, TimeUnit.MILLISECONDS);

            try {

                // verify netconf session established with client1
                verify(clientSessionListener, timeout(TIMEOUT)).onSessionUp(any(NetconfClientSession.class));
                verify(serverSessionListener, timeout(TIMEOUT)).onSessionUp(any(NetconfServerSession.class));

            } finally {
                client1.shutdown().get(2, TimeUnit.SECONDS);
                client2.shutdown().get(2, TimeUnit.SECONDS);
            }
