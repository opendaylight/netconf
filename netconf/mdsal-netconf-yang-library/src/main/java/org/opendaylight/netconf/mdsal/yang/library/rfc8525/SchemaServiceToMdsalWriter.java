...
public class SchemaServiceToMdsalWriter implements EffectiveModelContextListener, AutoCloseable {
    ...
    public void onModelContextUpdated(final EffectiveModelContext context) {
        ...
        if (ietfYangLibraryModule != null) {
            final YangLibrary newModuleState = createYangLibraryFromContext(context.getModules());
            final WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
            tx.put(LogicalDatastoreType.OPERATIONAL, YANG_LIBRARY_INSTANCE_IDENTIFIER, newModuleState);

            LOG.debug("Trying to write new yang library: {}", newModuleState);
            tx.commit().addCallback(new FutureCallback<CommitInfo>() {
                @Override
