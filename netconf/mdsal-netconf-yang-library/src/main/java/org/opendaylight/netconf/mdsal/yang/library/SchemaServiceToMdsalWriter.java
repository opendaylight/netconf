...
public class SchemaServiceToMdsalWriter implements SchemaContextListener, AutoCloseable {
    ...
    private org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.yang.library.rev160621.module.list.Module
        ...
        module.getQNameModule().getRevision().ifPresent(rev -> moduleBuilder.setRevision(new Revision(rev.toString())));
    ...
    private static List<Submodule> createSubmodulesForModule(final Module module) {
        return module.getSubmodules().stream().map(subModule -> {
            ...
            subModule.getQNameModule().getRevision().ifPresent(
                rev -> subModuleEntryBuilder.setRevision(new Revision(rev.toString())));
            return subModuleEntryBuilder.build();
