...
public class SslConfigurationProviderImpl implements SslConfigurationProvider {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SslConfigurationProviderImpl.class);
    ...
    public SslConfigurationProviderImpl(DataBroker dataBroker) {
    ...
    public SslHandler getSslHandler() {
        if (allowedKeys.isEmpty()) {
            LOG.error("No associated keys for TLS authentication");
            throw new IllegalStateException("No associated keys for TLS authentication");
        }
        else {
            // TODO: use cache and update only if something changes
            return sslHandlerFactory.createSslHandler(new HashSet<>(allowedKeys.values()));
        }
    }
