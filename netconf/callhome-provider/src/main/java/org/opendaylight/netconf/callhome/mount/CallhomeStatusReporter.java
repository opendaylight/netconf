...
class CallhomeStatusReporter implements DataChangeListener, StatusRecorder, AutoCloseable {
    ...
    interface DecoderFactory {
        AuthorizedKeysDecoder create();
    }



    public CallhomeStatusReporter(DataBroker broker) {
    ...
    /* MODIFIED-DOWNSTREAM */
    public void asForceListedDevice(String id, PublicKey serverKey)
    {
    ...
    /* MODIFIED-DOWNSTREAM */
    public void asUnlistedDevice(String id, PublicKey serverKey)
    {
        NodeId nid = new NodeId(id);
    ...
    private Device newDevice(String id, PublicKey serverKey, Device1.DeviceStatus status)
    {
        ...
        try {
            sshEncodedKey = decoder.encodePublicKey(serverKey);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void setDeviceStatus(Device device) {
        ...
        InstanceIdentifier<Device> Device_IID =
                InstanceIdentifier.create(NetconfCallhomeServer.class).child(AllowedDevices.class).child(Device.class, device.getKey());
