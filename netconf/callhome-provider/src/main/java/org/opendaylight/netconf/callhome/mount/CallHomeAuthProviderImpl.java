...
public class CallHomeAuthProviderImpl implements CallHomeAuthorizationProvider, AutoCloseable {
    ...
    private class DeviceOp implements DataTreeChangeListener<Device> {
        ...
        @Override
        public void onDataTreeChanged(Collection<DataTreeModification<Device>> mod) {
            for (DataTreeModification<Device> dataTreeModification : mod) {
                DataObjectModification<Device> rootNode = dataTreeModification.getRootNode();
                process(rootNode);
            }
        }
        ...
        public Device get(PublicKey serverKey)
        {
            try
            {
                String skey = AuthorizedKeysDecoder.encodePublicKey(serverKey);
