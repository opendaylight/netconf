...
import org.opendaylight.yang.gen.v1.urn.brocade.params.xml.ns.yang.zerotouch.callhome.server.rev161109.Admin;
...
class CallHomeAuthProviderImpl implements CallHomeAuthorizationProvider {
    ...
    private List<Device> getAllDevices() {
        List<Device> callHomeDevices = new ArrayList<Device>();
        ...
        try {
            ...
            if (opt.isPresent()) {
                ...
                if (devices != null) {
                    LOG.debug("Returning {} devices", devices.getDevice().size());

                    for (Device device : devices.getDevice()) {
                        callHomeDevices.add(device);
                    }
