    ...
    public CallHomeAuthorization provideAuth(final SocketAddress remoteAddress, final PublicKey serverKey) {
        ...
        if (deviceSpecific != null) {
            ...
            if (deviceSpecific.getTransport() instanceof Ssh) {
                SshClientParams params = ((Ssh) deviceSpecific.getTransport()).getSshClientParams();
                username = params.getUsername();
                passwords.addAll(params.getPasswords());
            }
            else {
                username = deviceSpecific.getCredentials().getUsername();
                passwords.addAll(deviceSpecific.getCredentials().getPasswords());
            }
        } else {
            ...
            if (globalConfig.allowedUnknownKeys()) {
                ...
                passwords.addAll(globalConfig.getCredentials().getPasswords());
