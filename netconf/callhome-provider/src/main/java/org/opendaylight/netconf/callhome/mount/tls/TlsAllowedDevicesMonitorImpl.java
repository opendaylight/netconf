...
public class TlsAllowedDevicesMonitorImpl implements TlsAllowedDevicesMonitor, AutoCloseable {
    ...
    private static final ConcurrentMap<String, String> ALLOWED_DEVICES_KEYS = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String, String> ALLOWED_DEVICES_CERTIFICATES = new ConcurrentHashMap<>();
    ...
    @Override
    public String findDeviceIdByCertificate(@NonNull final Certificate certificate) {
        final Set<String> certificateNames = TRUSTED_CERTIFICATES.entrySet().stream().filter(
            v -> certificate.equals(v.getValue())).map(Map.Entry::getKey).collect(Collectors.toSet());
        final Set<String> deviceNames = ALLOWED_DEVICES_CERTIFICATES.entrySet().stream().filter(
    ...
    public Set<String> findAllowedKeys() {
        return new HashSet<>(ALLOWED_DEVICES_KEYS.values());
    }
    ...
    private static class CertificatesMonitor implements ClusteredDataTreeChangeListener<Keystore> {
        ...
        private X509Certificate buildCertificate(final String encoded) {
    ...
    private static class AllowedDevicesMonitor implements ClusteredDataTreeChangeListener<Device> {
        ...
        private void deleteDevice(final Device dataBefore) {
            if (dataBefore != null && dataBefore.getTransport() instanceof Tls) {
                ...
                ALLOWED_DEVICES_KEYS.remove(dataBefore.getUniqueId());
                ALLOWED_DEVICES_CERTIFICATES.remove(dataBefore.getUniqueId());
            }
        }
        ...
        private void writeDevice(final Device dataAfter) {
            if (dataAfter != null && dataAfter.getTransport() instanceof Tls) {
                ...
                ALLOWED_DEVICES_KEYS.putIfAbsent(dataAfter.getUniqueId(), clientParams.getKeyId());
                ALLOWED_DEVICES_CERTIFICATES.putIfAbsent(dataAfter.getUniqueId(), clientParams.getCertificateId());
            }
        }
    }
}
