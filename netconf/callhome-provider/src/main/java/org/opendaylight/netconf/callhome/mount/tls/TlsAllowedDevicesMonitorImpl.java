...
public class TlsAllowedDevicesMonitorImpl implements TlsAllowedDevicesMonitor {
    ...
    private static final ConcurrentMap<String, String> allowedKeys = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String, String> allowedCertificates = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String, X509Certificate> trustedCertificates = new ConcurrentHashMap<>();
    ...
    public String findDeviceIdByCertificate(@NonNull Certificate certificate) {
        Set<String> certificateNames = trustedCertificates.entrySet().stream().filter(
            v -> certificate.equals(v.getValue())).map(Map.Entry::getKey).collect(Collectors.toSet());
        Set<String> deviceNames = allowedCertificates.entrySet().stream().filter(
    ...
    private static class CertificatesMonitor implements ClusteredDataTreeChangeListener<Keystore> {
        ...
        private void deleteCertificate(final TrustedCertificate dataBefore) {
            if (dataBefore != null) {
                ...
                trustedCertificates.remove(dataBefore.getName());
            }
        }
        ...
        private void writeCertificate(final TrustedCertificate dataAfter) {
            if (dataAfter != null) {
                ...
                trustedCertificates.putIfAbsent(dataAfter.getName(), buildCertificate(dataAfter.getCertificate()));
            }
        }
    ...
    private static class AllowedDevicesMonitor implements ClusteredDataTreeChangeListener<Device> {
        ...
        private void deleteDevice(final Device dataBefore) {
            if (dataBefore != null) {
                ...
                allowedCertificates.remove(dataBefore.getUniqueId());
            }
        }
        ...
        private void writeDevice(final Device dataAfter) {
            if (dataAfter != null) {
                ...
                allowedKeys.putIfAbsent(dataAfter.getUniqueId(), dataAfter.getTlsKeyId());
                allowedCertificates.putIfAbsent(dataAfter.getUniqueId(), dataAfter.getTlsCertId());
            }
        }
    }
}
