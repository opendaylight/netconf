...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.netconf.callhome.server.rev161109.netconf.callhome.server.allowed.devices.device.endpoint.Tls;
...
public class TlsAllowedDevicesMonitorImpl implements TlsAllowedDevicesMonitor, AutoCloseable {
    ...
    public Set<String> findAllowedKeys() {
        return new HashSet<>(ALLOWED_KEYS.values());
    }
    ...
    private static class AllowedDevicesMonitor implements ClusteredDataTreeChangeListener<Device> {
        ...
        private void deleteDevice(final Device dataBefore) {
            if (dataBefore != null && dataBefore.getEndpoint() instanceof Tls) {
        ...
        private void writeDevice(final Device dataAfter) {
            if (dataAfter != null && dataAfter.getEndpoint() instanceof Tls) {
                LOG.debug("Adding device {}", dataAfter.getUniqueId());
                String certificateId = ((Tls) dataAfter.getEndpoint()).getTlsClientParams().getCertificateId();
                String keyId = ((Tls) dataAfter.getEndpoint()).getTlsClientParams().getKeyId();
                ALLOWED_KEYS.putIfAbsent(dataAfter.getUniqueId(), keyId);
                ALLOWED_CERTIFICATES.putIfAbsent(dataAfter.getUniqueId(), certificateId);
            }
        }
    }
}
