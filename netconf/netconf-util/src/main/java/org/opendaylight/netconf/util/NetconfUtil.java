...
public final class NetconfUtil {
    ...
    public static void writeFilter(final YangInstanceIdentifier query, final DOMResult result,
        ...
        try {
            try (NormalizedNodeStreamWriter writer =
                ...
                streamInstanceIdentifier(writer, context, query);
            }
        } finally {
            xmlWriter.close();
        }
    ...
    private static void streamInstanceIdentifier(final NormalizedNodeStreamWriter writer, final SchemaContext context,
            final YangInstanceIdentifier query) throws IOException {
        ...
        final StreamingContext<?> instanceIdToNodes = StreamingContext.fromSchemaAndQNameChecked(context,
            first.getNodeType());
        instanceIdToNodes.streamToWriter(writer, first, it);
    }
}
