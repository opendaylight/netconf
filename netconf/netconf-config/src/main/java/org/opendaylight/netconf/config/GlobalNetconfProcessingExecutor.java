...
public final class GlobalNetconfProcessingExecutor extends FlexibleThreadPoolWrapper {
    ...
    public GlobalNetconfProcessingExecutor(final GlobalNetconfThreadFactory threadFactory, final int minThreadCount,
            final int maxThreadCount, final long keepAlive) {
        super(minThreadCount, maxThreadCount, keepAlive, TimeUnit.MILLISECONDS, threadFactory);
    }
    ...
    static Map<String, ?> props(final GlobalNetconfThreadFactory threadFactory, final Configuration configuration) {
        return Map.of(
            "type", "global-netconf-ssh-scheduled-executor",
            PROP_THREAD_FACTORY, requireNonNull(threadFactory),
            PROP_KEEPALIVE, configuration.keepAliveMillisFlexibleThreadPool(),
            PROP_MIN_THREAD_COUNT, configuration.minThreadCountFlexibleThreadPool(),
            PROP_MAX_THREAD_COUNT, configuration.maxThreadCountFlexibleThreadPool());
    }
}
