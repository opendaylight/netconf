...
final class ExecutionThead implements Callable<Void> {
    ...
    private final List<Request> requestsToSend;
    ...
    ExecutionThead(final List<Integer> openDevices, final TesttoolParameters params) {
        ...
        requestsToSend = createRequests(openDevices, params);
    ...
    private List<Request> createRequests(final List<Integer> openDevices, final TesttoolParameters params) {
        ...
        for (int i = 0; i < batches.size(); i++) {
            ...
            requests.add(createRequest(payload, params));
        }
        return requests;
    }

    private Request createRequest(final String payload, final TesttoolParameters params) {
    ...
    private void invokeSync() {
        ...
        for (final Request request : requestsToSend) {
            try {
