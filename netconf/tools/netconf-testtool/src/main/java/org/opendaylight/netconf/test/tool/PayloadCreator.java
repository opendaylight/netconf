...
final class PayloadCreator {
    ...
    static String createStringPayload(final int batchNumber, final List<Integer> devices,
            final TesttoolParameters parameters) {
        return normalizedNodeToString(createNormalizedNodePayload(batchNumber, devices, parameters));
    }
    ...
    private static NormalizedNode createNormalizedNodePayload(final int batchNumber, final List<Integer> devices,
            final TesttoolParameters parameters) {
        final var nodeBuilder = Builders.mapBuilder().withNodeIdentifier(NodeIdentifier.create(Node.QNAME));
        final int portNumberOffset = batchNumber * parameters.getGenerateConfigBatchSize();
        for (int i = 0; i < devices.size(); i++) {
            final int portNumber = parameters.getStartingPort() + i + portNumberOffset;
            nodeBuilder.withChild(Builders.mapEntryBuilder()
                    .withNodeIdentifier(NodeIdentifierWithPredicates.of(Node.QNAME, NODE_ID_QNAME,
                            createNodeID(portNumber)))
                    .withChild(leafPort(portNumber))
