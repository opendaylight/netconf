...
public final class ScaleUtil {
    ...
    private static int execCommand(String command, int timeout) {
        ...
        try {
            ...
            root.debug("Cmd output: {}", CharStreams.toString(new BufferedReader(new InputStreamReader(process.getInputStream()))));
            String cmdErrorOutput = CharStreams.toString(new BufferedReader(new InputStreamReader(process.getErrorStream())));
            if (cmdErrorOutput.length() > 0) {
                root.warn("While running command {} received error output: {}", cmdErrorOutput);
                return FAILURE_CODE;
            }
            return exitValue;
        } catch (IOException | InterruptedException e) {
            root.warn("Command '{}' failed", command, e);
            return FAILURE_CODE;
       }
    }
    ...
    private static Response requestNetconfTopology() {
        AsyncHttpClient asyncHttpClient = new AsyncHttpClient(new Builder()
            ...
            .build());
        String RESTCONF_URL2 = "http://127.0.0.1:8181/restconf/operational/"
            + "network-topology:network-topology/topology/topology-netconf/";
        AsyncHttpClient.BoundRequestBuilder requestBuilder = asyncHttpClient.prepareGet(RESTCONF_URL2)
    ...
    private static void setUpLoggers(final TesttoolParameters params) {
        ...
        if (!params.debug) {
            ch.qos.logback.classic.Logger rpcHandlerLog = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger("org.opendaylight.netconf.test.tool.rpchandler");
            rpcHandlerLog.setLevel(Level.WARN);
            ch.qos.logback.classic.Logger configurationtLog = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger("org.opendaylight.netconf.test.tool.config");
            configurationtLog.setLevel(Level.WARN);
            ch.qos.logback.classic.Logger serverSessionLog = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger("org.opendaylight.netconf.shaded.sshd.server.session");
            serverSessionLog.setLevel(Level.WARN);
            ch.qos.logback.classic.Logger nettyServBootLog = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger("io.netty.bootstrap.ServerBootstrap");
    ...
    private static void startKaraf( final TesttoolParameters params) {
        ...
        final String FEATURE_INSTALL_CMD = clientPath + " -u karaf -p karaf feature:install " +  String.join(" ", features) + " -t 5000";
        root.warn("Doing feature install {}", FEATURE_INSTALL_CMD);
        while(execCommand(FEATURE_INSTALL_CMD, 30) != 0) {
            try{
