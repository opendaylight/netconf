...
public class NcmountServiceImpl implements NcmountService {
    ...
    private LoadingCache<String, KeyedInstanceIdentifier<Node, NodeKey>> mountIds = CacheBuilder.newBuilder()
    ...
    public NcmountServiceImpl(MountPointService mountPointService) {
    ...
    public ListenableFuture<RpcResult<WriteRoutesOutput>> writeRoutes(final WriteRoutesInput input) {
        try {
            final Optional<MountPoint> mountPoint = mountPointService.getMountPoint(mountIds.get(input.getMountName()));

            final DataBroker broker = mountPoint.get().getService(DataBroker.class).get();
            ...
            final Map<VrfPrefixKey, VrfPrefix> routes = new HashMap<>();
        ...
        final WriteRoutesOutputBuilder outputBuilder = new WriteRoutesOutputBuilder();
