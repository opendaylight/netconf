    ...
    NetconfConnectorDTO createDeviceCommunicator(final NodeId nodeId, final NetconfNode node,
        ...
        return new NetconfConnectorDTO(
                userCapabilities.isPresent() ? new NetconfDeviceCommunicator(remoteDeviceId, device,
                        new UserPreferences(userCapabilities.get(), Objects.isNull(
                                node.getYangModuleCapabilities()) ? false : node.getYangModuleCapabilities()
                                .isOverride(), Objects.isNull(
                                node.getNonModuleCapabilities()) ? false : node.getNonModuleCapabilities()
                                .isOverride()), rpcMessageLimit) : new NetconfDeviceCommunicator(remoteDeviceId, device,
                        rpcMessageLimit), salFacade);
    }
