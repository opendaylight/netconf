...
import org.opendaylight.netconf.topology.singleton.impl.tx.NetconfMasterDOMTransactions;
...
class MasterSalFacade implements AutoCloseable, RemoteDeviceHandler<NetconfSessionPreferences> {
    ...
    private void registerMasterMountPoint() {
        ...
        NetconfDOMTransactions masterDOMTransactions =
                new NetconfMasterDOMTransactions(id, remoteSchemaContext, deviceRpc, netconfSessionPreferences);
        deviceDataBroker =
    ...
    private Future<Object> sendInitialDataToActor() {
        List<SourceIdentifier> sourceIdentifiers =
                remoteSchemaContext.getAllModuleIdentifiers().stream().map(mi ->
                    SourceIdentifier.create(mi.getName(),
    ...
    private void updateDeviceData() {
        salProvider.getTopologyDatastoreAdapter().updateDeviceData(true,
