...
class NetconfNodeManager
        ...
        for (final DataTreeModification<Node> change : changes) {
            ...
            switch (rootNode.getModificationType()) {
                case SUBTREE_MODIFIED:
                    LOG.debug("Operational for node {} updated. Trying to register slave mount point",
                ...
                case WRITE:
                    ...
                    break;
                case DELETE:
                    LOG.debug("Operational for node {} deleted. Trying to remove slave mount point",
                            NetconfTopologyUtils.getNodeId(rootNode.getIdentifier()));
                    close();
                default:
                    LOG.debug("Uknown operation for node: {}", NetconfTopologyUtils.getNodeId(rootNode.getIdentifier()));
            }
        }
    }
    ...
    private void handleSlaveMountPoint(final DataObjectModification<Node> rootNode) {
        ...
        final NetconfNode netconfNode = rootNode.getDataAfter().getAugmentation(NetconfNode.class);
        if (NetconfNodeConnectionStatus.ConnectionStatus.Connected.equals(netconfNode.getConnectionStatus())) {
            createActorRef();
            final String masterAddress = netconfNode.getClusteredConnectionStatus().getNetconfMasterNode();
