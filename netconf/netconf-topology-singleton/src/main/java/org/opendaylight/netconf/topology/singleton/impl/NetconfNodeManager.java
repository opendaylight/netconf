...
class NetconfNodeManager
    ...
    private void closeActor() {
        if (slaveActorRef != null) {
            LOG.debug("Seding poison pill to {} on node {}", slaveActorRef, id);
    ...
    private void unregisterSlaveMountpoint() {
        if (slaveActorRef != null) {
            LOG.debug("Seding message to unregister slave mountpoint to {} on node {}", slaveActorRef, id);
    ...
    private void createOrUpdateActorRef() {
        if (slaveActorRef == null) {
            ...
            LOG.debug("Slave actor created for node {}, with name {}", id, slaveActorRef);
        } else {
            slaveActorRef
