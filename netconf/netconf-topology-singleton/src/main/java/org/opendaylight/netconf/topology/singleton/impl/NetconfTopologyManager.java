...
public class NetconfTopologyManager
            ...
            switch (rootNode.getModificationType()) {
                ...
                case WRITE:
                    if (contexts.containsKey(dataModifIdent)) {
                        ...
                        try {
                            if (checkDeviceNotExists(rootNode.getDataAfter())) {
                                startNetconfDeviceContext(dataModifIdent, rootNode.getDataAfter());
                            } else {
                                LOG.error("RemoteDevice{{}} was already configured", nodeId);
                            }
    ...
    private boolean checkDeviceNotExists(Node node) {
        final Topology topology = read(LogicalDatastoreType.OPERATIONAL, NETCONF_TOPOLOGY_IID, dataBroker);
        final NetconfNode netconfNode = node.augmentation(NetconfNode.class);
        if (topology == null) {
            return true;
        }
        final List<Host> hostList = new ArrayList<Host>();
        for (final Node nodeList : topology.nonnullNode().values()) {
            final NetconfNode netconfNodeList = nodeList.augmentation(NetconfNode.class);
            hostList.add(netconfNodeList.getHost());
        }
        if (hostList.contains(netconfNode.getHost())) {
            return false;
        }
        return true;
    }

    public static final InstanceIdentifier<Topology> NETCONF_TOPOLOGY_IID = InstanceIdentifier
