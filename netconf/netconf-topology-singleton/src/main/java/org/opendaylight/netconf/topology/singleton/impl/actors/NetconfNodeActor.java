...
public class NetconfNodeActor extends UntypedActor {
    ...
    public static Props props(final NetconfTopologySetupBuilder.NetconfTopologySetup setup,
                              final RemoteDeviceId id, final SchemaSourceRegistry schemaRegistry,
                              final SchemaRepository schemaRepository) {
        return Props.create(new Creator<NetconfNodeActor>() {
            private static final long serialVersionUID = 1L;

            @Override
            public NetconfNodeActor create() throws Exception {
                return new NetconfNodeActor(setup, id, schemaRegistry, schemaRepository);
            }
        });
    }

    private NetconfNodeActor(final NetconfTopologySetupBuilder.NetconfTopologySetup setup,
    ...
    public void onReceive(Object message) throws Exception {
        if (message instanceof CreateInitialMasterActorData) { // master
            ...
            this.sourceIdentifiers = ((CreateInitialMasterActorData) message).getSourceIndentifiers();
            this.operationsProcessor =
    ...
    private void sendYangTextSchemaSourceProxy(SourceIdentifier sourceIdentifier, ActorRef sender) {
        CheckedFuture<YangTextSchemaSource, SchemaSourceException> yangTextSchemaSource =
    ...
    private void invokeSlaveRpc(final Iterable<QName> path, final boolean absolute,
    ...
    private DOMRpcService getDOMRpcService(ActorRef masterReference) {
        return new ProxyDOMRpcService(setup.getActorSystem(), masterReference);
    }
