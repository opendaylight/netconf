...
class NetconfTopologyContext implements ClusterSingletonService {
    ...
    private final ClusterSingletonServiceRegistration clusterSingletonServiceRegistration = null;
    ...
    void refresh(final NetconfTopologySetup setup) {
        ...
        if (!isMaster) {
            netconfNodeManager.setSetup(netconfTopologyDeviceSetup, remoteDeviceId);
        }
        remoteDeviceConnector = new RemoteDeviceConnectorImpl(netconfTopologyDeviceSetup, remoteDeviceId);
