...
public class MountPointEndToEndTest {
    ...
    private void setupMaster() throws Exception {
        ...
        ByteSource topYangByteSource = new ByteSource() {
            @Override
            public InputStream openStream() throws IOException {
                return topModuleInfo.getModuleSourceStream();
            }
        };

        NetconfTopologyUtils.DEFAULT_SCHEMA_REPOSITORY.registerSchemaSource(
            id -> Futures.immediateFuture(YangTextSchemaSource.delegateForByteSource(id, topYangByteSource)),
            PotentialSchemaSource.create(RevisionSourceIdentifier.create(TOP_MODULE_NAME,
                    Revision.of(topModuleInfo.getRevision())), YangTextSchemaSource.class, 1));
    ...
    private RpcDefinition findRpcDefinition(String rpc) {
        Module topModule = deviceSchemaContext.findModule(TOP_MODULE_NAME,
                Revision.of(topModuleInfo.getRevision())).get();
