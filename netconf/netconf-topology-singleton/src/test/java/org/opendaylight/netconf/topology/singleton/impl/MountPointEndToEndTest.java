...
public class MountPointEndToEndTest {
    ...
    private void setupMaster() throws Exception {
        ConstantSchemaAbstractDataBrokerTest dataBrokerTest = newDataBrokerTest();
        ...
        final SchemaContext sc = dataBrokerTest.getDataBrokerTestCustomizer().getSchemaService().getGlobalContext();
        LOG.info("!!!!!MASTER SCHEMA: {}", sc);
        for (Module mod: sc.getModules()) {
            LOG.info("!!!MODULE {}: AUGS: {}", mod.getName(), mod.getAugmentations());
        }

        final BindingRuntimeTypes tm = new BindingGeneratorImpl().generateTypeMapping(sc);
        LOG.info("!!!!NetconfNode type: {}",
                tm.findAugmentation(new ReferencedTypeImpl(JavaTypeName.create(NetconfNode.class))));
    ...
    private ConstantSchemaAbstractDataBrokerTest newDataBrokerTest() throws Exception {
        ConstantSchemaAbstractDataBrokerTest dataBrokerTest = new ConstantSchemaAbstractDataBrokerTest(true) {
            @Override
            @SuppressWarnings("checkstyle:IllegalCatch")
            protected Iterable<YangModuleInfo> getModuleInfos() throws Exception {
                LOG.info("!!!IN getModuleInfos");
                try {
                    final YangModuleInfo mi = BindingReflections.getModuleInfo(NetconfNode.class);
                    LOG.info("!!!NetconfNode YangModuleInfo: name: {}, imported: {}, \nsource: {}", mi.getName(),
                            mi.getImportedModules(), mi.getYangTextCharSource().read());
                    return ImmutableSet.of(mi /*,
                    ...
                    LOG.error("getModuleInfos failed", e);
                    throw e;
                }
            }
        };
