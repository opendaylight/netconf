...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology.rev180703.netconf.node.connection.status.available.capabilities.AvailableCapabilityBuilder;
...
public class NetconfCommandsImplTest {
    ...
    private void createTopology(final LogicalDatastoreType dataStoreType)
        ...
        final Network topology = new NetworkBuilder()
                .withKey(new NetworkKey(new NetworkId(NetconfNetwork.QNAME.getLocalName())))
                .setNetworkId(new NetworkId(NetconfNetwork.QNAME.getLocalName())).setNode(nodes).build();
