...
public class NetconfCommandsImpl implements NetconfCommands {
    ...
    public Map<String, Map<String, List<String>>> showDevice(final String deviceId) {
        Map<String, Map<String, List<String>>> device = new HashMap<>();
        List<Node> nodeList = NetconfConsoleUtils.getNetconfNodeFromId(deviceId, dataBroker);
    ...
    @Override
    public void connectDevice(NetconfNode netconfNode) {
        // The netconf node id is the ip address with dots and the port
        // ex: 192.168.1.100:800 will give netconf1921681100800
        NodeId nodeId = new NodeId("netconf" + netconfNode.getHost().getIpAddress().getIpv4Address().getValue().replace(".", "") + netconfNode.getPort().getValue());
    ...
    public boolean disconnectDevice(final String deviceIp, final String devicePort) {
        ...
        final InstanceIdentifier<Node> netconfIid = NetconfIidFactory.netconfNodeIid(NetconfConsoleConstants.NETCONF_NODE_CONTROLLER);
        ...
        if (mountPointService.getMountPoint(netconfIid).isPresent()) {
            ...
            LOG.error("Mount point couldn't be retrieved for nodeId:{}", netconfNodeId);
            return result;
        }
    ...
    public String updateDevice(final String netconfNodeId, String username, String password, Map<String, String> updated) {
        ...
        if (node != null && node.getAugmentation(NetconfNode.class) != null) {
            ...
            Node updatedNode = new NodeBuilder()
                    .setKey(node.getKey())
