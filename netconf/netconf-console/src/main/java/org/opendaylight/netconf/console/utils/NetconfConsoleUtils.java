...
public class NetconfConsoleUtils {
    ...
    public NetconfConsoleUtils(DataBroker db) {
        this.db = db;
    }
     ...
     * @return
     */
    public List<Node> getNetconfNodeFromIp(String deviceIp) {
        final Topology topology = read(LogicalDatastoreType.OPERATIONAL, NetconfIidFactory.getNetconfNodeIid());
     ...
     * @return
     */
    public List<Node> getNetconfNodeFromIp(String deviceIp, String devicePort) {
        final Topology topology = read(LogicalDatastoreType.OPERATIONAL, NetconfIidFactory.getNetconfNodeIid());
        if (topology == null || topology.getNode() == null || topology.getNode().isEmpty()) {
            return null;
        }
     ...
     */
    public static boolean pushConfigs(String blueprint, ConfigPusher configPusher) {
        if (blueprint == null) {
            return false;
        }
     ...
     */
    public String connectDeviceNetconfPayload(String deviceIp, String devicePort,
            String username, String password, Boolean tcpOnly, String name) {
        final InputStream stream = this.getClass().getResourceAsStream(NetconfConsoleConstants.NETCONF_CONNECTOR_XML);
     ...
     */
    public Topology read(final LogicalDatastoreType store, final InstanceIdentifier<Topology> path) {
