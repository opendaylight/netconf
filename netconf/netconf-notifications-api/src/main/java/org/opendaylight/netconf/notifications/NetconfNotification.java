...
public final class NetconfNotification extends NetconfMessage {
    ...
    public static final Function<String, Date> RFC3339_DATE_PARSER = time -> {
        try {
            final LocalDateTime localDateTime = LocalDateTime.parse(time, DATE_TIME_FORMATTER);
            final int startAt = 0;
            final TemporalAccessor parsed = DATE_TIME_FORMATTER.parseUnresolved(time, new ParsePosition(startAt));
            final int zoneOffset = getFieldFromTemporalAccessor(parsed, ChronoField.OFFSET_SECONDS);
            ...
            // Log warn in case we rounded the fraction of a second
            if (reminder != 0) {
