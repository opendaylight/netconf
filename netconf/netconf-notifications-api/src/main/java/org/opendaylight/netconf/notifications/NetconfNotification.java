...
public final class NetconfNotification extends NetconfMessage {
     ...
     * format having a leap second. If that's the case, a simple conversion is applied, replacing
     * the second-of-minute of 60 with 59.
    ...
    private static Date potentiallyHandleLeapSecond(final String time) {
        ...
        if (parsed.isSupported(ChronoField.SECOND_OF_MINUTE)) {
            long secondOfMinute = parsed.getLong(ChronoField.SECOND_OF_MINUTE);

            // We have a time containing leap second - adjusting
            if (Long.compareUnsigned(secondOfMinute, 60) == 0) {
                ...
                int year = parsed.isSupported(ChronoField.YEAR) ? parsed.get(ChronoField.YEAR) : 0;
                int monthOfYear = parsed.isSupported(ChronoField.MONTH_OF_YEAR) ? parsed.get(ChronoField.MONTH_OF_YEAR) : 0;
                int dayOfMonth = parsed.isSupported(ChronoField.DAY_OF_MONTH) ? parsed.get(ChronoField.DAY_OF_MONTH) : 0;
                int hourOfDay = parsed.isSupported(ChronoField.HOUR_OF_DAY) ? parsed.get(ChronoField.HOUR_OF_DAY) : 0;
                int minuteOfHour = parsed.isSupported(ChronoField.MINUTE_OF_HOUR) ? parsed.get(ChronoField.MINUTE_OF_HOUR) : 0;
                // If the time '23:59:60' is received, then a simple conversion is applied, replacing the second-of-minute of 60 with 59.
                secondOfMinute = secondOfMinute - 1;
                int nanoOfSecond = parsed.isSupported(ChronoField.NANO_OF_SECOND) ? parsed.get(ChronoField.NANO_OF_SECOND) : 0;
                int offsetSeconds = parsed.isSupported(ChronoField.OFFSET_SECONDS) ? parsed.get(ChronoField.OFFSET_SECONDS) : 0;
