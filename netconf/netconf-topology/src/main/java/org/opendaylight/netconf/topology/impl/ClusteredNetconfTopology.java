...
public class ClusteredNetconfTopology extends AbstractNetconfTopology implements AutoCloseable {
    ...
    private TopologyMountPointFacade obtainTopologyMountPointFacade(NetconfConnectorDTO netconfConnectorDTO) {
        RemoteDeviceHandler<NetconfSessionPreferences> facade = null;
        TopologyMountPointFacade result = null;

        if (netconfConnectorDTO.getFacade() instanceof KeepaliveSalFacade) {
            facade = ((KeepaliveSalFacade) netconfConnectorDTO.getFacade()).getSalFacade();
            if (facade instanceof TopologyMountPointFacade) {
                result = (TopologyMountPointFacade) facade;
            } else {
                result = null;
            }
        } else if (netconfConnectorDTO.getFacade() instanceof TopologyMountPointFacade) {
            result = ((TopologyMountPointFacade) netconfConnectorDTO.getFacade());
        } else {
            LOG.error("Wrong type of facade in NetconfConnectorDTO");
        }
