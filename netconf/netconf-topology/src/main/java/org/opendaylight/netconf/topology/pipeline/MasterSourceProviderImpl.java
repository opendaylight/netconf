...
public class MasterSourceProviderImpl extends RemoteYangTextSourceProviderImpl
    ...
    public void preStart() {
        ...
        for(Member node : cluster.state().getMembers()) {
            NetconfTopologyPathCreator pathCreator = new NetconfTopologyPathCreator(node.address().toString(),topologyId);
            final String path = pathCreator.withSuffix(nodeId).withSuffix(NetconfTopologyPathCreator.CLUSTEREDDEVICESOURCESRESOLVER).build();
