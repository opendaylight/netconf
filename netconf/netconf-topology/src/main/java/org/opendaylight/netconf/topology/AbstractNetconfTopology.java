...
public abstract class AbstractNetconfTopology implements NetconfTopology {
    ...
    private void encryptIfNeeded(final NodeId nodeId, final NetconfNode netconfNode) {
        final org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology.rev150114.netconf.node.credentials
             .credentials.LoginPassword creds = (org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology
             .rev150114.netconf.node.credentials.credentials.LoginPassword)netconfNode.getCredentials();
        // Check if password is not able to decrypt(ie. not encrypted), then try to encrypt it.
        // This code relies on the fact that encryption service returns same string if decryption is not possible.
        if (encryptionService.decrypt(creds.getPassword()).equals(creds.getPassword())) {
            ...
            org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology.rev150114.netconf.node.credentials
                .credentials.LoginPasswordBuilder passwordBuilder = new org.opendaylight.yang.gen.v1.urn.opendaylight
                .netconf.node.topology.rev150114.netconf.node.credentials.credentials.LoginPasswordBuilder();
