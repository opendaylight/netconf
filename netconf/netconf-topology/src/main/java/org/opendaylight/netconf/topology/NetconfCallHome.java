...
/*            synchronized (openedSessions) {
                if (openedSessions.containsKey(inetSocketAddress.getAddress().getHostAddress())) {
                    LOG.trace("{} session already exist", nodeId);
                    return;
                }
            }*/
            ...
            final NetconfReversedClientConfiguration clientConfig = getClientConfig(netconfConnectorDTO.getCommunicator(), ioSession,
    ...
    private NetconfReversedClientConfiguration getClientConfig(final NetconfDeviceCommunicator listener,
        ...
        return new NetconfReversedClientConfiguration(1000L, inetSocketAddress, null, listener, new AuthenticationHandler() {
            @Override
            ...
            public AuthFuture authenticate(final ClientSession session) throws IOException {
                LOG.trace("Authenticating session {}", session);
    ...
    public void sessionClosed(final IoSession ioSession) throws Exception {
        ...
        synchronized (openedSessions) {
            ...
            openedSessions.remove(ioSession.getId());
        }
    }
}