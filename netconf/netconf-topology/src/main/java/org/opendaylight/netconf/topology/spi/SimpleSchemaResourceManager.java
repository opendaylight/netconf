 ...
 */
@Beta
public final class SimpleSchemaResourceManager implements SchemaResourceManager {
    private static final Logger LOG = LoggerFactory.getLogger(SimpleSchemaResourceManager.class);
    ...
    @Override
    public SchemaResourcesDTO getSchemaResources(final NodeId nodeId, final NetconfNode node) {
        ...
        if (Strings.isNullOrEmpty(subdir)) {
            ...
            LOG.warn("schema-cache-directory for {} is null or empty;  using the default {}", nodeId.getValue(),
                defaultSubdirectory);
            return defaultResources;
        }

        LOG.info("Netconf connector for device {} will use schema cache directory {} instead of {}", nodeId.getValue(),
            subdir, defaultSubdirectory);
        return getSchemaResources(nodeId, subdir);
    }

    private synchronized @NonNull SchemaResourcesDTO getSchemaResources(final NodeId nodeId, final String subdir) {
