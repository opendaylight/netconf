...
public final class OSGiNetconfThreadGroups {
    ...
    public OSGiNetconfThreadGroups(final Configuration configuration) {
        bossGroup = buildEventLoopGroup("netconf-boss-group", configuration.bossThreadCount());
        workerGroup = buildEventLoopGroup("netconf-worker-group", configuration.workerThreadCount());
    }
    ...
    private static EventLoopGroup buildEventLoopGroup(final String name, final int threads) {
        return NettyTransportSupport.newEventLoopGroup(name, Math.max(threads, 0));
    }
}
