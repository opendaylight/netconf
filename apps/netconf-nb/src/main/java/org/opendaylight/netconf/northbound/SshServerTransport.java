...
public final class SshServerTransport extends BaseServerTransport implements AutoCloseable {
    ...
    @Activate
    public SshServerTransport(@Reference final NettyThreads threads, @Reference final OSGiNetconfServer backend,
            @Reference(target = "(type=netconf-auth-provider)") final AuthProvider authProvider,
            final Configuration configuration) {
        this(threads, backend, authProvider, new TcpServerParametersBuilder()
    ...
    public SshServerTransport(final NettyThreads threads, final OSGiNetconfServer backend,
            final AuthProvider authProvider, final TcpServerGrouping listenParams) {
        super(backend.serverChannelInitializer());
        ...
        try {
            sshServer = SSHServer.listen(this, threads.newServerBootstrap(), listenParams, null,
                factoryMgr -> {
