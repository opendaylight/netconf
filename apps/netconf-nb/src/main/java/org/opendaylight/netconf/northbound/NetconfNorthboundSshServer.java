...
import org.opendaylight.netconf.server.NetconfSshCommandFactory;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.server.transport.config.rev230512.server.transport.config.grouping.ServerTcpConfig;
...
public final class NetconfNorthboundSshServer implements AutoCloseable {
    ...
    public NetconfNorthboundSshServer(final NetconfServerFactory netconfServerFactory,
        ...
        final ServerTcpConfig tcpConfig = new ServerTcpConfigBuilder()
        ...
        final Consumer<ServerFactoryManager> serverInitializer = factoryMgr -> {
            ...
            if (factoryMgr instanceof SshServer server) {
                // workaround to set subsystem
                server.setSubsystemFactories(ImmutableList.of(NetconfSshCommandFactory.INSTANCE));
            }
        };

        try {
            sshServer = netconfServerFactory.createSshServer(tcpConfig, serverInitializer).get();
