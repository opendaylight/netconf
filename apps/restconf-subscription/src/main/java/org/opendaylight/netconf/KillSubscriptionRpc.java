...
public class KillSubscriptionRpc extends RpcImplementation {
    ...
    @Inject
    @Activate
    public KillSubscriptionRpc(@Reference final MdsalNotificationService mdsalService,
            ...
            .addCallback(new FutureCallback<CommitInfo>() {
                ...
                public void onSuccess(CommitInfo result) {
                    request.completeWith(ImmutableNodes.newContainerBuilder()
                        ...
                        //.withChild(ImmutableNodes.leafNode(QName.create(Subscription.QNAME, "result"), "OK"))
                        .build());
                    try {
                        subscriptionStateService.subscriptionTerminated(Instant.now().toString(),
                            id.longValue(), "subscription-kill");
                    } catch (InterruptedException e) {
                        LOG.info("Could not send subscription terminated notification: {}", e.getMessage());
                    }
                }
