...
public class EstablishSubscriptionRpc extends RpcImplementation {
    ...
    public EstablishSubscriptionRpc(@Reference final RestconfStream.Registry streamRegistry,
            @Reference final MdsalNotificationaService mdsalService) {
    ...
    public void invoke(final ServerRequest<ContainerNode> request, final URI restconfURI, final OperationInput input) {
        ...
        final var node = ImmutableNodes.newMapEntryBuilder()
            .withNodeIdentifier(YangInstanceIdentifier.NodeIdentifierWithPredicates.of(Subscription.QNAME, QNAME_ID, id))
            ...
            .build();
        mdsalService.writeSubscription(SUBSCRIPTIONS.node(node.name()), node);
