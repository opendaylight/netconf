...
final class CallHomeListenEndpoints {
    ...
    /**
     * Returns the SSH Call Home listener bind address, or {@code null} when no SSH endpoint is configured.
     */
    ...
    /**
     * Returns the TLS Call Home listener bind address, or {@code null} when no TLS endpoint is configured.
     */
    ...
    /**
     * Returns the SSH transport parameters (host-key/key-exchange/encryption/MAC algorithms) of the first
     * configured SSH endpoint, or {@code null} when none is configured.
     */
    ...
    private static @Nullable InetSocketAddress readBind(final DataBroker broker, final boolean ssh) {
        ...
        for (var endpoint : endpoints.nonnullEndpoint().values()) {
            ...
            if (ssh && transport instanceof Ssh sshCase && sshCase.getSsh() != null) {
                tcp = sshCase.getSsh().getTcpServerParameters();
            } else if (!ssh && transport instanceof Tls tlsCase && tlsCase.getTls() != null) {
                tcp = tlsCase.getTls().getTcpServerParameters();
            } else {
                continue;
            }

            final var bind = tcp == null ? null : tcp.nonnullLocalBind().values()
                .stream()
                .findFirst()
                .orElse(null);
