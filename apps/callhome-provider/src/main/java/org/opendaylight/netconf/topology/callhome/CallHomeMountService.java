...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.device.rev240319.connection.parameters.Protocol;
...
public final class CallHomeMountService implements AutoCloseable {
    ...
    public @interface Configuration {
        ...
        @AttributeDefinition(description = "Multiplier for backoff timeout. The backoff will be multiplied by this "
            + "value with every additional attempt.", min = "0")
        double backoff$_$multiplier() default 1.5;

        @AttributeDefinition(description = "Range of backoff randomization. The backoff will be multiplied by a "
        ...
        @AttributeDefinition(description = "Limit of concurrent messages that can be send before reply messages"
        ...
        @AttributeDefinition(description = "The operation allows the client to lock the entire configuration "
            + "datastore system of a device.")
        boolean lock$_$datastore() default true;

        @AttributeDefinition(description = "If true, the connector would auto disconnect/reconnect when schemas are "
