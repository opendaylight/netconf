 ...
 *     context the {@link CallHomeTopology#ensureNode(Node)} is called using synthetic {@link Node} instance composed
 *     via {@link #asNode(String, SocketAddress)}. This triggers Netconf client construct/connect logic
...
public final class CallHomeMountService implements AutoCloseable {
    ...
    public CallHomeMountService(
            ...
            @Reference final BaseNetconfSchemas baseSchemas,
        ...
        this(NetconfNodeUtils.DEFAULT_TOPOLOGY_NAME, eventExecutor, scheduledThreadPool, processingThreadPool,
            schemaRepositoryProvider, baseSchemas, dataBroker, mountService, deviceActionFactory);
    }

    public CallHomeMountService(final String topologyId, final EventExecutor eventExecutor,
        ...
        topology = new CallHomeTopology(topologyId, clientFactory, eventExecutor, scheduledThreadPool,
            processingThreadPool, schemaRepositoryProvider, dataBroker, mountService, clientConfBuilderFactory,
    ...
    @Override
    public void close() {
        netconfLayerMapping.entrySet().forEach(entry -> entry.getValue().netconfSessionFuture.cancel(true));
