...
public class DeviceNotificationAggregate extends AbstractNotificationSource implements DOMMountPointListener,
        AutoCloseable {
        ...
        try (var roTx = dataBroker.newReadOnlyTransaction()) {
            ...
            optNotifications.addCallback(new FutureCallback<>() {
                ...
                public void onSuccess(Optional<Notifications> result) {
                    ...
                    optNotification.orElseThrow().registerNotificationListener(new NodeAwareListener(sink,
                ...
                public void onFailure(Throwable t) {
                    LOG.error("Error while reading notifications settings {}. Ignoring mount point {}",
                            notificationObjId, devicePath, t);
                }
    ...
    private static Optional<String> getTopologyId(final YangInstanceIdentifier path) {
        var optTopologyPathArg = path.getPathArguments().stream()
                ...
                .findFirst();
        if (optTopologyPathArg.isPresent() && optTopologyPathArg.orElseThrow() instanceof YangInstanceIdentifier.NodeIdentifierWithPredicates topologyPathArg) {
