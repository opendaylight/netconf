...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.device.rev251205.negotiated.ssh.parameters.NegotiatedSshTransportParameters;
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.device.rev251205.negotiated.ssh.parameters.NegotiatedSshTransportParametersBuilder;
...
public final class NetconfDeviceTopologyAdapter implements FutureCallback<Empty> {
    ...
    public synchronized void updateDeviceData(final boolean up, final NetconfDeviceCapabilities capabilities,
            final SessionIdType sessionId, final NegotiatedSshKeys negotiatedSshAlg) {
        final var tx = txChain.newWriteOnlyTransaction();
    ...
    private NegotiatedSshTransportParameters getNegotiatedParameters(final NegotiatedSshKeys negotiatedSshAlg) {
