...
public final class NetconfNodeHandler extends AbstractRegistration implements RemoteDeviceHandler {
    ...
    public NetconfNodeHandler(final NetconfClientFactory clientFactory, final Timer timer,
        ...
        maxSleep = node.requireMaxTimeoutBetweenAttemptsMillis().toJava();
    ...
    private synchronized Exception scheduleReconnect() {
        ...
        // randomSleepFactor which is in range +- 0.1 of set sleepFactor (default 1.5).
        if (attempts != 0) {
            final double min = sleepFactor - 0.1;
            final double max = sleepFactor + 0.1;
            final double randomSleepFactor = Math.random() * (max - min) + min;
