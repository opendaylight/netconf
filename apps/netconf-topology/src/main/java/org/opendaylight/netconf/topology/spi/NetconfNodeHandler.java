...
public final class NetconfNodeHandler extends AbstractRegistration implements RemoteDeviceHandler {
    ...
    private void lockedConnect() {
        ...
        final var algListener = new SSHNegotiatedAlgListener() {
            @Override
            public void onKexNegotiated(final String kexAlgorithm, final String hostKey, final String encryption,
                    final String mac) {
                onAlgorithmsNegotiated(new NegotiatedSshKeys(kexAlgorithm, hostKey, encryption, mac));
            }
        };
    ...
    public void onAlgorithmsNegotiated(final NegotiatedSshKeys kexAlgorithm) {
        delegate.onAlgorithmsNegotiated(kexAlgorithm);
    }
