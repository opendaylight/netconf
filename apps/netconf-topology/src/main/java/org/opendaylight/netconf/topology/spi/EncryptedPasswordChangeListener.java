...
public class EncryptedPasswordChangeListener implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    public void onDataTreeChanged(@NonNull final Collection<DataTreeModification<Node>> changes) {
        for (final var change : changes) {
            ...
            final var netconfAugmentation = rootNode.getModifiedAugmentation(NetconfNode.class);

            String passwordBefore = null;
            String passwordAfter = null;
            if (netconfAugmentation != null) {
                passwordBefore = getPassword(netconfAugmentation.getDataBefore());
                passwordAfter = getPassword(netconfAugmentation.getDataAfter());
            }

            if (isEncryptionRequired(passwordBefore, passwordAfter)) {
                final var credentialsAfter = netconfAugmentation.getDataAfter().getCredentials();
                Credentials credentials = handleEncryption(credentialsAfter);
    ...
    private boolean isEncryptionRequired(final String passwordBefore, final String passwordAfter) {
        ...
        try {
            ...
            return !decryptedPassword.equals(passwordBefore);
        } catch (Exception e) {
            return true;
        }
    }
    ...
    private static String getPassword(final NetconfNode node) {
        if (node != null) {
            final Credentials credentialsBefore = node.getCredentials();
