...
public final class NetconfTopologyRPCProvider implements AutoCloseable {
    ...
    NetconfNode encryptPassword(final CreateDeviceInput input) {
        ...
        return builder.setCredentials(translate(handleEncryption(input.getCredentialsRpc())))
            .build();
    }

    private CredentialsRpc handleEncryption(final CredentialsRpc credentials) {
        if (credentials instanceof LoginPwRpc loginPw) {
            final var loginPassword = loginPw.getLoginPasswordRpc();

            return new LoginPwRpcBuilder()
                .setLoginPasswordRpc(new LoginPasswordRpcBuilder()
        ...
        if (credentialsRpc instanceof LoginPwRpc loginPw) {
            ...
            return new LoginPwBuilder()
                .setLoginPassword(new LoginPasswordBuilder()
                    .setUsername(loginPassword.getUsername())
                    .setPassword(loginPassword.getPassword().getBytes(StandardCharsets.US_ASCII))
                    .build())
                .build();
