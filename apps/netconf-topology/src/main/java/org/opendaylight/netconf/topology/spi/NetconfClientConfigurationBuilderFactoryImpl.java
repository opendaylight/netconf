...
public final class NetconfClientConfigurationBuilderFactoryImpl implements NetconfClientConfigurationBuilderFactory {
    ...
    private void setSshParametersFromCredentials(final NetconfClientConfigurationBuilder confBuilder,
        ...
        if(node.getProtocol().getSpecification() instanceof SshCase sshCase) {
            ...
            // TODO: if these values are not present use topology values
            transportParamBuilder.setHostKey(sshParams.getHostKey());
            transportParamBuilder.setEncryption(sshParams.getEncryption());
            transportParamBuilder.setKeyExchange(sshParams.getKeyExchange());
            transportParamBuilder.setMac(sshParams.getMac());
    ...
    private @NonNull TransportParams parseTransportParams(final NetconfNode node) {
        ...
        final var kex = kexRaw.isEmpty() ? new String[0] : COMMA_WITH_WHITESPACE.split(kexRaw);
        ...
        final var macs = macsRaw.isEmpty() ? new String[0] : COMMA_WITH_WHITESPACE.split(macsRaw);
        ...
        final var encryptions = encryptionsRaw.isEmpty() ? new String[0] : COMMA_WITH_WHITESPACE.split(encryptionsRaw);
        ...
        final var hostKeys = hostKeysRaw.isEmpty() ? new String[0] : COMMA_WITH_WHITESPACE.split(hostKeysRaw);
