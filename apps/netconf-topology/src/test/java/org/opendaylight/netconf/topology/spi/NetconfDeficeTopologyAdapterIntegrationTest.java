...
public class NetconfDeficeTopologyAdapterIntegrationTest {
    ...
    public void setUp() throws Exception {
        ...
        adapter = new NetconfDeviceTopologyAdapter(dataBroker, TEST_TOPOLOGY_ID, ID,
            new NetconfNodeBuilder().setCredentials(new LoginPwUnencryptedBuilder()
    ...
    public void testDeviceAugmentedNodePresence() throws Exception {
        ...
        final var loginPasswordUnencryptedNode = Builders.containerBuilder()
            ...
            .build();
        final var choiceNode = Builders.choiceBuilder()
            .withNodeIdentifier(new YangInstanceIdentifier.NodeIdentifier(credentialsQname))
            .withChild(loginPasswordUnencryptedNode)
            .build();

        final var nodeList = ImmutableNodes.mapEntryBuilder(Node.QNAME, QName.create(Node.QNAME, "node-id"), "test")
        ...
        //assertNormalizedNodeListEquals("Data mismatch", expectedData, actualData);
        ...
        final var nodeMessageLeaf = ImmutableNodes.leafNode(QName.create(Node.QNAME, "connected-message"),
            "Unknown reason");
        final var nodeStatusFiledLeaf = ImmutableNodes.leafNode(QName.create(Node.QNAME, "connection-status"),
            "unable-to-connect");
        final var expectedDataFailedDevice = new ArrayList<NormalizedNode>();

        expectedDataFailedDevice.add(nodeIdLeaf);
        expectedDataFailedDevice.add(choiceNode);
        expectedDataFailedDevice.add(augmentNode);
        expectedDataFailedDevice.add(nodeSessionIdLeaf);
        expectedDataFailedDevice.add(nodeStatusLeaf);
        expectedDataFailedDevice.add(nodeHostLeaf);
        expectedDataFailedDevice.add(nodePortLeaf);
/*        assertNormalizedNodeListEquals("Failed device data mismatch", expectedDataFailedDevice,
            actualDataFailedDevice);*/
        assertEquals(expectedData, actualDataFailedDevice);
    }


/*    private static void assertNormalizedNodeListEquals(String message, List<NormalizedNode> expected,
        List<NormalizedNode> actual) {
        assertEquals(message + ": lists differ in size", expected.size(), actual.size());

        for (int i = 0; i < expected.size(); i++) {
            final var expectedNode = expected.get(i);
            final var actualNode = actual.get(i);

            // Assuming NormalizedNode has a properly overridden equals method
            // If not, you might need a more detailed comparison based on the properties of NormalizedNode
            assertEquals(message + ": mismatch at index " + i, expectedNode, actualNode);
        }
    }*/
}
