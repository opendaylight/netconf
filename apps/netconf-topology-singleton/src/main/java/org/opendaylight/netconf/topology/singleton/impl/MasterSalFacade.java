...
class MasterSalFacade implements RemoteDeviceHandler, AutoCloseable {
    ...
    public void onDeviceConnected(final NetconfDeviceSchema deviceSchema,
        ...
        sendInitialDataToActor().onComplete(new OnComplete<>() {
            ...
            public void onComplete(final Throwable failure, final Object success) {
                if (failure == null) {
                    updateDeviceData(negotiatedSshAlg);
                    return;
                }
    ...
    private Future<Object> sendInitialDataToActor() {
        ...
        return Patterns.ask(masterActorRef, new CreateInitialMasterActorData(deviceDataBroker,
            dataStoreService, sourceIdentifiers, deviceServices), actorResponseWaitTime);
    }

    private void updateDeviceData(final NegotiatedSshAlg negotiatedSshAlg) {
        ...
        datastoreAdapter.updateClusteredDeviceData(true, masterAddress, currentSchema.capabilities(),
                netconfSessionPreferences.sessionId(), negotiatedSshAlg);
    }
