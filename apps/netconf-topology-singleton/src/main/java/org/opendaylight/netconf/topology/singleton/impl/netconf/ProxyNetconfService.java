...
public class ProxyNetconfService implements DataStoreService {
    ...
    @Override
    public ListenableFuture<? extends DOMRpcResult> editConfig(EffectiveOperation operation, NormalizedNode child,
        YangInstanceIdentifier path) {
        final SettableFuture<DOMRpcResult> returnFuture = SettableFuture.create();
        processNetconfOperation(facade -> returnFuture.setFuture(facade.editConfig(operation, child, path)));
        return returnFuture;
    }

    @Override
    public ListenableFuture<? extends DOMRpcResult> editConfig(EffectiveOperation operation,
            YangInstanceIdentifier path) {
        final SettableFuture<DOMRpcResult> returnFuture = SettableFuture.create();
        processNetconfOperation(facade -> returnFuture.setFuture(facade.editConfig(operation, path)));
        return returnFuture;
    }

    @Override
    public ListenableFuture<? extends DOMRpcResult> editConfig(AnyxmlNode<DOMSource> node) {
        final SettableFuture<DOMRpcResult> returnFuture = SettableFuture.create();
        processNetconfOperation(facade -> returnFuture.setFuture(facade.editConfig(node)));
        return returnFuture;
    }
    ...
    public ListenableFuture<Optional<NormalizedNode>> read(LogicalDatastoreType store,
        ...
        processNetconfOperation(facade -> returnFuture.setFuture(facade.read(store, path, fields)));
        return returnFuture;
    }
    ...
    public ListenableFuture<Optional<NormalizedNode>> read(LogicalDatastoreType store,
            YangInstanceIdentifier path) {
        final SettableFuture<Optional<NormalizedNode>> returnFuture = SettableFuture.create();
        processNetconfOperation(facade -> returnFuture.setFuture(facade.read(store, path)));
        return returnFuture;
    }
