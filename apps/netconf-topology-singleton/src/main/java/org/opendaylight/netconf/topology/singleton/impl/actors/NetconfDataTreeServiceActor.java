...
public final class NetconfDataTreeServiceActor extends UntypedAbstractActor {
    ...
    private final DataOperationService netconfService;
    private final long idleTimeout;

    private NetconfDataTreeServiceActor(final DataOperationService netconfService, final Duration idleTimeout) {
    ...
    static Props props(final DataOperationService netconfService, final Duration idleTimeout) {
    ...
    public void onReceive(final Object message) {
        if (message instanceof GetRequest getRequest) {
            final YangInstanceIdentifier path = getRequest.getPath().instance();
            final ListenableFuture<Optional<NormalizedNode>> future = netconfService.getData(
                    getRequest.getPath(), getRequest.getParams());
            context().stop(self());
            sendResult(future, path, sender(), self());
        } else if (message instanceof MergeEditConfigRequest request) {
            netconfService.mergeData(
