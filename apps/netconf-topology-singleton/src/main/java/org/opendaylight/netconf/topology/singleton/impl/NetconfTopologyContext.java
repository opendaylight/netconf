...
class NetconfTopologyContext implements ClusterSingletonService, AutoCloseable {
    ...
    NetconfTopologyContext(final SchemaResourceManager schemaManager,
        ...
        remoteDeviceId = NetconfNodeUtils.toRemoteDeviceId(setup.getNode().getNodeId(),
            setup.getNode().augmentation(NetconfNodeAugment.class).getNetconfNode());
    ...
    void refresh(final @NonNull NetconfTopologySetup setup) {
        ...
        remoteDeviceId = NetconfNodeUtils.toRemoteDeviceId(node.getNodeId(),
            node.augmentation(NetconfNodeAugment.class).getNetconfNode());
    ...
    void refreshSshConfig(final @NonNull SshTransportTopologyParameters sshParams) {
        final var singleton = getTopologySingleton();
        if (isMaster) {
            singleton.refreshSshParamsConnection(sshParams, remoteDeviceId);
        } else {
