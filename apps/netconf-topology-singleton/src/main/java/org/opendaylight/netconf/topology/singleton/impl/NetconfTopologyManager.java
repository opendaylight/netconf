...
public class NetconfTopologyManager implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    private void startNetconfDeviceContext(final InstanceIdentifier<Node> instanceIdentifier, final Node node) {
        final NetconfNode netconfNode = requireNonNull(requireNonNull(node.augmentation(NetconfNodeAugment.class))
            .getNetconfNode());
    ...
    private NetconfTopologySetup createSetup(final InstanceIdentifier<Node> instanceIdentifier, final Node node) {
        final NetconfNode netconfNode = requireNonNull(node.augmentation(NetconfNodeAugment.class)).getNetconfNode();
