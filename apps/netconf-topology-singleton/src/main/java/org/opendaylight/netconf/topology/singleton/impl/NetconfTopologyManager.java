...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology.rev251205.TopologyTypes1Builder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.node.topology.rev251205.network.topology.topology.topology.types.TopologyNetconfBuilder;
...
public class NetconfTopologyManager implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    private void onDataTreeChangedParams(
            final @NonNull List<DataTreeModification<SshTransportTopologyParameters>> changes) {
        for (var change : changes) {
            switch (change.getRootNode()) {
                case WithDataAfter<SshTransportTopologyParameters> present -> updateSshParams(present.dataAfter());
                // keep default values in datastore
                case DataObjectDeleted<SshTransportTopologyParameters> deleted ->
                    updateSshParams(AbstractNetconfTopology.defaultSshParams());
            }
        }
    }
