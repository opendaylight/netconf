...
public final class OSGiNorthbound {
    ...
    public @interface Configuration {
        ...
        @AttributeDefinition(
            name = "HTTP/2 initial window size (bytes)",
            description = """
                HTTP/2 flow-control window (SETTINGS_INITIAL_WINDOW_SIZE) this server advertises to
                clients. It bounds how much request body a peer may send before waiting for a
                WINDOW_UPDATE; it does not by itself reject anything.
                """,
            min = "0", max = "2147483647")
        int http2$_$initial$_$window$_$size() default 65535; // 64 KiB - 1, the RFC9113 default
        ...
        @AttributeDefinition(
            name = "HTTP socket receive buffer size (bytes)",
            description = """
                Netty SO_RCVBUF for accepted connections. 0 leaves the operating system default in place.
                """,
            min = "0")
        int http$_$socket$_$receive$_$buffer$_$size() default 32768; // 32 KiB

        @AttributeDefinition(
            name = "HTTP ByteBuf allocator",
            description = """
                Netty ByteBuf allocator used for the endpoint. Expected 'pooled', which recycles buffers
                through a pool, or 'unpooled', which allocates every buffer afresh and is useful when
                diagnosing a leak.
                """)
        String http$_$bytebuf$_$allocator() default "pooled";
    ...
    private static Map<String, ?> newNettyEndpointProps(final BootstrapFactory bootstrapFactory,
        ...
        final var limits = new HTTPServerLimits(
            ...
            configuration.http2$_$initial$_$window$_$size(),
            configuration.http$_$socket$_$receive$_$buffer$_$size(),
            parseAllocator(configuration.http$_$bytebuf$_$allocator()));
    ...
    private static @NonNull BytebufAllocator parseAllocator(final String str) {
        final var allocator = BytebufAllocator.forName(str);
        if (allocator == null) {
            throw new IllegalArgumentException("Invalid bytebuf-allocator '" + str + "'");
        }
        return allocator;
    }
