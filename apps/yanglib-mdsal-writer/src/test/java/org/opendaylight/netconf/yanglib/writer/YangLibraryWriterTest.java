...
public class YangLibraryWriterTest {

    private static final YangLibrarySchemaSourceUrlProvider URL_PROVIDER = new YangLibrarySchemaSourceUrlProvider() {
        @Override
        public Optional<Uri> getSchemaSourceUrl(final @NonNull String moduleSetName, final @NonNull ModuleLike module) {
            return Optional.of(new Uri("/url/to/" + module.getName()
                + module.getQNameModule().getRevision().map(rev -> "/" + rev).orElse("")));
        }
    };
    ...
    private static final InstanceIdentifier<YangLibrary> YANG_LIBRARY_II =
        InstanceIdentifier.create(YangLibrary.class);
    private static final InstanceIdentifier<ModulesState> MODULES_STATE_II =
    ...
    @Captor
    ArgumentCaptor<YangLibrary> yangLibraryCaptor;
    @Captor
    ArgumentCaptor<ModulesState> modulesStateCaptor;
    ...
    void writeContent(final boolean withUrls, final boolean writeLegacy, final YangLibrary expectedData,
        ...
        doNothing().when(writeTransaction).put(eq(OPERATIONAL), any(), any());
        ...
        verify(writeTransaction).put(eq(OPERATIONAL), eq(YANG_LIBRARY_II), yangLibraryCaptor.capture());
        assertEquals(expectedData, yangLibraryCaptor.getValue());
        if (writeLegacy) {
            verify(writeTransaction).put(eq(OPERATIONAL), eq(MODULES_STATE_II), modulesStateCaptor.capture());
            assertEquals(expectedLegacyData, modulesStateCaptor.getValue());
        } else {
            verify(writeTransaction, never()).put(eq(OPERATIONAL), eq(MODULES_STATE_II), any());
        }
                ...
                new ModuleSetBuilder()
                    ...
                    .setModule(BindingMap.of(
                        ...
                        new ModuleBuilder().setName(new YangIdentifier("test-more_2023-07-25"))
                            ...
                            .setDeviation(Set.of(new YangIdentifier("test-module_2013-07-22")))
                            .build(),
