...
public class BenchmarkIT extends AbstractE2ETest {
    ...
    public void benchmarkTest() throws Exception {
        ...
        var headers = client.send(request, HttpResponse.BodyHandlers.discarding()).headers();
        assertEquals("200", headers.firstValue(":status").orElseThrow());
        // Mainly here to verify there is some data in the response
        assertEquals("1357417988", headers.firstValue("content-length").orElseThrow());
