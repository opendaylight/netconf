...
public class Http3NettyTestClient implements AutoCloseable {
    ...
    public Http3NettyTestClient(final String username, final String password) throws Exception {
    ...
    public Http3NettyTestClient() throws Exception {
        this(null, null);
    }

    public Http3Response invoke(final HttpRequest request) throws Exception {
        return invoke(request, false);
    }

    public Http3Response invoke(final HttpRequest request, final boolean discardBody) throws Exception {
        ...
        final var timeout = request.timeout().isEmpty() ? DEFAULT_TIMEOUT : request.timeout().get().getSeconds();
        ...
        streamChannel.writeAndFlush(frame).sync();

        // Add body if present
        if (request.bodyPublisher().isPresent()) {
            final var dataFrame =
                new DefaultHttp3DataFrame(Unpooled.wrappedBuffer(getBody(request.bodyPublisher().get())));
    ...
    private static byte[] getBody(BodyPublisher publisher) throws Exception {
