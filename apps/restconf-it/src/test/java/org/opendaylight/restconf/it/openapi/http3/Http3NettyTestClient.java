...
public class Http3NettyTestClient implements AutoCloseable {
    ...
    public Http3Response invoke(final HttpRequest request) throws Exception {
        final var scheme = request.uri().getScheme();
        final var host = request.uri().getHost();
        final var port = request.uri().getPort();
        ...
        byte[] bodyBytes = getBodyBytes(request); // helper from earlier
        if (bodyBytes.length > 0) {
            var dataFrame = new DefaultHttp3DataFrame(Unpooled.wrappedBuffer(bodyBytes));
    ...
    private static byte[] getBodyBytes(HttpRequest request) throws Exception {
