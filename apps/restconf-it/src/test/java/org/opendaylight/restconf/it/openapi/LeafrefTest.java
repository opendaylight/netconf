...
public class LeafrefTest extends AbstractOpenApiTest {
    ...
    public void leafrefResolvedTest() throws Exception {
        startDeviceSimulator("target/test-classes/leafref");
    ...
    public void leafrefNotResolvedTest() throws Exception {
        startDeviceSimulator("target/test-classes/leafref-deviated");
        mountDeviceJson(devicePort);
        final var client = HttpClient.newBuilder()
            .authenticator(new Authenticator() {
                @Override
        ...
        try {
            ...
            assertEquals("request timed out", hte.getMessage());
        }
    }
