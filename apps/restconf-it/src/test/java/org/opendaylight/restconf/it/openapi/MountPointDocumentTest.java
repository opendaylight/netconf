...
import org.opendaylight.restconf.it.AbstractIT.ProtocolVersion;
...
class MountPointDocumentTest extends AbstractOpenApiTest {
     ...
     *
     * <p>FIXME NETCONF-1566: fails for all three protocol versions because {@link #invokeRequest} always routes
     * through {@code netconf.transport.http.HTTPClient}, whose {@code ClientHttp1Session}/{@code
     * ClientHttp2Session}/{@code ClientHttp3Session} pipelines all cap responses at
     * {@code HTTPTransportStack.MAX_HTTP_CONTENT_LENGTH} via {@code HttpObjectAggregator}, and this device-all
     * document exceeds it. The formerly-separate HTTP/2 and HTTP/3 variants of this test only appeared to pass
     * because they bypassed this shared client (a JDK {@code HttpClient} and an ad hoc {@code Http3NettyTestClient}
     * respectively, neither bound by that same cap) -- not because the limitation doesn't apply to them.
     */
