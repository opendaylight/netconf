...
@Provider
@Produces({ MediaTypes.PATCH_STATUS + RestconfService.JSON, MediaType.APPLICATION_JSON })
public class PATCHJsonBodyWriter implements MessageBodyWriter<PATCHContext> {
    @Override
    public boolean isWriteable(Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
        return type.equals(PATCHContext.class);
    }
    ...
    public void writeTo(PATCHContext patchContext, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders, OutputStream entityStream) throws IOException, WebApplicationException {
        //TODO implement, existing json/xml capabilities can be used, because there is no anyxml in response
    }
}
