...
public class JsonToPATCHBodyReader extends AbstractIdentifierAwareJaxRsProvider implements MessageBodyReader<PATCHContext> {
     ...
     * @throws IOException
     */
    private void readEditDefinition(final PatchEdit edit, final JsonReader in, final InstanceIdentifierContext path,
                                    final StringModuleInstanceIdentifierCodec codec) throws IOException {
        in.beginObject();
        while (in.peek() != JsonToken.END_OBJECT) {
            ...
            switch (editDefinition) {
                ...
                case "operation" :
                    ...
                    break;
                case "target" :
                    edit.target = codec.deserialize(codec.serialize(path
                            .getInstanceIdentifier()) + in.nextString());
                    edit.targetSchemaNode = SchemaContextUtil.findDataSchemaNode(path
                            .getSchemaContext(), codec.getDataContextTree().getChild
                            (edit.target).getDataSchemaNode().getPath().getParent());
                    break;
                case "value" :
                    edit.data = readEditData(in, edit.targetSchemaNode, path);
                    break;
                default:
                    break;
            }
     ...
     */
    private NormalizedNode readEditData(final JsonReader in, SchemaNode targetNode, InstanceIdentifierContext path) {
        final NormalizedNodeResult resultHolder = new NormalizedNodeResult();
        final NormalizedNodeStreamWriter writer = ImmutableNormalizedNodeStreamWriter.from(resultHolder);
        JsonParserStream.create(writer, path.getSchemaContext(), targetNode).parse(in);
    ...
    private PATCHEntity prepareEditOperation(final PatchEdit edit) {
        if (edit.operation != null && edit.targetSchemaNode != null
            ...
            if (isPatchOperationWithValue(edit.operation)) {
                ...
                return new PATCHEntity(edit.id, edit.operation, edit.target.getParent());
            }
        }
