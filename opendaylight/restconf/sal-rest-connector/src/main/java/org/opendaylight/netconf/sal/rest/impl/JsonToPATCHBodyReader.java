...
public class JsonToPATCHBodyReader extends AbstractIdentifierAwareJaxRsProvider implements MessageBodyReader<PATCHContext> {
        ...
        while (in.hasNext()) {
            switch (in.peek()) {
                ...
                case END_DOCUMENT:
                    break;
                case NAME:
                    switchParsedName(in.nextName(), edit, in, path, codec, resultCollection);
                    break;
                case END_OBJECT:
                    in.endObject();
                    break;
    ...
    /**
     * Switch value of JsonToken.NAME
     ...
     */
    private void switchParsedName(final String name, final PatchEdit edit, final JsonReader in,
                                  final InstanceIdentifierContext path, final StringModuleInstanceIdentifierCodec codec,
                                  final List<PATCHEntity> resultCollection) throws IOException {
        switch (name) {
            case "edit" :
