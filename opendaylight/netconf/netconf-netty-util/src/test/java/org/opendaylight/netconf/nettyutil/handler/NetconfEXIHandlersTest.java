...
public class NetconfEXIHandlersTest {
    ...
    public void setUp() throws Exception {
        final NetconfEXICodec codec = new NetconfEXICodec(EXIParameters.empty());
    ...
    private static byte[] msgToExi(final NetconfMessage msg, final NetconfEXICodec codec) throws IOException, EXIException, TransformerException {
        final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        final SAXEncoder encoder = codec.getWriter();
        encoder.setOutputStream(byteArrayOutputStream);
        ThreadLocalTransformers.getDefaultTransformer().transform(new DOMSource(msg.getDocument()), new SAXResult(encoder));
