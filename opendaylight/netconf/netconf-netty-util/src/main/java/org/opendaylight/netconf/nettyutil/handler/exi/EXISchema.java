...
public enum EXISchema {
    ...
    private EXISchema(final String name, final Grammars grammar) {
        this.schemaName = Preconditions.checkNotNull(name);
    ...
    static EXISchema forSchemaName(final String name) {
        for (EXISchema s : EXISchema.values()) {
            if (name.equals(s.getSchemaName())) {
                return s;
            }
        }
