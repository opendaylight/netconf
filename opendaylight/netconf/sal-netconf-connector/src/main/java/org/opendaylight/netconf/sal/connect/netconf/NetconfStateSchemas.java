...
public final class NetconfStateSchemas {
    ...
    static {
        ...
        GET_SCHEMAS_RPC
                = Builders.containerBuilder().withNodeIdentifier(NetconfMessageTransformUtil.toId(NetconfMessageTransformUtil.NETCONF_GET_QNAME)).withChild(filter).build();
    }
    ...
    private static NetconfStateSchemas create(final NetconfDeviceRpc deviceRpc, final NetconfSessionPreferences remoteSessionCapabilities, final RemoteDeviceId id) {
        ...
        try {
            schemasNodeResult = deviceRpc.invokeRpc(NetconfMessageTransformUtil.toPath(NetconfMessageTransformUtil.NETCONF_GET_QNAME), GET_SCHEMAS_RPC).get();
    ...
    private static Optional<? extends NormalizedNode<?, ?>> findSchemasNode(final NormalizedNode<?, ?> result) {
        ...
        final Optional<DataContainerChild<?, ?>> dataNode = ((DataContainerNode<?>) result).getChild(NetconfMessageTransformUtil.toId(NetconfMessageTransformUtil.NETCONF_DATA_QNAME));
        if(dataNode.isPresent() == false) {
            return Optional.absent();
        }

        final Optional<DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?>> nStateNode =
                ((DataContainerNode<?>) dataNode.get()).getChild(NetconfMessageTransformUtil.toId(NetconfState.QNAME));
        if(nStateNode.isPresent() == false) {
            return Optional.absent();
        }

        return ((DataContainerNode<?>) nStateNode.get()).getChild(NetconfMessageTransformUtil.toId(Schemas.QNAME));
    }
    ...
    protected static NetconfStateSchemas create(final RemoteDeviceId id, final ContainerNode schemasNode) {
        ...
        final Optional<DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?>> child = schemasNode.getChild(NetconfMessageTransformUtil.toId(Schema.QNAME));
    ...
    public final static class RemoteYangSchema {
        ...
        private static Set<String> getAllChildNodeValues(final DataContainerNode<?> schemaNode, final QName childNodeQName) {
            ...
            final Optional<DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?>> child = schemaNode.getChild(NetconfMessageTransformUtil.toId(childNodeQName));
        ...
        private static Optional<String> getSingleChildNodeValue(final DataContainerNode<?> schemaNode, final QName childNode) {
            final Optional<DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?>> node = schemaNode.getChild(NetconfMessageTransformUtil.toId(childNode));
