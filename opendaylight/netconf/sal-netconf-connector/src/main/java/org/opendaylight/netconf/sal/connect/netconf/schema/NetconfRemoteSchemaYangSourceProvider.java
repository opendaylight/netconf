...
public final class NetconfRemoteSchemaYangSourceProvider implements SchemaSourceProvider<YangTextSchemaSource> {
    ...
    private static Optional<String> getSchemaFromRpc(final RemoteDeviceId id, final NormalizedNode<?, ?> result) {
        ...
        final QName schemaWrapperNode = QName.cachedReference(QName.create(NetconfMessageTransformUtil.GET_SCHEMA_QNAME, NetconfMessageTransformUtil.NETCONF_DATA_QNAME.getLocalName()));
        final Optional<DataContainerChild<? extends YangInstanceIdentifier.PathArgument, ?>> child = ((ContainerNode) result).getChild(NetconfMessageTransformUtil.toId(schemaWrapperNode));
