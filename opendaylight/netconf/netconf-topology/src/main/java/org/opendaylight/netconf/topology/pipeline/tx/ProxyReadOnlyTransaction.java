...
public class ProxyReadOnlyTransaction implements DOMDataReadOnlyTransaction{
    ...
    public CheckedFuture<Optional<NormalizedNode<?, ?>>, ReadFailedException> read(final LogicalDatastoreType store, final YangInstanceIdentifier path) {
        final Future<Optional<NormalizedNode<?, ?>>> future = delegate.read(store, path);
        ...
        future.onComplete(new OnComplete<Optional<NormalizedNode<?, ?>>>() {
            @Override
            public void onComplete(Throwable throwable, Optional<NormalizedNode<?, ?>> normalizedNodeOptional) throws Throwable {
                if (throwable == null) {
                    settableFuture.set(normalizedNodeOptional);
                } else {
