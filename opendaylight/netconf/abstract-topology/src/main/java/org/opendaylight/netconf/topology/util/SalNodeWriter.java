...
public final class SalNodeWriter<N extends Node> implements NodeWriter<N> {
    ...
    private static final KeyedInstanceIdentifier<Topology, TopologyKey> TOPOLOGY_LIST_PATH =
            NETWORK_TOPOLOGY_PATH.child(Topology.class, new TopologyKey(new TopologyId(TopologyNetconf.QNAME.getLocalName())));

    public SalNodeWriter(final DataBroker dataBroker) {
        this.dataBroker = dataBroker;
    }

    //FIXME we will need to use txChains
    @Override public void init(@Nonnull final NodeId id, @Nonnull final N operationalDataNode) {
        // put into Datastore
    ...
    @Override public void update(@Nonnull final NodeId id, @Nonnull final N operationalDataNode) {
        // merge
        final WriteTransaction wTx = dataBroker.newWriteOnlyTransaction();
        wTx.put(LogicalDatastoreType.OPERATIONAL, createBindingPathForTopology(id), operationalDataNode);
    ...
    private static InstanceIdentifier<Node> createBindingPathForTopology(final NodeId id) {
        return TOPOLOGY_LIST_PATH.child(Node.class, new NodeKey(id));
    }
}
