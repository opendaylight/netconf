    ...
    def device_schema_directory(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_populate_schema_directories"):
            ...
            for member_dir in cluster.get_cluster_dirs():
                ...
                missing = set(SCHEMA_CACHE_MODELS) - set(listing.split())
                assert not missing, f"{target} is missing {sorted(missing)}"
        yield SCHEMA_CACHE_DIRECTORY
        with allure_step_with_separate_logging("step_check_schema_cache_was_used"):
            # Seeding only proves the files landed. Mounting the device makes
            # the connector write the models it fetched over netconf into the
            # same cache, so a directory holding more than what was seeded is
            # the evidence that this is the directory ODL actually used. Left
            # unchecked, a cache ODL ignored would leave this suite a silent
            # copy of the clustering CRUD suite.
            for member_dir in cluster.get_cluster_dirs():
                target = f"{member_dir}/cache/{SCHEMA_CACHE_DIRECTORY}"
                _, listing = infra.shell(f"ls {target}")
                fetched = set(listing.split()) - set(SCHEMA_CACHE_MODELS)
                assert fetched, f"{target} holds only seeded models, ODL ignored it"
