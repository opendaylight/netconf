...
# The suite mirrors the original Robot "node1/node2/node3" naming: the device
# is configured and deconfigured through node 1, and each of the three data
# operations is routed through a node that is still up while another is down.
...
OUTAGE_CYCLES = (
    OutageCycle(
        ...
        data_operation=netconf_cluster.create_device_data,
    ...
    OutageCycle(
        ...
        data_operation=netconf_cluster.modify_device_data,
    ...
    OutageCycle(
        ...
        data_operation=netconf_cluster.delete_device_data,
        template_dir=f"{DIRECTORY_WITH_TEMPLATE_FOLDERS}/datamod1",
...
class TestClusteringOutages:
    ...
    def check_device_data_on_nodes(
        ...
        for host in hosts:
            netconf.wait_device_config_data(
                DEVICE_NAME, expected, timeout=timeout, host=host
            )
    ...
    def run_outage_cycle(self, cycle: OutageCycle, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            netconf_cluster.wait_device_ownership_settled(DEVICE_NAME, host=node_ip)
