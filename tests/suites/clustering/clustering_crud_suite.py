...
from libraries import netconf_cluster
...
def report_known_bug(bug_id: str | None):
    ...
    Returns:
        The utils.report_known_bug_on_failure context manager for a bug id, a
...
class ClusteringCrudSuite:
    ...
    Attributes:
        DEVICE_TYPE (str): Template type the device is configured with.
            Declared without a value on purpose: a subclass that forgets to set
            it fails with AttributeError instead of silently running as a copy
            of the CRUD suite.
    ...
    def configured_device(
        ...
        with allure_step_with_separate_logging("step_configure_device_via_configurer"):
            ...
            with report_known_bug(self.CONFIGURE_BUG):
        ...
        # Everything from here on runs with the device mounted, so the
        # teardown has to happen even when one of the remaining setup steps
        # fails. Without this a failed setup leaves the device behind and the
        # next suite fails on its own first check, for an unrelated reason.
        try:
            with allure_step_with_separate_logging(
                "step_check_configurer_has_netconf_connector_for_device"
            ):
                # Get the list of mounts and search for our device there.
                # Fail if not found.
            ...
            with allure_step_with_separate_logging(
                ...
                # request is sent to the config subsystem which then triggers
                # asynchronous disconnection of the device which is reflected in
                # the operational data once completed. This test makes sure this
            ...
            with allure_step_with_separate_logging(
                ...
                # Check that the device is going to be gone from the checker
                # node. Fail if still there after default timeout.
                netconf.wait_device_fully_removed(DEVICE_NAME, host=CHECKER_IP)

            with allure_step_with_separate_logging(
                "step_check_device_deconfigured_on_setter"
            ):
                # Check that the device is going to be gone from the setter
                # node. Fail if still there after default timeout.
    ...
    def run_crud_flow(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_create_device_data_via_setter"):
            ...
            netconf_cluster.create_device_data(
                DEVICE_NAME,
        ...
        with allure_step_with_separate_logging("step_modify_device_data_via_setter"):
            # Send a request to change the sample test data. Reissued and read
            # back like the create above: measured on a three member cluster,
            # this write fails the same two ways, either rejected outright with
            # "Commit of operation failed" or accepted while the same member
            # still answers the pre-write value for about a second.
            with report_known_bug(self.DATA_OP_BUG):
                netconf_cluster.modify_device_data(
                    DEVICE_NAME,
        ...
        with allure_step_with_separate_logging("step_delete_device_data_via_setter"):
            # Send a request to delete the sample test data, for the same
            # reasons reissued and read back, and only sent while the data is
            # still there so a retry after a delete that did land does not fail
            # on data that is already gone.
            with report_known_bug(self.DATA_OP_BUG):
                netconf_cluster.delete_device_data(
                    DEVICE_NAME,
                    f"{DIRECTORY_WITH_TEMPLATE_FOLDERS}/datamod1",
                    EMPTY_DATA,
                    host=SETTER_IP,
