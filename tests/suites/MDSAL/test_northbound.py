...
class TestNorthbound:
    ...
    def reopen_odl_netconf_connection(self):
    ...
    def open_odl_netconf_conenction(self):
        """Open a prepared netconf connecion."""
    ...
    def transmit_message(self,message: str):
        """Transmit message to Netconf connection and discard the echo of the message."""
    ...
    def prepare_for_search(self, searched_string: str):
    ...
    def load_and_send_message(self, name: str):
        """Load a message from the data file set, send it to Netconf and return the reply."""
        request = self.get_data(name + "-request")
        reply = self.send_message(request)

        return reply
    
    def load_expected_reply(self, name: str):
    ...
    def check_test_object_not_present_in_config(self, name):
        """Use dataset with the specified name to get the configuration and check that none of our test objects are there."""
    ...
    def perform_test(self, name: str):
        """Load and send the request from the dataset and compare the returned reply to the one stored in the dataset."""
        actual = self.load_and_send_message(name)
        expected = self.load_expected_reply(name)
        actual = actual.replace("]]>]]>", "").strip()
        utils.verify_xmls_match(actual, expected, "Actual response", "Expected response")
    ...
    def test_northbound(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            "step_connect_to_ODL_netconf"
        ):
            """Connect to ODL Netconf and fail if that is not possible."""
            self.open_odl_netconf_conenction()

        with allure_step_with_separate_logging(
            "step_get_config_running"
        ):
            """Make sure the configuration has only the default elements in it."""
            self.check_test_object_not_present_in_config("get-config")

        with allure_step_with_separate_logging(
            "step_missing_message_id_attribute"
        ):
            """Check that messages with missing "message-ID" attribute are rejected with the correct error (RFC 6241, section 4.1)."""
            self.perform_test("missing-attr")

        with allure_step_with_separate_logging(
            "step_additional_attributes_in_message"
        ):
            """Check that additional attributes in messages are returned properly (RFC 6241, sections 4.1 and 4.2)."""
            ...
            assert 'xmlns:prefix="http://www.example.com/my-schema-example.html"' in reply
        
        with allure_step_with_separate_logging(
            "step_send_stuff_in_undefined_namespace"
        ):
            """Try to send something within an undefined namespace and check the reply complains about the nonexistent namespace and element."""
            reply = self.load_and_send_message("merge-nonexistent-namespace")
            assert "java.lang.NullPointerException" not in reply, "Failed on know bug: 5125"
            assert "urn:this:is:in:a:nonexistent:namespace" in reply
            assert "<rpc-error>" in reply

        with allure_step_with_separate_logging(
            "step_edit_config_first_batch_merge"
        ):
            """Request a "merge" operation adding an element in candidate configuration and check the reply."""
            self.perform_test("merge-1")

        with allure_step_with_separate_logging(
            "step_get_config_running_to_confirm_no_edit_before_commit"
        ):
            """Make sure the running configuration is still unchanged as the change was not commited yet."""
            self.check_test_object_not_present_in_config("get-config-no-edit-before-commit")
        ...
        with allure_step_with_separate_logging(
            "step_terminate_connection_gracefully"
        ):
        ...
        with allure_step_with_separate_logging(
            ...
            """Request a "create" operation of an element that already exists and check that it fails with the correct error (RFC 6241, section 7.2, operation "create")."""
            self.perform_test("create")

        with allure_step_with_separate_logging(
            "step_delete_first_batch"
        ):
            """Delete the created element from the candidate configuration and check the reply."""
            self.perform_test("delete")

        with allure_step_with_separate_logging(
            "step_get_config_running_to_confirm_no_delete_before_commit"
        ):
            """Make sure the element is still present in the running configuration as the delete command was not committed yet."""
        ...
        with allure_step_with_separate_logging(
            ...
            self.check_test_object_not_present_in_config("get-config-delete-after-commit")

        with allure_step_with_separate_logging(
            "step_commit_no_transaction"
        ):
            """Attempt to perform "commit" when there are no changes in the candidate configuration and check that it returns OK status."""
            self.perform_test("commit-no-transaction")
            #report_failure_due_to_bug()


        with allure_step_with_separate_logging(
            "step_edit_config_second_batch_merge"
        ):
        ...
        with allure_step_with_separate_logging(
            ...
            """Check that the element to be discarded is present in the candidate configuration."""
        ...
        with allure_step_with_separate_logging(
            ...
            """Use a create request with the third batch to create the infrastructure."""
        ...
        with allure_step_with_separate_logging(
            ...
            """Check that the 3 subelements are now present in the running configuration."""
        ...
        with allure_step_with_separate_logging(
            ...
            """Simulate session failure by disconnecting without terminating the session."""
            self.abort_odl_netconf_connection()
        ...
        with allure_step_with_separate_logging(
            "step_edit_multiple_merge_data"
        ):
            """Add another subelement named "test" to the element and check the reply."""
        ...
        with allure_step_with_separate_logging(
            ...
            """Check that the "test" subelement exists and has correct value for "port" subelement."""
        ...
        with allure_step_with_separate_logging(
            "step_update_multiple_modules_merge"
        ):
            """Update the value of the "port" subelement of the "test" subelement and check the reply."""
        ...
        with allure_step_with_separate_logging(
            "step_replace_multiple_modules_merge"
        ):
            """Replace the content of the "test" with another completely different and check the reply."""
        ...
        with allure_step_with_separate_logging(
            "step_remove_test_data"
        ):
            """Remove the testing elements and all their subelements and check the reply."""
            self.perform_test("merge-multiple-remove")

        with allure_step_with_separate_logging(
            "step_commit_test_data_removal"
        ):
            """Commit the removal and check the reply."""
            self.perform_test("merge-multiple-remove-commit")

        with allure_step_with_separate_logging(
            "step_connector_simpliefied_pattern"
        ):
            """Several requests in a (simplified) pattern typical for requests from netconf-connector."""
        ...
        with allure_step_with_separate_logging(
            ...
            self.perform_test("delete")
            self.perform_test("commit-edit")
            #Report_Failure_Due_To_Bug(7791)

        with allure_step_with_separate_logging(
            "step_delete_not_existing_element"
        ):
            """Attempt to delete the elements again and check that it fails with the correct error."""
        ...
        with allure_step_with_separate_logging(
            ...
            self.perform_test("commit-no-transaction")
            #Utils.Report_Failure_Due_To_Bug(4455)

        with allure_step_with_separate_logging(
            "step_remove_not_existing_module"
        ):
            """Attempt to remove the "module" element again and check that the operation is "silently ignored"."""
