...
DEVICE_DATA_SEEN_RETRY_COUNT = 60
RESTPERFCLIENT_URL = (
...
class TestPerformance:
    ...
    def netconf_testtool(self, allure_step_with_separate_logging):
        ...
        starts the netconf testtool with the required schemas, yields the
        running process and guarantees the process is terminated after
        execution.
    ...
    def connected_netconf_testtools(
        ...
        until its data becomes visible on SETTER_IP (node 2) and PERFCLIENT_IP
        (node 3). This forces the cluster nodes to communicate with each other
        about the device before any data operations run against it. By
        requiring the `netconf_testtool` fixture, this setup ensures the
        simulator process is running first.
        ...
        with allure_step_with_separate_logging(
            "step_wait_for_device_data_to_be_seen_on_perfclient"
        ):
            # Wait until the device data show up at node 3, so the restperfclient
            # requests issued against it later don't race the mount point.
            utils.wait_until_function_pass(
                DEVICE_DATA_SEEN_RETRY_COUNT,
                1,
                netconf.check_device_data_is_empty,
    ...
    # TODO: Remove once NETCONF-1666 is fixed; strict=False reports XPASS then.
    @pytest.mark.xfail(
        reason="NETCONF-1666: commit of data sent via non-master cluster node fails",
    ...
    def test_performance(
        ...
        with allure_step_with_separate_logging(
            "step_run_restperfclient_and_collect_results"
        ):
            ...
            # The log is copied into results/ even if this call raises.
            perf_log = rest_perf_client.run_restperfclient_and_collect_results(
                edits=REQUEST_COUNT,
