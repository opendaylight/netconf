...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/netconf/CRUD/CRUD.robot
#
...
class TestCrud:
    ...
    def connected_netconf_testtools(
        ...
        with allure_step_with_separate_logging("step_configure_device_on_netconf"):
            ...
            global DEVICE_TYPE
            DEVICE_TYPE = "default" if USE_NETCONF_CONNECTOR else DEVICE_TYPE
    ...
    def test_crud(self, connected_netconf_testtools, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            """Get the device data label and make sure it contains the created
            content."""
            self.check_config_data(
        ...
        with allure_step_with_separate_logging("step_modify_device_data_label_via_xml"):
            """Send a request to change the sample test data label and check that
            the request went OK."""
            mapping = {"DEVICE_NAME": DEVICE_NAME, "RESTCONF_ROOT": RESTCONF_ROOT}
        ...
        with allure_step_with_separate_logging(
            ...
            """Get the device data label and make sure it contains the modified
            content."""
            self.check_config_data(
                f'<data xmlns="{ODL_NETCONF_NAMESPACE}">'
                f'<cont xmlns="urn:opendaylight:test:netconf:crud">'
        ...
        with allure_step_with_separate_logging(
            ...
            The purpose of cycling the device like this is to see that
            the configuration data was really stored in the device."""
            netconf.remove_device_from_netconf(DEVICE_NAME)
        ...
        with allure_step_with_separate_logging("step_configure_the_device_back"):
            """Configure the device again. This is the second step of the device
            configuration."""
        ...
        with allure_step_with_separate_logging(
            ...
            """Send a JSON request to change the sample test data label and check that
            the request went OK."""
            mapping = {"DEVICE_NAME": DEVICE_NAME, "RESTCONF_ROOT": RESTCONF_ROOT}
            templated_requests.put_templated_request(
        ...
        with allure_step_with_separate_logging(
            ...
            """Get the device data label as XML and make sure it matches the content
            posted as JSON."""
            self.check_config_data(
                f'<data xmlns="{ODL_NETCONF_NAMESPACE}">'
                f'<cont xmlns="urn:opendaylight:test:netconf:crud">'
        ...
        with allure_step_with_separate_logging("step_create_car_list"):
            """Send a request to create a list of cars in the sample test data label
            and check that the request went OK."""
            mapping = {"DEVICE_NAME": DEVICE_NAME, "RESTCONF_ROOT": RESTCONF_ROOT}
            templated_requests.post_templated_request(
        ...
        with allure_step_with_separate_logging(
            ...
            """Send a request to create a data item in the test list and check that
            the request went OK."""
            mapping = {"DEVICE_NAME": DEVICE_NAME, "RESTCONF_ROOT": RESTCONF_ROOT}
            templated_requests.post_templated_request(
        ...
        with allure_step_with_separate_logging(
            ...
            """Send a JSON request to change the sample test data and check that
            the request went OK."""
            mapping = {"DEVICE_NAME": DEVICE_NAME, "RESTCONF_ROOT": RESTCONF_ROOT}
            templated_requests.post_templated_request(
        ...
        with allure_step_with_separate_logging("step_check_item2_is_created"):
            """Get the device data as XML and make sure item2 matches the content
            posted as JSON in the previous case."""
            data = self.get_config_data()
            assert "<id>SMALL</id>" in data
            assert "<model>Isetta</model>" in data
            assert "<manufacturer>BMW</manufacturer>" in data
