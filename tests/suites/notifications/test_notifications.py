...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/netconf/notifications/notifications_basic.robot
#
    ...
    @allure.description(
        textwrap.dedent(
            ...
            **Basic tests for RESTCONF Data Change Notifications (DCN).**
            ...
            Test suite performs basic subscription case for data store notifications. \
            ...
            To avoid unexpected responses, this suite deletes all data from config \
    ...
    def test_data_change_notification(
        ...
        with allure_step_with_separate_logging("step_check_bug_3934"):
            ...
            with utils.report_known_bug_on_failure("3934"):
                ...
                log.debug(f"Config data loaded from template: {data}")
                log.info(f"Received notification payload: {self.notification}")
