...
RFC8040_STREAMS_URI = f"{RESTCONF_ROOT}/data/ietf-restconf-monitoring:restconf-state/streams"
...
class TestCallHome:
    ...
    def test_data_change_notification(
        ...
        with allure_step_with_separate_logging("step_create_dcn_stream"):
            """Create DCN stream."""
            body = infra.get_file_content(f"{TEMPLATE_FOLDER}/{RESTCONF_SUBSCRIBE_DATA}")
            uri = restconf.generate_uri("sal-remote:create-data-change-event-subscription", "rpc")
            resp = templated_requests.post_to_uri(uri=uri, headers=SEND_ACCEPT_XML_HEADERS, data=body)
            self.log_response(resp)
            assert resp.status_code in templated_requests.ALLOWED_STATUS_CODES
            root = ET.fromstring(resp.content)
            stream_name = root.find('.//{*}stream-name').text
            #stream_name = resp.json()["stream-name"]
        ...
        with allure_step_with_separate_logging("step_subscribe_to_dcn_stream"):
            ...
            resp = templated_requests.get_from_uri(self.RFC8040_DCN_STREAM_URI, headers=SEND_ACCEPT_XML_HEADERS)
            #resp = templated_requests.get_from_uri(self.RFC8040_DCN_STREAM_URI)
            self.log_response(resp)
            assert resp.status_code in templated_requests.ALLOWED_STATUS_CODES
            #xpath = "access[2]/location"
            #STREAM_LOCATION = XML.Get_Element_Text    ${resp.content}    ${xpath}
            new_root = ET.fromstring(resp.content)
            STREAM_LOCATION = new_root.findall('.//{*}access')[1].find('{*}location').text
        ...
        with allure_step_with_separate_logging("step_start_receiver"):
            ...
            infra.shell(f"python3 tools/wstools/ssereceiver.py --uri {STREAM_LOCATION} --logfile {RECEIVER_LOG_FILE}", run_in_background=True)
            time.sleep(2)
            #log.info(output)
        ...
        with allure_step_with_separate_logging("step_change_ds_config"):
            ...
            resp = templated_requests.put_to_uri_request(uri=uri, headers=SEND_ACCEPT_XML_HEADERS, data=body)
            #resp = templated_requests.put_to_uri_request(uri=uri, data=body)
            self.log_response(resp)
            assert resp.status_code in templated_requests.ALLOWED_STATUS_CODES

            uri = f"{RESTCONF_ROOT}/data/network-topology:network-topology/topology=netconf-notif"
        ...
        with allure_step_with_separate_logging("step_check_bug_3934"):
            ...
            with utils.report_known_bug_on_failure("3934"):
                data = infra.get_file_content(f"{TEMPLATE_FOLDER}/{RESTCONF_CONFIG_DATA}")
                log.info(f"{data=}")
                log.info(f"{self.notification=}")
