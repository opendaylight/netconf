...
def check_netconf_mdsal_up_and_running() -> None:
    ...
    assert (
        count == 1
    ), f"Expected MDSAL port {ODL_NETCONF_MDSAL_PORT} to be listening, got count={count}"
    ...
    def test_netconf_connector_ready(self, allure_step_with_separate_logging):
        with allure_step_with_separate_logging(
            ...
            try:
                ...
                log.warning(
                    f"Failed to execute step_check_whether_netconf_connector_is_up_and_running with the following exception {e}"
                )
        ...
        with allure_step_with_separate_logging("step_wait_for_netconf_connector"):
            try:
                ...
                log.warning(
                    f"Failed to execute step_wait_for_netconf_connector with the following exception {e}"
                )
        ...
        with allure_step_with_separate_logging("step_wait_even_longer"):
            try:
                ...
                log.warning(
                    f"Failed to execute step_wait_even_longer with the following exception {e}"
                )

        with allure_step_with_separate_logging("step_check_for_bug_5014"):
            netconf.configure_device_in_netconf(
                device_name="test-device", device_type="configure-via-topology"
            )
    ...
    def test_mdsal_ready(self, allure_step_with_separate_logging):
        with allure_step_with_separate_logging("step_wait_for_mdsal"):
            if not infra.is_karaf_feature_installed("odl-netconf-mdsals"):
                pytest.skip(
                    "The 'odl-netconf-mdsals' feature is not installed so no need to wait for it."
                )
            utils.wait_until_function_pass(
