...
KEYCLOAK_ADMIN_TOKEN_URL = f"{KEYCLOAC_BASE_URL}/realms/master/protocol/openid-connect/token"
KEYCLOAK_USER_TOKEN_URL = f"{KEYCLOAC_BASE_URL}/realms/{ODL_REALM}/protocol/openid-connect/token"
...
def set_direct_access_for_client():
    ...
    response = requests.put(
        KEYCLOAK_CLIENTS_URL + "/" + client_uuid,
        json=data,
        headers=headers
    )
...
def restore_client_config(original_config):
    ...
    requests.put(f"{KEYCLOAK_CLIENTS_URL}/{client_uuid}", json=original_config, headers=headers)
...
def update_token_lifespan(lifespan_duration):
    ...
    payload = {
        "realm": ODL_REALM,
        "accessTokenLifespan": lifespan_duration
    }
...
def add_nbf_claim_mapper_in_keycloack(nbf_value):
    ...
    client_uuid = get_client_uuid(keycloak_admin_token, CLIENT_ID)
    requests.post(KEYCLOAK_MAPPER_MODELS.format(client_uuid=client_uuid), json=payload, headers=headers)
...
def get_mapper_id(keycloak_admin_token, mapper_name):
    ...
    response = requests.get(KEYCLOAK_MAPPER_MODELS.format(client_uuid=client_uuid), headers=headers)
...
def set_aud_claim_mapper_in_keycloack(aud_value):
    ...
    response = requests.put(KEYCLOAK_MAPPER_MODEL.format(client_uuid=client_uuid, mapper_id=mapper_id), json=payload, headers=headers)
...
def remove_aud_claim_mapper_in_keycloack():
    ...
    requests.delete(KEYCLOAK_MAPPER_MODEL.format(client_uuid=client_uuid, mapper_id=mapper_id), headers=headers)
...
def test_valid_token(direct_access_tmp_settings, user_name, user_password, endpoint, expected_status_code):
...
@pytest.mark.parametrize(
        ...
        [
            ("2027875600", ADMIN_USER_NAME, ADMIN_PASSWORD, KEY_STORES_URL, 401), # future nbf value
            ("1081184921", ADMIN_USER_NAME, ADMIN_PASSWORD, KEY_STORES_URL, 200) # past nbg value
        ]
)
def test_nbf_token(direct_access_tmp_settings, nbf, user_name, user_password, endpoint, expected_status_code):
...
@pytest.mark.parametrize(
        ...
        [
            ("completely-wrong-audience", ADMIN_USER_NAME, ADMIN_PASSWORD, KEY_STORES_URL, 401),
        ]
)
def test_custom_aud_token(direct_access_tmp_settings, aud, user_name, user_password, endpoint, expected_status_code):
...
@pytest.mark.parametrize(
        ...
        [
            ("http://malicious-domain.local/auth", ADMIN_USER_NAME, ADMIN_PASSWORD, KEY_STORES_URL, 401),
        ]
)
def test_custom_iss_token(direct_access_tmp_settings, iss, user_name, user_password, endpoint, expected_status_code):
