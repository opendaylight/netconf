...
class TestMdsal:
    ...
    def cars_test_data(self, allure_step_with_separate_logging):
        with allure_step_with_separate_logging("step_create_test_data"):
            """Send some sample test data into the device and check that the request went OK."""
    ...
    @allure.description(
        textwrap.dedent(
            """
            **netconf-restperfclient MDSAL dirrect access test case.**
    ...
    def test_mdsal_direct_access(self, cars_test_data, allure_step_with_separate_logging):

        with allure_step_with_separate_logging("step_run_restperfclient_directly_on_mdsal"):
            """Deploy and execute restperfclient, asking it to send the specified amount of requests to the MDSAL via Restconf."""
        ...
        with allure_step_with_separate_logging("step_check_for_failed_direct_mdsal_requests"):
            ...
            """
            assert rest_perf_client.grep_restperfclient_log(direct_log, "thread timed out") == ""
            assert rest_perf_client.grep_restperfclient_log(direct_log, "Request failed") == ""
            assert rest_perf_client.grep_restperfclient_log(direct_log, "Status code") == ""
    ...
    def test_mdsal_netconf_connector(self, cars_test_data, odl_netconf_connector, allure_step_with_separate_logging):
        with allure_step_with_separate_logging("step_run_restperfclient_through_netconf_connector"):
            """Ask RestPerfClient to send the requests to the MDSAL mapped via netconf topology device."""
        ...
        with allure_step_with_separate_logging("step_check_for_failed_netconf_connector_requests"):
            ...
            with utils.report_known_bug_on_failure("5581"):
                assert rest_perf_client.grep_restperfclient_log(connector_log, "thread timed out") == ""
                assert rest_perf_client.grep_restperfclient_log(connector_log, "Request failed") == ""
                assert rest_perf_client.grep_restperfclient_log(connector_log, "Status code") == ""
