...
class TestMdsal:
    ...
    def test_mdsal_direct_access(
        ...
        with allure_step_with_separate_logging(
            "step_run_restperfclient_directly_on_mdsal_and_collect_results"
        ):
            # Deploy and execute restperfclient, asking it to send the specified
            # amount of requests to the MDSAL via Restconf.
            direct_log = rest_perf_client.run_restperfclient_and_collect_results(
                edits=REQUEST_COUNT,
    ...
    def test_mdsal_netconf_connector(
        ...
        with allure_step_with_separate_logging(
            "step_run_restperfclient_through_netconf_connector_and_collect_results"
        ):
            # Ask RestPerfClient to send the requests to the MDSAL mapped via netconf
            # topology device.
            connector_log = rest_perf_client.run_restperfclient_and_collect_results(
                edits=REQUEST_COUNT,
