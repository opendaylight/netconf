...
class KarafShell:
    ...
    def connect(self):
        ...
        if self.client is None or self.shell is None:
            log.debug(f"Opening interactive SSH connection to {self.host}:{self.port}...")
    ...
    def _clean_terminal_noise(self, raw_text: str) -> str:
        """
        Emulates terminal \r behavior to yield clean text.
        Removes JLine redraw artifacts but preserves valid text.
        """
        filtered_lines = []
