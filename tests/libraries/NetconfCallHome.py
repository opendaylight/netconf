...
def apply_ssh_based_call_home_configuration():
    """Copy netopeer2 configuration files needed for SSH transport"""
    infra.copy_file(
...
def apply_tls_based_call_home_configuration():
    """Copy netopeer2 configuration files needed for TLS transport"""
...
def generate_certificates_for_tls_configuration():
    ...
    infra.shell(
        "openssl req -x509 -new -extensions v3_ca -nodes -key tmp/certs/ca.key "
        '-sha256 -days 365 -subj "/C=US/ST=CA/L=Netopeer/O=netopeerCA/CN=netopeerCA" '
        "-out tmp/certs/ca.pem"
    )
    infra.shell("openssl genrsa -out tmp/certs/server.key 2048")
    infra.shell(
        "openssl req -new -sha256 -key tmp/certs/server.key "
        '-subj "/C=US/ST=CA/L=Netopeer/O=Netopeer2/CN=netopeer2-server" '
        "-out tmp/certs/server.csr"
    )
    infra.shell(
        "openssl x509 -req -in tmp/certs/server.csr -CA tmp/certs/ca.pem "
        "-CAkey tmp/certs/ca.key -CAcreateserial -extfile tmp/x509_v3.cfg "
        "-out tmp/certs/server.crt -days 365 -sha256"
    )
    ...
    infra.shell(
        "openssl req -new -sha256 -key tmp/certs/client.key "
        '-subj "/C=US/ST=CA/L=Netopeer/O=Netopeer2/CN=netopeer2-client" '
        "-out tmp/certs/client.csr"
    )
    infra.shell(
        "openssl x509 -req -in tmp/certs/client.csr -CA tmp/certs/ca.pem "
        "-CAkey tmp/certs/ca.key -CAcreateserial -extfile tmp/x509_v3.cfg "
        "-out tmp/certs/client.crt -days 1024 -sha256"
    )
    infra.shell("mv tmp/certs tmp/configuration-files/")
