...
def generate_certificates_for_tls_configuration():
    ...
    infra.shell(
        "openssl x509 -req -in tmp/certs/server.csr -CA tmp/certs/ca.pem -CAkey tmp/certs/ca.key -CAcreateserial -extfile x509_v3.cfg -out tmp/certs/server.crt -days 365 -sha256"
    )
    ...
    infra.shell(
        "openssl x509 -req -in tmp/certs/client.csr -CA tmp/certs/ca.pem -CAkey tmp/certs/ca.key -CAcreateserial -extfile x509_v3.cfg -out tmp/certs/client.crt -days 1024 -sha256"
    )
    infra.copy_file(
        src_dir="tmp/", src_file_name="certs", dst_dir="tmp/configuration-files/"
    )
...
def get_certificate_file_content(file_name: str):
    ...
    rc, content = infra.shell(
        f"sed -u '1d; $d' tmp/configuration-files/certs/${file_name} | sed -z 's!\\n!\\\\n!g'"
    )
...
def register_ssh_call_home_devices_in_odl_controller(
    device_name: str,
    hostkey: str,
...
def suite_setup():
    ...
    infra.shell(
        "sed -i -e 's/NETCONF_CH_SSH/${netconf_cl_ssh_port}/g' tmp/docker-compose.yaml"
    )
