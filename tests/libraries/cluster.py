...
def _member_process_pattern(member: int) -> str:
...
def _verify_member_stopped(member: int):
    ...
    _, out = infra.shell(f"pgrep -f '{_member_process_pattern(member)}' || true")
...
def kill_member(member: int):
    ...
    Returns:
        None
    """
    log.info(f"Killing cluster member-{member} ({member_ip(member)})")
    infra.shell(f"pkill -9 -f '{_member_process_pattern(member)}'")
...
def start_member(member: int, timeout: int = 300):
    ...
    log.info(
        f"Starting cluster member-{member} ({member_ip(member)}) from {member_dir}"
    )
...
def _member_number(node_name: str) -> int:
...
def get_device_entity_owner_and_followers(
    ...
    owner = _member_number(entity["owner-node"])
    candidates = sorted(_member_number(node) for node in entity["candidate-nodes"])
...
def _wait_until_device_data_applied(
    ...
    Args:
        ...
        apply_data (Callable[[], None]): Sends the data operation to ODL.
