...
public abstract class TransportChannel implements Mutable {
    ...
    private final CompletableFuture<Empty> closeFuture = new CompletableFuture<>();
    ...
    protected TransportChannel(final Channel channel) {
        ...
        channel.closeFuture().addListener(future -> {
            if (future.isSuccess()) {
                closeFuture.complete(Empty.value());
            } else {
    ...
    /**
     * Return a {@link CompletionStage} which completes when this channel is completely closed.
     *
     * @return A {@link CompletionStage}
     */
    public final CompletionStage<Empty> closeFuture() {
        return closeFuture;
    }
    ...
    public final Channel channel() {
        // FIXME: IllegalStateException when closed?
        return channel;
    }
