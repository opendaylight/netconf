...
final class ClientHttp2SseService extends ChannelInboundHandlerAdapter
    ...
    public void handlerAdded(final ChannelHandlerContext ctx) throws Exception {
        LOG.info("Client HTTP/2 SSE enabled on channel {}", channel);
    }
    ...
    public void startEventStream(final String requestUri, final EventStreamListener listener,
        ...
        LOG.info("SSE request sent to {}", requestUri);
    }

    void onChannelClosed() {
        frameListeners.forEach((streamId, frameListener) ->
            frameListener.listener().onStreamEnd());
    }
}
