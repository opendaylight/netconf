...
import org.opendaylight.yang.gen.v1.urn.opendaylight.yang.http.server.rev260731.http3.server.grouping.QuicUnderHttp;
...
public final class HTTPServer extends HTTPTransportStack {
     ...
     * @return a future completed with the established {@link HTTPServer}
    ...
    private static @NonNull ListenableFuture<HTTPServer> listenQuic(
        ...
        final var certificateKey = readTlsIdentity(quic.nonnullTlsServerParameters());
    ...
    private static TlsIdentity readTlsIdentity(final TlsServerGrouping tlsServerParameters)
            throws UnsupportedConfigurationException {
        ...
        return new TlsIdentity(certificate, keyPair.getPrivate());
    }
    ...
    private record QuicUnderlay(Channel channel, EventLoopGroup quicGroup) implements TransportStack {
        ...
        public @NonNull ListenableFuture<Empty> shutdown() {
            return Futures.whenAllComplete(HTTPServer.toListenableFuture(channel.close()),
