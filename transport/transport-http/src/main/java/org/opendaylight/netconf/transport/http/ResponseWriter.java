...
final class ResponseWriter extends ChannelInboundHandlerAdapter {
    ...
    private final Queue<HttpContent> pendingChunks = new ArrayDeque<>();
    ...
    synchronized boolean sendResponseStart(final HttpResponseStatus status, final ReadOnlyHttpHeaders headers,
        ...
        switch (state) {
            ...
            case Writable writable -> {
                var response = new DefaultHttpResponse(version, status);
    ...
    /**
     * Send a part of a response started via {@link #sendResponseStart(HttpResponseStatus, ReadOnlyHttpHeaders, HttpVersion)}.
     *
    ...
    synchronized private void sendQueue() {
