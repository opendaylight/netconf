...
import org.opendaylight.yang.gen.v1.urn.opendaylight.yang.http.server.rev260731.odl.server.limits.grouping.OdlServerLimits.BytebufAllocator;
 ...
 * @param initialWindowSize HTTP/2 flow-control window advertised as {@code SETTINGS_INITIAL_WINDOW_SIZE}, in bytes
 * @param socketReceiveBufferSize {@code SO_RCVBUF} of an accepted socket, in bytes, or zero to leave the operating
 *        system default in place
 * @param bytebufAllocator mode of the allocator backing the channel's buffers, resolved to a Netty allocator by
 *        {@link #allocator()}
 */
@NonNullByDefault
public record HTTPServerLimits(
    ...
    public static final HTTPServerLimits DEFAULT = new HTTPServerLimits(8192, 16384, 8192, 10485760, MIN_FRAME_SIZE,
        65535, 32768, BytebufAllocator.Pooled);
    ...
    public HTTPServerLimits {
        ...
        if (initialWindowSize < 0) {
            throw new IllegalArgumentException("initialWindowSize must not be negative");
        }
        if (socketReceiveBufferSize < 0) {
            throw new IllegalArgumentException("socketReceiveBufferSize must not be negative");
        }
    }
    ...
    private static <T extends ServerLimitsGrouping & OdlServerLimitsGrouping> HTTPServerLimits of(
        ...
        return new HTTPServerLimits(
            ...
            intOrDefault(limits == null ? null : limits.getInitialWindowSize(), DEFAULT.initialWindowSize),
            intOrDefault(odlLimits == null ? null : odlLimits.getSocketReceiveBufferSize(),
                DEFAULT.socketReceiveBufferSize),
            allocatorOrDefault(odlLimits == null ? null : odlLimits.getBytebufAllocator()));
    }
    ...
    public HTTPServerLimits withMaxFrameSize(final int newMaxFrameSize) {
        return newMaxFrameSize == maxFrameSize ? this
            : new HTTPServerLimits(maxInitialLineLength, maxHeaderSize, maxRequestChunkSize, maxRequestBodySize,
                newMaxFrameSize, initialWindowSize, socketReceiveBufferSize, bytebufAllocator);
    }

    /**
     * {@return the {@code server-limits} container carrying these limits}
     */
    public ServerLimits toServerLimits() {
        return new ServerLimitsBuilder()
            .setMaxInitialLineLength(Uint32.valueOf(maxInitialLineLength))
            .setMaxHeaderSize(Uint32.valueOf(maxHeaderSize))
            .setMaxRequestBodySize(Uint32.valueOf(maxRequestBodySize))
            .setMaxFrameSize(Uint32.valueOf(maxFrameSize))
            .setInitialWindowSize(Uint32.valueOf(initialWindowSize))
            .build();
    }

    /**
     * {@return the {@code odl-server-limits} container carrying these limits}
     */
    public OdlServerLimits toOdlServerLimits() {
        return new OdlServerLimitsBuilder()
            .setMaxRequestChunkSize(Uint32.valueOf(maxRequestChunkSize))
            .setSocketReceiveBufferSize(Uint32.valueOf(socketReceiveBufferSize))
            .setBytebufAllocator(bytebufAllocator)
            .build();
    }

    /**
     * {@return the Netty allocator implementing {@link #bytebufAllocator()}}
     */
    public ByteBufAllocator allocator() {
        return switch (bytebufAllocator) {
            // note: deliberately not ByteBufAllocator.DEFAULT, which is the pooled instance unless the JVM says
            // otherwise -- that would make the two modes indistinguishable
    ...
    private static BytebufAllocator allocatorOrDefault(final @Nullable BytebufAllocator allocator) {
        return allocator != null ? allocator : DEFAULT.bytebufAllocator;
    }
