...
public final class ResponseBodyOutputStream extends OutputStream {
    ...
    private static final class PendingBody extends Open {
        ...
        PendingBody(final HTTPServerSession session, final ChannelHandlerContext ctx, final int maxChunkSize,
                final HttpResponseStatus status, final ReadOnlyHttpHeaders headers, final @Nullable Integer streamId,
        ...
        @Override
        WithBody flush() throws IOException {
        ...
        private FirstChunk newFirstChunk() {
            return new FirstChunk(session, alloc, maxChunkSize, alloc.buffer(), status, headers, ctx, streamId, version);
        }
    }
    ...
    private static final class FirstChunk extends WithBody {
        ...
        FirstChunk(final HTTPServerSession session, final ByteBufAllocator alloc, final int maxChunkSize,
                ...
                final ChannelHandlerContext ctx, final @Nullable Integer streamId, final HttpVersion version) {
        ...
        State writeByte(final int value) throws IOException {
            ...
            session.sendResponsePart(buffer.retain());
...
//            return followingChunk();
        ...
        Closed close(final ReadOnlyHttpHeaders trailers) throws IOException {
            // FIXME: implement this
            HTTPServerSession.respond(ctx, streamId, new DefaultFullHttpResponse(version, status, buffer.retain()));
            return new Closed();
        }
    ...
    ResponseBodyOutputStream(final ChannelHandlerContext ctx, final HttpResponseStatus status,
            final ReadOnlyHttpHeaders headers, final @Nullable Integer streamId, final HttpVersion version) {
        this(ctx, status, headers, streamId, version, MAX_CHUNK_SIZE);
    }
    ...
    ResponseBodyOutputStream(final ChannelHandlerContext ctx, final HttpResponseStatus status,
            final ReadOnlyHttpHeaders headers, final @Nullable Integer streamId, final HttpVersion version,
        final int maxChunkSize) {
        state = new PendingBody(ctx.pipeline().get(HTTPServerSession.class), ctx, maxChunkSize, status,
            headers, streamId, version);
    }
