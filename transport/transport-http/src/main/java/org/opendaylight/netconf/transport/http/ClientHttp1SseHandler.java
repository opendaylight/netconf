...
final class ClientHttp1SseHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception {
        ...
        if (AWAITING_RESPONSE == currentState && msg instanceof HttpResponse response) {
            // response header to stream request
            if (HttpResponseStatus.OK.equals(response.status()) && response.headers()
                .contains(HttpHeaderNames.CONTENT_TYPE, HttpHeaderValues.TEXT_EVENT_STREAM, true)) {
        ...
        } else if (AWAITING_ERROR_MESSAGE == currentState && msg instanceof HttpContent content) {
            // decline response body
            onRequestDeclineError(ctx, content.content(), content instanceof LastHttpContent);

        } else if (AWAITING_EVENTS == currentState && msg instanceof HttpContent chunk) {
            // event chunk
            ...
            LOG.warn("Unexpected SSE message {} in state {}", msg, currentState);
        }
    }
