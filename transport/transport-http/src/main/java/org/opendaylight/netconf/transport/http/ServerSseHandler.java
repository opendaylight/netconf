...
import org.opendaylight.netconf.transport.http.EventStreamService.EventStream;
...
final class ServerSseHandler extends ChannelInboundHandlerAdapter implements EventStreamListener {
    ...
    private void channelRead(final ChannelHandlerContext ctx, final FullHttpRequest request) {
        ...
        if (HttpMethod.GET.equals(request.method())
            ...
            service.startEventStream(request.retain().uri(), this, new StartCallback() {
                @Override
                public void onStreamStarted(final EventStream result) {
                    confirmEventStreamRequest(request, result);
                    request.release();
                }
    ...
    private void confirmEventStreamRequest(final FullHttpRequest request, final EventStream startedStream) {
