...
public record HttpRequestLimits(Uint32 maxInitialLineLength, Uint32 maxHeaderSize, Uint32 maxRequestChunkSize,
    ...
    public HttpRequestLimits {
        checkPositiveIntRange("HTTP/1.1 request line length", maxInitialLineLength);
        checkPositiveIntRange("HTTP header size", maxHeaderSize);
        checkPositiveIntRange("HTTP/1.1 request decoder chunk size", maxRequestChunkSize);
        checkPositiveIntRange("HTTP request body size", maxRequestBodySize);
    }

    private static void checkPositiveIntRange(final String limitName, final Uint32 value) {
        if (requireNonNull(value).longValue() < 1) {
            throw new IllegalArgumentException(limitName + " must be at least one byte");
        }
        if (value.longValue() > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(limitName + " must not exceed " + Integer.MAX_VALUE);
        }
    }
}
