...
final class ClientHttp1SseService implements EventStreamService {
    ...
    ClientHttp1SseService(final Channel channel) {
        ...
        channel.closeFuture().addListener(ignored -> onChannelClose());
    ...
    public void startEventStream(final String requestUri, final EventStreamListener listener,
        ...
        if ((ctx = channel.pipeline().context(HttpObjectAggregator.class)) != null) {
            ...
            callback.onFailure(new IllegalStateException("No HttpObjectAggregator detected in channel, "
                + "SSE Handler cannot be positioning failed."));
        }
        ...
        LOG.info("SSE stream requested to {}", requestUri);
    }

    void onChannelClose() {
        final var handler = currentHandler.get();
        if (handler != null) {
            // report stream ends on channel close
            handler.listener().onStreamEnd();
        }
    }
}
