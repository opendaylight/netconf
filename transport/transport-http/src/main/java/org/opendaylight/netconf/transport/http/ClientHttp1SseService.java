...
final class ClientHttp1SseService implements EventStreamService {
    final class ClientSseHandler extends ClientHttp1SseHandler {
        @NonNullByDefault
        private ClientSseHandler(final String uri, final EventStreamListener listener,
    ...
    public void startEventStream(final String requestUri, final EventStreamListener listener,
        ...
        if (existing != null) {
            ...
            callback.onStartFailure(new IllegalStateException("Another stream is in progress."));
            return;
        }
        ...
        if (aggregatorContext == null) {
            ...
            callback.onStartFailure(new IllegalStateException("SSE Handler position cannot be defined"));
            channel.close();
            return;
        }

        final var newHandler = new ClientSseHandler(requestUri, listener, callback);
        pipeline.addBefore(aggregatorContext.name(), SSE_HANDLER_NAME, newHandler);
