 ...
 *
 * <p>When streaming is enabled, only a request using {@code Transfer-Encoding: chunked} is selected as
 * {@link RequestMode#STREAMING} and routed past {@link HttpObjectAggregator}, so that it and its body travel down the
 * pipeline as they are decoded. This choice is based on message framing, as the body length is not known from the
 * initial headers: a chunked request is streamed even if it eventually carries no body. A request declaring
 * {@code Content-Length}, including one exceeding the configured limit, is selected as {@link RequestMode#AGGREGATED}
 * and forwarded through the aggregator onto the existing {@link FullHttpRequest} path.
 *
...
final class Http1RequestDispatcher extends ChannelInboundHandlerAdapter {
    ...
    private void startRequest(final ChannelHandlerContext ctx, final HttpRequest request) {
        ...
        mode = streamingSupported && HttpUtil.isTransferEncodingChunked(request) && expectationSupported
            ? RequestMode.STREAMING : RequestMode.AGGREGATED;
    ...
    private void forward(final ChannelHandlerContext ctx, final Object msg) {
        switch (mode) {
            ...
            case STREAMING -> {
                ...
                if (aggregator != null && isDownstreamOf(aggregator, ctx)) {
                    aggregator.fireChannelRead(msg);
                } else {
                    // Should not happen: whoever installs us also installs the aggregator after us. Forwarding from
                    // our own context avoids re-entering this handler if a handler with that name is installed ahead.
                    LOG.error("{}: no handler named {} follows {}, forwarding {} from the dispatcher context",
                        ctx.channel(), AGGREGATOR_NAME, ctx.name(), msg.getClass());
                    ctx.fireChannelRead(msg);
                }
            }
        }
    ...
    private static boolean isDownstreamOf(final ChannelHandlerContext candidate, final ChannelHandlerContext current) {
        final var names = current.pipeline().names();
        final var currentIndex = names.indexOf(current.name());
        return currentIndex >= 0 && names.indexOf(candidate.name()) > currentIndex;
    }
}
