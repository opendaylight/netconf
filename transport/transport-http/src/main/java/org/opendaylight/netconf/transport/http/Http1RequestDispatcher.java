 ...
 * observes a streamed request just like it observes an aggregated one.
 *
...
final class Http1RequestDispatcher extends ChannelInboundHandlerAdapter {
    ...
    private void startRequest(final ChannelHandlerContext ctx, final HttpRequest request) {
        ...
        final var expectationSupported = HttpHeaderValues.CONTINUE.contentEqualsIgnoreCase(expect);
        ...
        if (!HttpUtil.isTransferEncodingChunked(request) || !expectationSupported) {
            ...
            LOG.info("{}: {} {} would be {}, but a streaming request body is not implemented yet", ctx.channel(),
                request.method(), request.uri(), RequestMode.STREAMING);
            mode = RequestMode.AGGREGATED;
        }
