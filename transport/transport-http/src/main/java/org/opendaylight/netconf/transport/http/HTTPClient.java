...
public abstract sealed class HTTPClient extends HTTPTransportStack
    ...
    public static ListenableFuture<HTTPClient> connect(
        ...
        switch (transport) {
            ...
            case Quic quicCase -> {
                return connectQuic(listener, quicCase);
            }
     ...
     *
     * @param listener {@link TransportChannelListener} to notify when the session is established
     * @param quicCase QUIC connection parameters
    ...
    private static ListenableFuture<HTTPClient> connectQuic(
            final TransportChannelListener<? super HTTPTransportChannel> listener, final Quic quicCase)
            throws UnsupportedConfigurationException {
        final var quic = quicCase.getQuic();
        if (quic == null) {
            throw new UnsupportedConfigurationException("Missing quic parameters");
        }

        final var udpParams = quic.nonnullUdpClientParameters();
        final var remoteAddress = remoteAddressOf(udpParams);
        final var localAddress = localAddressOf(udpParams);
        final var trustCertificates = readTrustCertificates(quic.nonnullTlsClientParameters());
        final var quicParams = requireQuicClientParameters(quic.getQuicClientParameters());
        final var httpParams = quic.getHttpClientParameters();

        final var sslContext = QuicSslContextBuilder.forClient()
            .trustManager(trustCertificates)
            .applicationProtocols(Http3.supportedApplicationProtocols())
            .build();
        final var initialMaxStreamData = quicParams.requireInitialMaxStreamDataBidiRemote().getValue().longValue();
        final var codec = Http3.newQuicClientCodecBuilder()
            .sslContext(sslContext)
            .initialMaxData(quicParams.requireInitialMaxData().getValue().longValue())
            ...
            .initialMaxStreamsBidirectional(quicParams.requireInitialMaxStreamsBidi().longValue())
            .build();

        final var client = new QuicHTTPClient(listener);
        final var group = NettyTransportSupport.newEventLoopGroup(0,
            Thread.ofPlatform().name("transport-http3-client-", 0).factory());
            ...
            .addListener((ChannelFutureListener) bindFuture -> {
                ...
                QuicChannel.newBootstrap(datagramChannel)
                    ...
                    .addListener((GenericFutureListener<Future<QuicChannel>>) connectFuture -> {
                        ...
                        underlayFuture.set(new QuicUnderlay(datagramChannel, quicChannel, group));
                        client.addTransportChannel(new HTTPTransportChannel(new QuicTransportChannel(quicChannel),
                            client.scheme()));
                    });
            });
    ...
    private static void collectTrustCertificates(final List<X509Certificate> certificates,
    ...
    private record QuicUnderlay(Channel datagramChannel, QuicChannel quicChannel, EventLoopGroup quicGroup)
            implements TransportStack {
        @Override
        public @NonNull ListenableFuture<Empty> shutdown() {
            return Futures.whenAllComplete(HTTPClient.toListenableFuture(quicChannel.close()),
                HTTPClient.toListenableFuture(datagramChannel.close()),
                HTTPClient.toListenableFuture(quicGroup.shutdownGracefully()))
