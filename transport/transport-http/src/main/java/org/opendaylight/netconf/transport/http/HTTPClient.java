...
public abstract sealed class HTTPClient extends HTTPTransportStack
    ...
    private static ListenableFuture<HTTPClient> connectQuic(
        ...
        final var underlayFuture = SettableFuture.<ClientQuicUnderlay>create();
            ...
            .addListener((ChannelFutureListener) bindFuture -> {
                ...
                QuicChannel.newBootstrap(datagramChannel)
                    ...
                    .addListener((GenericFutureListener<Future<QuicChannel>>) connectFuture -> {
                        ...
                        if (underlayFuture.set(new ClientQuicUnderlay(datagramChannel, quicChannel))) {
                            ...
                            // ClientQuicUnderlay#shutdown(), so close both channels here to avoid leaking the QUIC
    ...
    private static @NonNull InetSocketAddress remoteAddressOf(final UdpClientParameters udpParams)
        ...
        final var domainName = host.getDomainName();
        if (domainName != null) {
            return new InetSocketAddress(domainName.getValue(), portNumber);
        }
        throw new UnsupportedConfigurationException("Host has neither an IP address nor a domain name: " + host);
    }
