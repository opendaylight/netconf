...
public enum HTTPScheme {
        ...
        void initializeServerPipeline(final ChannelHandlerContext ctx) {
            ...
            ctx.pipeline()
                .addBefore(ctx.name(), null, new CleartextHttp2ServerUpgradeHandler(
                    ...
                    twoToOne))
                .addBefore(ctx.name(), null, new SimpleChannelInboundHandler<HttpMessage>() {
                    @Override
                    ...
                    public void userEventTriggered(final ChannelHandlerContext ctx, final Object event)
                            throws Exception {
                        // We may be involved in a cleartext upgrade to HTTP/2, in which case we need to route it
                        // outside the normal read flow. Since we are restoring that magic, there is no need for
                        // downstream handlers to see this event.
    ...
    HTTPS(HttpScheme.HTTPS) {
        @Override
        void initializeServerPipeline(final ChannelHandlerContext ctx) {
            ctx.pipeline().addBefore(ctx.name(), null, new AlpnHandler());
        }
    };
