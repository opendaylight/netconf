...
public enum HTTPScheme {
    ...
    HTTP(HttpScheme.HTTP) {
        ...
        void initializeServerPipeline(final ChannelHandlerContext ctx) {
            ...
            ctx.pipeline()
                .addBefore(ctx.name(), null, new CleartextHttp2ServerUpgradeHandler(sourceCodec,
                    new HttpServerUpgradeHandler(sourceCodec,
                        protocol -> {
                            if (AsciiString.contentEquals(Http2CodecUtil.HTTP_UPGRADE_PROTOCOL_NAME, protocol)) {
                                final var childChannelInit = ctx.channel().attr(Http2Hooks.CHILD_INIT).get();
                                return new Http2ServerUpgradeCodec(newHttp2FrameCodec(FRAME_LOGGER),
                                    new Http2MultiplexHandler(childChannelInit));
                            }
                            return null;
                        },
                        HTTPServer.MAX_HTTP_CONTENT_LENGTH), newHttp2FrameCodec(FRAME_LOGGER)))
