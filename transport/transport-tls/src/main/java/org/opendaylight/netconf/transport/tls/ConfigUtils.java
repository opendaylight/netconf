...
import org.opendaylight.netconf.transport.crypto.CertificateSupport;
...
final class ConfigUtils {
    ...
    private static Map<String, Certificate> extractCertificates(final @Nullable InlineOrTruststoreCertsGrouping certs,
        ...
        for (var cert : inlineDef.nonnullCertificate().values()) {
            ...
            mapBuilder.put(alias, CertificateSupport.parseCertificate(cert.requireCertData()));
        }
    ...
    static void setEndEntityCertificateWithKey(final @NonNull KeyStore keyStore,
        ...
        final var keyPair = extractKeyPair(inlineDef);
        final var certificate = CertificateSupport.parseCertificate(inlineDef.requireCertData());
