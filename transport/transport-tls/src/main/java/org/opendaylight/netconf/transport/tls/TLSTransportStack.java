...
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.crypto.types.rev220707.EcPrivateKeyFormat;
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.crypto.types.rev220707.RsaPrivateKeyFormat;
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.crypto.types.rev220707.SubjectPublicKeyInfoFormat;
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.crypto.types.rev220707.TrustAnchorCertCms;
...
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.keystore.rev220524.LocalOrKeystoreAsymmetricKeyGrouping;
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.keystore.rev220524.LocalOrKeystoreEndEntityCertWithKeyGrouping;
...
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.truststore.rev220524.LocalOrTruststoreCertsGrouping;
import org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.truststore.rev220524.LocalOrTruststorePublicKeysGrouping;
 ...
 */
public abstract sealed class TLSTransportStack extends AbstractTransportStack<TLSTransportChannel>
        ...
        public void onTransportChannelEstablished(final TransportChannel channel) {
            ...
            sslHandler.handshakeFuture().addListener(future -> {
                ...
                if (cause != null) {
                    ...
                    addTransportChannel(new TLSTransportChannel(channel));
                }
            });
        }
    };

    private volatile TCPTransportStack tcpStack;
    ...
    final @NonNull TransportChannelListener asListener() {
        return asListener;
    }
    ...
    final void setTcpStack(final TCPTransportStack tcpStack) {
        this.tcpStack = requireNonNull(tcpStack);
    }

    static final <T extends TLSTransportStack> @NonNull ListenableFuture<T> transform(final T stack,
            final ListenableFuture<? extends TCPTransportStack> tcpFuture) {
        return Futures.transform(tcpFuture, tcpStack -> {
            stack.setTcpStack(tcpStack);
            return stack;
    ...
    static KeyStore loadKeyStore(final KeyStore existing, final String aliasPrefix,
        ...
        if (!(localOrTruststore
            instanceof org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.truststore.rev220524.local.or
    ...
    static final KeyManager newKeyManager(final @NonNull LocalOrKeystoreEndEntityCertWithKeyGrouping certificate)
        ...
        final var keyStore = loadKeyStore(localDef);
    ...
    static final KeyManager newKeyManager(final @NonNull LocalOrKeystoreAsymmetricKeyGrouping rawPrivateKey)
        ...
        if (!(localOrKeystore
            instanceof org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.keystore.rev220524.local.or
        ...
        return newKeyManager(loadKeyStore(localDef));
    }
    ...
    private static KeyStore loadKeyStore(final AsymmetricKeyPairGrouping keyPair)
            throws UnsupportedConfigurationException {
        ...
        try {
            ...
            throw new UnsupportedConfigurationException("Cannot handle private key format " + privateFormat);
        }
