...
import static org.opendaylight.netconf.transport.tls.config.ConfigUtils.setAsymmetricKey;
import static org.opendaylight.netconf.transport.tls.config.ConfigUtils.setEndEntityCertificateWithKey;
import static org.opendaylight.netconf.transport.tls.config.ConfigUtils.setX509Certificates;
import static org.opendaylight.netconf.transport.tls.config.KeyStoreUtils.buildKeyManagerFactory;
import static org.opendaylight.netconf.transport.tls.config.KeyStoreUtils.buildTrustManagerFactory;
import static org.opendaylight.netconf.transport.tls.config.KeyStoreUtils.newKeyStore;
...
public abstract sealed class TLSTransportStack extends AbstractOverlayTransportStack<TLSTransportChannel>
    ...
    private static String[] createTlsStrings(final Set<TlsVersionBase> versions)
        ...
        for (var version : versions) {
            final var str = ofNullable(IetfTlsCommonFeatureProvider.algorithmNameOf(version))
                    .orElseThrow(() -> new UnsupportedConfigurationException("Unhandled TLS version " + version));
            ret[idx++] = str;
        }
        return ret;
    }
    ...
    private static ImmutableList<String> createCipherStrings(final List<CipherSuiteAlgBase> ciphers)
        ...
        for (var cipher : ciphers) {
            final var str = ofNullable(CIPHER_SUITES.get(cipher))
                    .orElseThrow(() -> new UnsupportedConfigurationException("Unhandled cipher suite " + cipher));
            builder.add(str);
        }
        return builder.build();
    }
