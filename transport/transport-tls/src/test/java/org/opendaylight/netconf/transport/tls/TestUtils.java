...
public class TestUtils {
    ...
    private static X509CertInputs buildX509CertInputs(final String resourcePefix) {
        try {
            final String keyAlgorithm = resourcePefix.contains("rsa") ? "RSA" : "EC";
            // x.509 certificate and public key
            final byte[] certInBytes = resourceToBytes(resourcePefix + ".crt");
            ...
            try (InputStream in = new ByteArrayInputStream(resourceToBytes(resourcePefix + ".key"));
            ...
            final String sshKeyStr = new String(resourceToBytes(resourcePefix + ".pub")).trim();
    ...
    private static byte[] resourceToBytes(final String resource) throws IOException {
        return TestUtils.class.getResourceAsStream(resource).readAllBytes();
    }
