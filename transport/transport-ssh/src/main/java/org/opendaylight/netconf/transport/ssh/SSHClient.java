...
import org.opendaylight.netconf.transport.ssh.client.SshServerKeyVerifier;
import org.opendaylight.netconf.transport.ssh.config.ConfigUtils;
...
public final class SSHClient extends SSHTransportStack {
    ...
    private static String getUsername(final SshClientGrouping clientParams) {
        return Optional.ofNullable(clientParams.getClientIdentity()).map(ClientIdentity::getUsername).orElse("");
    }
    ...
    private static void setServerAuthentication(final @NonNull ClientFactoryManager factoryMgr,
        ...
        if (!certificatesList.isEmpty() || !publicKeys.isEmpty()) {
            factoryMgr.setServerKeyVerifier(new SshServerKeyVerifier(certificatesList, publicKeys));
        } else {
