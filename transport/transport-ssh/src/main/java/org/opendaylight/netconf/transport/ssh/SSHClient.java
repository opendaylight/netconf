...
public final class SSHClient extends SSHTransportStack {
    ...
    void onKeyEstablished(final Session session) throws IOException {
        ...
        if (algListener != null) {
            final var kex = new SshKeyExchangeAlgorithm(clientSession.getNegotiatedKexParameter(KexProposalOption.ALGORITHMS));
            final var hostKey = new SshPublicKeyAlgorithm(clientSession.getNegotiatedKexParameter(KexProposalOption.SERVERKEYS));
            final var encryption = new SshEncryptionAlgorithm(clientSession.getNegotiatedKexParameter(KexProposalOption.C2SENC));
