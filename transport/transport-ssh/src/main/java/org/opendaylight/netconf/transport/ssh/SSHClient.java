...
public final class SSHClient extends SSHTransportStack {
    ...
    void onKeyEstablished(final Session session) throws IOException {
        ...
        if (algListener != null) {
            final var kex = clientSession.getNegotiatedKexParameter(KexProposalOption.ALGORITHMS);
            final var hostKey = clientSession.getNegotiatedKexParameter(KexProposalOption.SERVERKEYS);
            final var encryption = clientSession.getNegotiatedKexParameter(KexProposalOption.C2SENC);
            final var mac = clientSession.getNegotiatedKexParameter(KexProposalOption.C2SMAC);
