...
final class TransportIoService extends NettyIoService {
    TransportIoService(final FactoryManager factoryManager, final IoHandler handler) {
        ...
        // we are not using NettyIoService's logic for channel group and sessions management, see overrides below
        doCloseImmediately();
    }
    ...
    @Override
    protected void mapSession(final IoSession session) throws CancellationException {
        // we do not need to use any NettyIoService's session management because we have our own in SSHTransportStack
