...
public class UserAuthSessionListener implements SessionListener {
    ...
    private void deleteSession(final Session session) {
        ...
        sessions.remove(id);
        ofNullable(sessionAuthHandlers.remove(id)).map(AuthHandler::onFailure).ifPresent(Runnable::run);
    }
    ...
    public void sessionEvent(Session session, Event event) {
        ...
        if (Event.Authenticated == event) {
            ofNullable(sessionAuthHandlers.remove(idOf(session))).map(AuthHandler::onSuccess).ifPresent(Runnable::run);
        }
    }
