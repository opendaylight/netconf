 ...
 */
final class BuiltinKeyExchangePolicy implements KeyExchangePolicy {
    static final @NonNull BuiltinKeyExchangePolicy CLIENT;
    static final @NonNull BuiltinKeyExchangePolicy SERVER;
    ...
    static {
        ...
        CLIENT = new BuiltinKeyExchangePolicy(ClientBuilder.DH2KEX, factories, BaseBuilder.DEFAULT_KEX_PREFERENCE);
        SERVER = new BuiltinKeyExchangePolicy(ServerBuilder.DH2KEX, factories, BaseBuilder.DEFAULT_KEX_PREFERENCE);
    }
    ...
    private final @NonNull Map<
        ...
        KeyExchangeFactory> factories;
    private final @NonNull List<KeyExchangeFactory> defaultExchangeConfig;

    private BuiltinKeyExchangePolicy(final Function<DHFactory, KeyExchangeFactory> dh2kex, final Map<
            org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.ssh.common.rev241010.SshKeyExchangeAlgorithm,
            DHFactory> dhFactories, final List<? extends DHFactory> defaultDhFactories) {
        factories = Map.copyOf(Maps.transformValues(dhFactories, dh2kex::apply));
        defaultExchangeConfig = defaultDhFactories.stream()
            .map(ServerBuilder.DH2KEX)
            .collect(Collectors.toUnmodifiableList());
    }

    @Override
    public List<KeyExchangeFactory> defaultExchangeConfig() {
        return defaultExchangeConfig;
    }

    @Override
    public List<KeyExchangeFactory> exchangeConfigOf(final KeyExchange keyExchange)
        ...
        if (kexAlg != null && !kexAlg.isEmpty()) {
            ...
            return ConfigUtils.mapValues(factories, kexAlg, "Unsupported Key Exchange algorithm %s");
        }
        return defaultExchangeConfig;
    }

    @VisibleForTesting
    @Nullable KeyExchangeFactory factoryFor(final SshKeyExchangeAlgorithm alg) {
        return factories.get(keyOf(alg));
    }

    @VisibleForTesting
    Stream<KeyExchangeFactory> allFactories() {
