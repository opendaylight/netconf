...
public abstract sealed class SSHTransportStackBootstrap extends AbstractTransportStackBootstrap {
    ...
    private static final ImmutableMap<KeyExchangeAlgBase, BuiltinDHFactories> KEXS =
        ImmutableMap.<KeyExchangeAlgBase, BuiltinDHFactories>builder()
...
//            .put(Rsa2048Sha256.VALUE, null)
    ...
    private static List<CipherFactory> createCipherFactories(final List<EncryptionAlgBase> encAlg) {
        // FIXME: cache these
        final var builder = ImmutableList.<CipherFactory>builderWithExpectedSize(encAlg.size());
        for (var alg : encAlg) {
    ...
    private static @NonNull KeyExchangeFactory kexFactoryOf(final KeyExchangeAlgBase alg) {
