...
import org.opendaylight.netconf.transport.tcp.TransportBootstrap;
 ...
 */
public abstract sealed class SSHTransportStackBootstrap extends TransportBootstrap {
    private static final class Client extends SSHTransportStackBootstrap {
        ...
        CompletionStage<TransportStack> initiate(final SshdSocketAddress local, final SshdSocketAddress remote)
            ...
            try {
                connectFuture = client.connect("FIXME", remote, local);
            } catch (IOException e) {
                closeAndComplete(client, ret, e);
                return ret;
            }
            ...
            connectFuture.addListener(future -> {
                ...
                if (ex == null) {
                    final var stack = new SSHTransportStack.Initiate(listener(), client);
                    ret.complete(stack);
//                    future.getClientSession();
                } else {
                    closeAndComplete(client, ret, ex);
                }
            });
        ...
        private @NonNull NetconfSshClient newClient() throws UnsupportedConfigurationException {
...
//            .put(ExtInfoS.VALUE, null)
    ...
    private static void closeAndComplete(final AbstractCloseable closeable,
            final CompletableFuture<TransportStack> future, final Throwable cause) {
        closeable.close(true).addListener(ignored -> future.completeExceptionally(cause));
    }
}
