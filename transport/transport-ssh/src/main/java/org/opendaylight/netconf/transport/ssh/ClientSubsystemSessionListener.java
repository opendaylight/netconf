 ...
 */
final class ClientSubsystemSessionListener implements SessionListener {
    private final String subsystemName;

    ClientSubsystemSessionListener(final String subsystemName) {
    ...
    public void sessionEvent(final Session session, final Event event) {
        if (Event.Authenticated == event && session instanceof ClientSession clientSession) {
            try {
                ...
                subsystem.onClose(() -> clientSession.close(true));
                subsystem.open().addListener(future -> {
                    final var failure = future.getException();
                    if (failure != null) {
                        clientSession.exceptionCaught(failure);
                    }
                });
            } catch (IOException e) {
                clientSession.exceptionCaught(e);
            }
        }
    }
}
