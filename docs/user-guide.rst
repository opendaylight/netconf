...
.. note::
    ...
    How to do this is described in the `Netconf-keystore configuration`_ section, where you find the
    necessary RPC to add keystore entries.
...
..note::
    ...
    If you already have installed feature ``odl-netconf-topology`` to connect
    device it is not necessary to install ``odl-netconf-keystore`` feature.
...
.. note::
    ...
    it means that the data must be base64 encoded and wrapped in the
