...
- The OpenAPI feature installed (``odl-restconf-openapi``).

Keycloak Client Setup
^^^^^^^^^^^^^^^^^^^^^

1. In the Keycloak Admin Console, select your realm and create a new **Client**.

2. Set **Client type** to ``OpenID Connect`` and choose a ``Client ID`` (e.g. ``odl-openapi``).

3. Set **Client authentication** to *Off* (public client — no client secret is needed for PKCE).

4. Enable the **Standard flow** (authorization code).

5. Under **Valid redirect URIs**, add the Swagger UI OAuth2 redirect callback::

       http://<odl-host>:8181/openapi/explorer/oauth2-redirect.html

6. Note the authorization and token endpoint URLs. They are visible at:
   ``Realm Settings → General → OpenID Endpoint Configuration`` (the ``.well-known`` discovery
   document), or directly as:
...
``org.opendaylight.restconf.nb.rfc8040``. The simplest way is to create (or edit) the
corresponding ``.cfg`` file in Karaf's ``etc/`` directory::

    # etc/org.opendaylight.restconf.nb.rfc8040.cfg
...
.. note::
    ...
    When either property is left empty (the default), the OAuth2 security scheme is omitted from
    the generated OpenAPI document and only HTTP Basic authentication is offered.

After the configuration is applied, restart or reload the ``odl-restconf-openapi`` bundle for the
...
If a user leaves the Swagger UI open for more than 30 minutes and the JWT expires, **Try it out**
