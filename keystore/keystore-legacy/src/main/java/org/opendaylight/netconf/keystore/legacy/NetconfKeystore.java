 ...
 */
@Singleton
@Component(service = KeystoreContent.class)
public final class NetconfKeystore implements KeystoreContent, TruststoreContent, AutoCloseable {
    @NonNullByDefault
    private record State(
            Map<String, CertifiedPrivateKey> privateKeys,
            Map<String, X509Certificate> trustedCertificates) implements Immutable {
        static final State EMPTY = new State(Map.of(), Map.of());
    ...
    private static final Logger LOG = LoggerFactory.getLogger(NetconfKeystore.class);

    private final AtomicReference<@NonNull ConfigState> configState = new AtomicReference<>(ConfigState.EMPTY);
    ...
    @Inject
    @Activate
    public NetconfKeystore(@Reference final DataBroker dataBroker, final @Reference RpcProviderService rpcProvider,
            final @Reference ClusterSingletonServiceProvider cssProvider,
            final @Reference AAAEncryptionService encryptionService) {
    ...
    public void close() {
        ...
        if (listener != null) {
            configListener = null;
            listener.close();
            configState.set(ConfigState.EMPTY);
        }
        LOG.info("NETCONF keystore service stopped");
    }

    @Override
    public Map<String, X509Certificate> certificates() {
        return state.getAcquire().trustedCertificates;
    }

    @Override
    public Map<String, CertifiedPrivateKey> privateKeys() {
        return state.getAcquire().privateKeys;
    }

    void runUpdate(final Consumer<@NonNull ConfigStateBuilder> task) {
        final var prevState = configState.getAcquire();
        ...
        if (configListener == null || configState.compareAndExchangeRelease(prevState, newState) != prevState) {
            return;
        }
        ...
        state.set(new State(keys, certs));
