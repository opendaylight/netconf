...
public class DefinitionGenerator {
    ...
    public JsonGenerator convertToJsonSchema(final Module module, final EffectiveModelContext schemaContext,
            final JsonGenerator gen, final DefinitionNames definitionNames, final OAversion oaversion,
        ...
        processIdentities(module, gen, definitionNames, schemaContext);
        processContainersAndLists(module, gen, definitionNames, schemaContext, oaversion);
        processRPCs(module, gen, definitionNames, schemaContext, oaversion);

        if (isForSingleModule) {
            processModule(module, gen, definitionNames, schemaContext, oaversion);
        }
        return gen;
    }
        ...
        for (final DataSchemaNode node : module.getChildNodes()) {
            ...
            if (node.isConfiguration()) {
                if (node instanceof ContainerSchemaNode || node instanceof ListSchemaNode) {
                    ...
                    processLeafNode(leafNode, localName, stack, definitionNames, oaversion, definitionObject);
                }
            }
            stack.exit();
        }
    ...
    private void processContainersAndLists(final Module module, final JsonGenerator gen,
        ...
        for (final DataSchemaNode childNode : module.getChildNodes()) {
            ...
            if (childNode instanceof ContainerSchemaNode || childNode instanceof ListSchemaNode) {
                DefinitionObject definitionObject = new DefinitionObject();
                processDataNodeContainer((DataNodeContainer) childNode, moduleName, definitionNames, stack, oaversion,
                        definitionObject);
                definitionObject.writeDataToJsonGenerator(gen);

                definitionObject = new DefinitionObject();
                processActionNodeContainer(childNode, moduleName, definitionNames, stack, oaversion, definitionObject);
                definitionObject.writeDataToJsonGenerator(gen);
            }
            stack.exit();
        }
    ...
    private void processRPCs(final Module module, final JsonGenerator gen, final DefinitionNames definitionNames,
        ...
        for (final RpcDefinition rpcDefinition : module.getRpcs()) {
            ...
            processOperations(rpcDefinition, moduleName, definitionNames, stack, oaversion, definitionObject);
            stack.exit();
        }
        definitionObject.writeDataToJsonGenerator(gen);
    }
    ...
    private void processOperationInputOutput(final ContainerLike container, final String operationName,
        ...
        if (!container.getChildNodes().isEmpty()) {
            ...
            processChildren(container.getChildNodes(), parentName, definitionNames, stack, oaversion, childSchemaObj);
    ...
    private DefinitionObject processDataNodeContainer(final DataNodeContainer dataNode, final String parentName,
        ...
        if (dataNode instanceof ListSchemaNode || dataNode instanceof ContainerSchemaNode) {
            ...
            final DefinitionObject propertiesObj = processChildren(containerChildren, parentName + "_" + localName,
                    definitionNames, stack, oaversion, childSchemaObj);
    ...
    private DefinitionObject processChildren(final Collection<? extends DataSchemaNode> nodes, final String parentName,
        ...
        for (final DataSchemaNode node : nodes) {
            processChildNode(node, parentName, definitionNames, stack, oaversion, propertiesObj);
        }
            ...
            if (node instanceof ListSchemaNode || node instanceof ContainerSchemaNode) {
                ...
                for (final CaseSchemaNode variant : choice.getCases()) {
                    ...
                    for (final DataSchemaNode childNode : variant.getChildNodes()) {
                        processChildNode(childNode, parentName, definitionNames, stack, oaversion, defObj);
                    }
                    stack.exit();
                }
                propertyObj = null;
    ...
    private DefinitionObject processLeafListNode(final LeafListSchemaNode listNode, final SchemaInferenceStack stack,
            final DefinitionNames definitionNames, final OAversion oaversion, final DefinitionObject defObj) {
    ...
    private void processLeafNode(final LeafSchemaNode leafNode, final String jsonLeafName,
            final SchemaInferenceStack stack, final DefinitionNames definitionNames, final OAversion oaversion,
    ...
    private String processIdentityRefType(final IdentityrefTypeDefinition leafTypeDef,
            final DefinitionNames definitionNames, final OAversion oaversion, final EffectiveModelContext schemaContext,
            final DefinitionObject defObj) {
