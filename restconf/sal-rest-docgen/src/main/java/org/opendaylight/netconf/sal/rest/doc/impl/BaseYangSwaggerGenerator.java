...
public class BaseYangSwaggerGenerator {
    ...
    private void addApis(final DataSchemaNode node, final List<Api> apis, final String parentPath, final List<Parameter> parentPathParams, final SchemaContext schemaContext,
        ...
        String putParameterType = null;
        if (apis.size() > 0) {
            for (final Operation operation : apis.get(apis.size() - 1).getOperations()) {
                if ("POST".equals(operation.getMethod())) {
                    for (final Parameter parameter : operation.getParameters()) {
                        if ("body".equals(parameter.getParamType())) {
                            putParameterType = parameter.getType();
                        }
                    }
                }
            }
        }
        api.setOperations(operation(node, pathParams, addConfigApi, childSchemaNodes, putParameterType));
        apis.add(api);
    ...
    private List<Operation> operation(final DataSchemaNode node, final List<Parameter> pathParams, final boolean isConfig, final Iterable<DataSchemaNode> childSchemaNodes, final String putParameterType) {
        ...
        if (isConfig) {
            ...
            if (putParameterType != null) {
                for (final Parameter parameter : operations.get(operations.size() - 1).getParameters()) {
                    if ("body".equals(parameter.getParamType())) {
                        parameter.setType(putParameterType);
                    }
                }
            }
