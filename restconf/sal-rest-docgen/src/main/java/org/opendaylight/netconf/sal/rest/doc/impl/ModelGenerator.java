...
public class ModelGenerator {
    ...
    private void processRPCs(final Module module, final JSONObject models, final SchemaContext schemaContext) throws JSONException,
        ...
        for (final RpcDefinition rpc : rpcs) {
            ...
            if (input != null) {
                ...
                dataNodeProperties.put(TYPE_KEY, OBJECT_TYPE);
                dataNodeProperties.put(ITEMS_KEY, items);

                dataNodeProperties.putOpt(DESCRIPTION_KEY, input.getDescription());
                final JSONObject properties2 = new JSONObject();
                properties2.put("input", dataNodeProperties);
                final JSONObject finalChildSchema = getSchemaTemplate();
                finalChildSchema.put(TYPE_KEY, OBJECT_TYPE);
                finalChildSchema.put(PROPERTIES_KEY, properties2);
                finalChildSchema.put("id", filename + OperationBuilder.TOP);
            ...
            if (output != null) {
                ...
                dataNodeProperties.putOpt(DESCRIPTION_KEY, output.getDescription());
                final JSONObject properties2 = new JSONObject();
                properties2.put("output", dataNodeProperties);
                final JSONObject finalChildSchema = getSchemaTemplate();
                finalChildSchema.put(TYPE_KEY, OBJECT_TYPE);
                finalChildSchema.put(PROPERTIES_KEY, properties2);
                finalChildSchema.put("id", filename + OperationBuilder.TOP);
    ...
    private JSONObject processDataNodeContainer(final DataNodeContainer dataNode, final String moduleName, final JSONObject models,
        ...
        if (dataNode instanceof ListSchemaNode || dataNode instanceof ContainerSchemaNode) {
            ...
            dataNodeProperties.putOpt(DESCRIPTION_KEY, ((SchemaNode) dataNode).getDescription());
            final JSONObject properties2 = new JSONObject();
            properties2.put(((SchemaNode) dataNode).getQName().getLocalName(), dataNodeProperties);
            final JSONObject finalChildSchema = getSchemaTemplate();
            finalChildSchema.put(TYPE_KEY, OBJECT_TYPE);
            finalChildSchema.put(PROPERTIES_KEY, properties2);
            finalChildSchema.put("id", nodeName + OperationBuilder.TOP);
            models.put(nodeName + OperationBuilder.TOP, finalChildSchema);
