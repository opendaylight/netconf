...
public final class ApiDocGeneratorRFC8040Test extends AbstractApiDocTest {
    ...
    public void testSchemas() throws IOException {
        final var module = CONTEXT.findModule(NAME, Revision.of(REVISION_DATE)).orElseThrow();
        final OpenApiObject doc = generator.getOpenApiDocSpec(module, "http", "localhost:8181", "/", "", CONTEXT);
        doc.getComponents().close();

        final JsonFactory factory = new JsonFactory();
        final JsonParser parser = factory.createParser(doc.getComponents().getSchemas());
    ...
    public void testChoice() throws IOException {
        ...
        final var parser = factory.createParser(doc.getComponents().getSchemas());
