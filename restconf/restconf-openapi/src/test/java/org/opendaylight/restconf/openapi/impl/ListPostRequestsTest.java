...
public class ListPostRequestsTest {
    ...
    public void testListPostRequest() {
        ...
        final var pathToContainerUnderList = "/rests/data/list-post:container/list={name},{address}/child-cont";
        assertNull(doc.paths().get(pathToContainerUnderList).post());
        assertNotNull(doc.paths().get(pathToContainerUnderList).put());

        final var pathToYoungestContainer = "/rests/data/list-post:container/list={name},{address}/child-cont/"
            + "youngest-cont";
        assertNull(doc.paths().get(pathToYoungestContainer).post());
        assertNotNull(doc.paths().get(pathToYoungestContainer).put());
    }
}
