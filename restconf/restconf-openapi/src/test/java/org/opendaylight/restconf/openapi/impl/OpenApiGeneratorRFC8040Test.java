...
public final class OpenApiGeneratorRFC8040Test {
    ...
    public void testRecursiveParameters() {
        ...
        for (final var expectedPath : configPaths.entrySet()) {
            ...
            assertNotNull(get);
            assertEquals(expectedSize + 1, get.getParameters().size());

            final var put = path.getPut();
            assertNotNull(put);
            assertEquals(expectedSize, put.getParameters().size());

            final var delete = path.getDelete();
            assertNotNull(delete);
            assertEquals(expectedSize, delete.getParameters().size());

            final var post = path.getPost();
            assertNotNull(post);
            assertEquals(expectedSize, post.getParameters().size());

            final var patch = path.getPatch();
            assertNotNull(patch);
            assertEquals(expectedSize, patch.getParameters().size());
        }
    }
    ...
    public void testToaster2OpenApiObjects() {
        ...
        final var postContent = jsonNodeCancelToast.getPost().getRequestBody().get("content");
     ...
     */
    private static void verifyRequestRef(final Operation path, final String expectedJsonRef,
            final String expectedXmlRef) {
        final JsonNode postContent;
        if (path.getRequestBody() != null) {
            postContent = path.getRequestBody().get("content");
        } else {
            postContent = path.getResponses().get("200").get("content");
        }
