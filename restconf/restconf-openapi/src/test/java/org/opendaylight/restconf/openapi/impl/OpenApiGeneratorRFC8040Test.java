...
public final class OpenApiGeneratorRFC8040Test {
    ...
    public void testXmlNodes() {
        ...
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", simpleList1.xml().get("namespace").asText());
        assertNull(simpleList1.properties().get("leaf-x").get("xml"));

        final var simpleAbc = schemas.get("module_root_simple-root_abc");
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", simpleAbc.xml().get("namespace").asText());
        assertNull(simpleAbc.properties().get("leaf-abc").get("xml"));

        final var simple = schemas.get("module_root_simple-root");
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", simple.properties().get("leaf-y")
        ...
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", topList1.xml().get("namespace").asText());
        assertNull(topList1.properties().get("leaf-x").get("xml"));

        final var topAbc = schemas.get("module_root_top-list_abc");
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", topAbc.xml().get("namespace").asText());
        assertNull(topAbc.properties().get("leaf-abc").get("xml"));

        final var top = schemas.get("module_root_top-list");
        assertEquals("urn:ietf:params:xml:ns:yang:test:augmentation", top.properties().get("leaf-y")
                .get("xml").get("namespace").asText());
