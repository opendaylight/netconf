window.onload = function () {
  ...
  xmlHttp.open('GET', document.URL.split('/openapi')[0] + `/openapi/api/v3/mounts/?depth=${depthValue}&width=${widthValue}`, false);
  xmlHttp.send(null);

  var base_url_rfc = document.URL.split('/openapi')[0] + `/openapi/api/v3/mounts/?depth=${depthValue}&width=${widthValue}`;
  var swagger_urls = [{ url: document.URL.split('/openapi')[0] + `/openapi/api/v3/single?depth=${depthValue}&width=${widthValue}`, name: 'Controller resources - RestConf RFC 8040' }];
  ...
  const ui = SwaggerUIBundle({
    ...
    onComplete: function (swaggerApi, swaggerUi) {
      ...
      selectDepthWrapper.classList.add('select-wrapper');
      ...
      depthInput.addEventListener('input', setTimeout(() => window.location.reload(), 100));
      ...
      selectWidthWrapper.classList.add('select-wrapper');
      ...
      widthInput.addEventListener('input', setTimeout(() => window.location.reload(), 100));
