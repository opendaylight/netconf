...
public final class PaginationService {
     ...
     *
     * @param modulesWithoutDuplications The set of modules without duplications.
     * @param offset The offset.
     * @param limit The limit.
     */
    public PaginationService(final List<Module> modulesWithoutDuplications, final int offset, final int limit) {
        metadata = new HashMap<>();
        if (limit == 0 && offset == 0) {
            this.modules = getAllModules(modulesWithoutDuplications, metadata);
        } else {
            this.modules = getPortionOfModels(modulesWithoutDuplications, offset, limit, metadata);
        }
    }
     ...
     * @param modulesWithoutDuplications The set of modules without duplications.
     */
    public PaginationService(final @NonNull List<Module> modulesWithoutDuplications) {
        metadata = new HashMap<>();
        this.modules = getAllModules(modulesWithoutDuplications, metadata);
    }

    private static Collection<? extends Module> getPortionOfModels(final List<Module> modulesWithoutDuplications,
    ...
    private static Collection<? extends Module> getAllModules(final List<Module> modulesWithoutDuplications,
