...
public final class SchemasEntity extends OpenApiEntity {
    ...
    public void generate(final JsonGenerator generator) throws IOException {
        ...
        for (var module : modules) {
            generateComponents(generator, module);
        }

        generator.writeEndObject();
    }

    private void generateComponents(final JsonGenerator generator, final Module module) throws IOException {
        ...
        for (final var rpc : module.getRpcs()) {
            ...
            if (!rpcInput.getChildNodes().isEmpty()) {
                new RpcSchemaEntity(rpcInput, moduleName + "_" + rpcName + INPUT_SUFFIX, null, OBJECT_TYPE, stack,
                    moduleName, false, definitionNames, width, depth, 0).generate(generator);

                stack.enterSchemaTree(rpcInput.getQName());
                for (final var child : rpcInput.getChildNodes()) {
                    if (!children.contains(child)) {
                        children.add(child);
                        processDataAndActionNodes(generator, child, moduleName, stack, definitionNames, moduleName,
                            false, 0);
                    }
                }
                stack.exit();
            ...
            if (!rpcOutput.getChildNodes().isEmpty()) {
                new RpcSchemaEntity(rpcOutput, moduleName + "_" + rpcName + OUTPUT_SUFFIX, null, OBJECT_TYPE, stack,
                    moduleName, false, definitionNames, width, depth, 0).generate(generator);
        ...
        for (final var childNode : widthList(module, width)) {
            processDataAndActionNodes(generator, childNode, moduleName, stack, definitionNames, moduleName, true, 0);
        }
    }

    private void processDataAndActionNodes(final JsonGenerator generator, final DataSchemaNode node, final String title,
            final SchemaInferenceStack stack, final DefinitionNames definitionNames, final String parentName,
        ...
        if (node instanceof ContainerSchemaNode || node instanceof ListSchemaNode) {
            ...
            new NodeSchemaEntity(node, newTitle, definitionNames.pickDiscriminator(node, List.of(newTitle)),
                OBJECT_TYPE, stack, parentName, isParentConfig, definitionNames, width, depth, nodeDepth + 1)
                    .generate(generator);

            final var isConfig = node.isConfiguration() && isParentConfig;

            stack.enterSchemaTree(node.getQName());
            processActions(generator, node, newTitle, stack, definitionNames, parentName, 0);
            final var childNodes = widthList((DataNodeContainer) node, width);
            for (final var childNode : childNodes) {
                processDataAndActionNodes(generator, childNode, newTitle, stack, definitionNames, newTitle, isConfig,
                    nodeDepth + 1);
            }
            stack.exit();
            ...
            for (final var childNode : childNodes) {
                processDataAndActionNodes(generator, childNode, title, stack, definitionNames, parentName,
                    isParentConfig, nodeDepth + 1);
            }
    ...
    private void processActions(final JsonGenerator generator, final DataSchemaNode node, final String title,
            final SchemaInferenceStack stack, final DefinitionNames definitionNames, final String parentName,
        ...
        for (final var actionDef : ((ActionNodeContainer) node).getActions()) {
            ...
            if (!actionInput.getChildNodes().isEmpty()) {
                new RpcSchemaEntity(actionInput, title + "_" + actionName + INPUT_SUFFIX, null, OBJECT_TYPE, stack,
                    parentName, false, definitionNames, width, depth, nodeDepth + 1).generate(generator);
            }

            final var actionOutput = actionDef.getOutput();
            if (!actionOutput.getChildNodes().isEmpty()) {
                new RpcSchemaEntity(actionOutput, title + "_" + actionName + OUTPUT_SUFFIX, null, OBJECT_TYPE, stack,
                    parentName, false, definitionNames, width, depth, nodeDepth + 1).generate(generator);
            }
            stack.exit();
        }
