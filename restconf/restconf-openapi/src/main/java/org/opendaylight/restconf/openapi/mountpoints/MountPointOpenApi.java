...
public class MountPointOpenApi implements DOMMountPointListener, AutoCloseable {
    ...
    public OpenApiInputStream getMountPointApi(final UriInfo uriInfo, final Long id, final @Nullable String strPageNum)
        ...
        var modules = openApiGenerator.getModulesWithoutDuplications(context);
        if (strPageNum != null) {
    ...
    private OpenApiInputStream generateDataStoreOpenApi(final EffectiveModelContext modelContext,
        ...
        final var url = schema + "://" + host + "/";
        final var basePath = openApiGenerator.getBasePath();
        final var modules = openApiGenerator.getModulesWithoutDuplications(modelContext);
    ...
    private static List<Module> filterByRange(final List<Module> unequallyModules, final Integer start) {
        final var sortedModules = new TreeSet<>(unequallyModules);
        final int end = start + DEFAULT_PAGESIZE - 1;
        var firstModule = sortedModules.first();

        final var iterator = unequallyModules.iterator();
        int counter = 0;
        ...
        return List.copyOf(iterator.hasNext()
