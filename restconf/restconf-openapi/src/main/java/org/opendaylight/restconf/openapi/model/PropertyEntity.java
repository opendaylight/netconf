...
public class PropertyEntity {
    ...
    public PropertyEntity(final DataSchemaNode node, final JsonGenerator generator, final SchemaInferenceStack stack,
            ...
            DefinitionNames definitionNames) throws IOException {
        ...
        if (leafTypeDef instanceof BinaryTypeDefinition binaryType) {
            ...
            if (leafTypeDef.getDefaultValue().isPresent()) {
                generator.writeBooleanField(DEFAULT,
                    Boolean.parseBoolean((String) leafTypeDef.getDefaultValue().orElseThrow()));
            }
    ...
    private String processBinaryType(final BinaryTypeDefinition definition) throws IOException {
        if (definition.getDefaultValue().isPresent()){
            generator.writeStringField(DEFAULT, definition.getDefaultValue().toString());
        }
    ...
    private String processEnumType(final EnumTypeDefinition enumLeafType) throws IOException {
        ...
        if (enumLeafType.getDefaultValue().isPresent()) {
            ...
            generator.writeStringField(EXAMPLE, defaultValue);
        } else {
            generator.writeStringField(EXAMPLE, enumLeafType.getValues().iterator().next().getName());
        }
        return STRING_TYPE;
    }
    ...
    private String processStringType(final StringTypeDefinition stringType, final String nodeName) throws IOException {
        ...
        if (stringType.getDefaultValue().isPresent()) {
            generator.writeStringField(DEFAULT, stringType.getDefaultValue().orElseThrow().toString());
        }
        return STRING_TYPE;
    }
    ...
    private String processNumberType(final RangeRestrictedTypeDefinition<?, ?> leafTypeDef) throws IOException {
        ...
        if (leafTypeDef instanceof DecimalTypeDefinition) {
            if (leafTypeDef.getDefaultValue().isPresent()) {
                generator.writeNumberField(DEFAULT,
                    Decimal64.valueOf(leafTypeDef.getDefaultValue().orElseThrow().toString()).decimalValue());
            }
            if (maybeLower.isPresent()) {
                generator.writeNumberField(EXAMPLE, ((Decimal64) maybeLower.orElseThrow()).decimalValue());
            }
            return NUMBER_TYPE;
        }
        if (leafTypeDef instanceof Uint8TypeDefinition
            ...
            if (leafTypeDef.getDefaultValue().isPresent()) {
                generator.writeNumberField(DEFAULT,
                    Integer.parseInt(leafTypeDef.getDefaultValue().orElseThrow().toString()));
            }
            if (maybeLower.isPresent()) {
                generator.writeNumberField(EXAMPLE, Integer.parseInt(maybeLower.orElseThrow().toString()));
            }
            ...
            if (leafTypeDef.getDefaultValue().isPresent()) {
                generator.writeNumberField(DEFAULT,
                    Long.parseLong(leafTypeDef.getDefaultValue().orElseThrow().toString()));
            }
            if (maybeLower.isPresent()) {
                generator.writeNumberField(EXAMPLE, Long.parseLong(maybeLower.orElseThrow().toString()));
            }
        } else {
            //uint64
            if (leafTypeDef.getDefaultValue().isPresent()) {
                generator.writeNumberField(DEFAULT,
                    new BigInteger(leafTypeDef.getDefaultValue().orElseThrow().toString()));
            }
    ...
    private String processInstanceIdentifierType(final InstanceIdentifierTypeDefinition iidType,
        ...
        if (iidType.getDefaultValue().isPresent()) {
            generator.writeStringField(DEFAULT, iidType.getDefaultValue().orElseThrow().toString());
        }
        return STRING_TYPE;
    }
}
