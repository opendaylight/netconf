...
public class PropertyEntity {
    ...
    private void generate(final SchemaInferenceStack stack, final boolean isParentConfig) throws IOException {
        if (node instanceof ChoiceSchemaNode choice) {
            ...
            final var isConfig = isParentConfig && node.effectiveConfig().isPresent() ? node.effectiveConfig().get() : true;
    ...
    private void processChildNode(final DataSchemaNode node, final SchemaInferenceStack stack,
        ...
        final var name = node.getQName().getLocalName();

        final var shouldBeAddedAsChild = !isParentConfig || node.effectiveConfig().isPresent() ? node.effectiveConfig().get() : true;
        if (node instanceof ListSchemaNode || node instanceof ContainerSchemaNode) {
            processDataNodeContainer((DataNodeContainer) node, parentName);
            if (shouldBeAddedAsChild && isSchemaNodeMandatory(node)) {
                required.add(name);
            }
    ...
    private void processLeafNode(final LeafSchemaNode leafNode, final String jsonLeafName, final List<String> required,
    ...
    private String processIdentityRefType(final IdentityrefTypeDefinition leafTypeDef,
        ...
        final var node = leafTypeDef.getIdentities().iterator().next();
        generator.writeStringField(EXAMPLE, node.getQName().getLocalName());

        final var derivedIds = schemaContext.getDerivedIdentities(node);
