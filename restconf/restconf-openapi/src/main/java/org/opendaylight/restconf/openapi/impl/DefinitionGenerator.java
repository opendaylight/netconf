...
public final class DefinitionGenerator {
    ...
    private static List<Map<String, Object>> createExamples(final ListSchemaNode node) {
        ...
        for (int i = 0; i < minElements; i++) {
            ...
            for (final var example : childNodesMap.entrySet()) {
                final var name = example.getKey();
                var exampleValue = keys.contains(name) ? editExample(example.getValue(), i) : example.getValue();
                exampleMap.put(name.getLocalName(), exampleValue);
            }
            examples.add(exampleMap);
        }
        if (!unique.isEmpty()) {
            final var alreadyUnique = new ArrayList<>(keys);
            for (final var uniqueSet : unique) {
                final var uniqueNames = uniqueSet.argument()
                    .stream().map(SchemaNodeIdentifier::firstNodeIdentifier).toList();
                if (Collections.disjoint(uniqueNames, alreadyUnique)) {
                    final var exampleToEdit = uniqueNames.get(0);
                    final var localName = exampleToEdit.getLocalName();
                    for (int i = 0; i < examples.size(); i++) {
                        final var example = examples.get(i);
                        example.put(localName,
                            editExample(example.get(localName), i));
                    }
