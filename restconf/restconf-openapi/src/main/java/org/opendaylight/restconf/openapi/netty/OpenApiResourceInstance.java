...
final class OpenApiResourceInstance extends WebHostResourceInstance {
    ...
    private RequestResponse prepareApi(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
        ...
        return switch (next) {
            ...
            default -> peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND : switch (method) {
                case GET -> prepareModule(targetUri, headers, next, true);
                case HEAD -> prepareModule(targetUri, headers, next, false);
                case OPTIONS -> GHO_OK;
    ...
    private RequestResponse prepareModule(final URI targetUri, final HttpHeaders headers, final String module,
    ...
    private RequestResponse prepareMounts(final ImplementedMethod method, final URI targetUri,
        ...
        return peeler.hasNext() ? prepareMountsInstance(method, targetUri, headers, peeler, restconf)
            : switch (method) {
    ...
    private RequestResponse prepareMountsInstance(final ImplementedMethod method, final URI targetUri,
            final HttpHeaders headers, final SegmentPeeler peeler, final Link restconf) {
        ...
        if (!peeler.hasNext()) {
            return switch (method) {
                case GET -> prepareMountsInstanceGet(targetUri, headers, restconf, instance, true);
                case HEAD -> prepareMountsInstanceGet(targetUri, headers, restconf, instance, false);
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
        }
        ...
        return peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND : switch (next) {
            case "meta" -> prepareMountsInstanceMeta(method, targetUri, headers, restconf, instance);
            default -> prepareMountsInstanceModule(method, targetUri, headers, restconf, instance, next);
        };
    }

    private RequestResponse prepareMountsInstanceGet(final URI targetUri, final HttpHeaders headers,
            final Link restconf, final long instance, final boolean withContent) {
        final var params = queryParams(targetUri);
        final var width = params.lookup("width", Integer::valueOf);
        final var depth = params.lookup("depth", Integer::valueOf);
    ...
    private RequestResponse prepareMountsInstanceMeta(final ImplementedMethod method, final URI targetUri,
            final HttpHeaders headers, final Link restconf, final long instance) {
        return switch (method) {
            case GET -> prepareMountsInstanceMetaGet(targetUri, headers, restconf, instance, true);
            case HEAD -> prepareMountsInstanceMetaGet(targetUri, headers, restconf, instance, false);
            case OPTIONS -> GHO_OK;
            default -> GHO_METHOD_NOT_ALLOWED;
        };
    }
    ...
    private RequestResponse prepareMountsInstanceModule(final ImplementedMethod method, final URI targetUri,
            final HttpHeaders headers, final Link restconf, final long instance, final String module) {
        return switch (method) {
            case GET -> prepareMountsInstanceModuleGet(targetUri, headers, restconf, instance, module, true);
            case HEAD -> prepareMountsInstanceModuleGet(targetUri, headers, restconf, instance, module, false);
            case OPTIONS -> GHO_OK;
            default -> GHO_METHOD_NOT_ALLOWED;
        };
    }
    ...
    private RequestResponse prepareMountsInstanceModuleGet(final URI targetUri, final HttpHeaders headers,
            final Link restconf, final long instance, final String module, final boolean withContent) {
        final var params = queryParams(targetUri);
        final var revision = params.lookup("revision");
        final var width = params.lookup("width", Integer::valueOf);
        final var depth = params.lookup("depth", Integer::valueOf);
    ...
    private RequestResponse prepareSingle(final ImplementedMethod method, final URI targetUri,
        ...
        return !peeler.next().equals("meta") || peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND
            : switch (method) {
                case GET ->  prepareSingleMeta(targetUri, headers, true);
                case HEAD ->  prepareSingleMeta(targetUri, headers, true);
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
    }

    private RequestResponse prepareSingleMeta(final URI targetUri, final HttpHeaders headers,
            final boolean withContent) {
        final var params = queryParams(targetUri);
        final var offset = params.lookup("offset", Integer::valueOf);
        final var limit = params.lookup("limit", Integer::valueOf);
