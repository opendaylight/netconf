...
final class OpenApiResourceInstance extends WebHostResourceInstance {
    ...
    public RequestResponse prepare(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
        ...
        return switch (segment) {
            case "api" -> api(method, targetUri, headers, peeler, xrd);
            case "explorer" -> explorer(method, targetUri, headers, peeler);
            case "ui" -> ui(method, peeler);
            default -> EmptyRequestResponse.NOT_FOUND;
        };
    }

    // the /api resource
    private RequestResponse api(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
            final SegmentPeeler peeler, final XRD xrd) {
        ...
        return switch (next) {
            case "mounts" -> peeler.hasNext() ? apiMounts(method, targetUri, headers, peeler) : switch (method) {
                case GET -> new EntityRequestResponse(service.getListOfMounts());
                case HEAD -> JSON_OK;
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
            case "single" -> single(method, targetUri, headers, peeler);
            ...
            default -> peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND : switch (method) {
                case GET -> apiModule(targetUri, headers, next, true);
                case HEAD -> apiModule(targetUri, headers, next, false);
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
        };
    ...
    private RequestResponse apiModule(final URI targetUri, final HttpHeaders headers, final String module,
    ...
    private RequestResponse apiMounts(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
        ...
        if (!peeler.hasNext()) {
            return switch (method) {
                case GET -> apiMount(targetUri, headers, instance, true);
                case HEAD -> apiMount(targetUri, headers, instance, false);
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
        }
        ...
        return peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND : switch (next) {
            case "meta" -> prepareMountsInstanceMeta(method, targetUri, headers, instance);
            default -> prepareMountsInstanceModule(method, targetUri, headers, instance, next);
        };
    }

    private RequestResponse apiMount(final URI targetUri, final HttpHeaders headers, final long instance,
            final boolean withContent) {
        final var params = queryParams(targetUri);
        final var width = params.lookup("width", Integer::valueOf);
        final var depth = params.lookup("depth", Integer::valueOf);
    ...
    private RequestResponse prepareMountsInstanceMeta(final ImplementedMethod method, final URI targetUri,
            final HttpHeaders headers, final long instance) {
        return switch (method) {
            case GET -> prepareMountsInstanceMetaGet(targetUri, headers, instance, true);
            case HEAD -> prepareMountsInstanceMetaGet(targetUri, headers, instance, false);
            case OPTIONS -> GHO_OK;
            default -> GHO_METHOD_NOT_ALLOWED;
        };
    }

    private RequestResponse prepareMountsInstanceMetaGet(final URI targetUri, final HttpHeaders headers,
            final long instance, final boolean withContent) {
        final var params = queryParams(targetUri);
        final var offset = params.lookup("offset", Integer::valueOf);
        final var limit = params.lookup("limit", Integer::valueOf);
    ...
    private RequestResponse prepareMountsInstanceModule(final ImplementedMethod method, final URI targetUri,
            final HttpHeaders headers, final long instance, final String module) {
        return switch (method) {
            case GET -> prepareMountsInstanceModuleGet(targetUri, headers, instance, module, true);
            case HEAD -> prepareMountsInstanceModuleGet(targetUri, headers, instance, module, false);
            case OPTIONS -> GHO_OK;
            default -> GHO_METHOD_NOT_ALLOWED;
        };
    }

    private RequestResponse prepareMountsInstanceModuleGet(final URI targetUri, final HttpHeaders headers,
            final long instance, final String module, final boolean withContent) {
        final var params = queryParams(targetUri);
        final var revision = params.lookup("revision");
        final var width = params.lookup("width", Integer::valueOf);
        final var depth = params.lookup("depth", Integer::valueOf);
    ...
    private RequestResponse single(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
            final SegmentPeeler peeler) {
        if (!peeler.hasNext()) {
            return switch (method) {
                case GET -> single(targetUri, headers, true);
                case HEAD -> single(targetUri, headers, false);
                case OPTIONS -> GHO_OK;
                default -> GHO_METHOD_NOT_ALLOWED;
            };
        }
        return !peeler.next().equals("meta") || peeler.hasNext() ? EmptyRequestResponse.NOT_FOUND : switch (method) {
            case GET -> singleMeta(targetUri, headers, true);
            case HEAD -> singleMeta(targetUri, headers, false);
            case OPTIONS -> GHO_OK;
            default -> GHO_METHOD_NOT_ALLOWED;
        };
    }

    private RequestResponse single(final URI targetUri, final HttpHeaders headers, final boolean withContent) {
        final var params = queryParams(targetUri);
        final var width = params.lookup("width", Integer::valueOf);
        final var depth = params.lookup("depth", Integer::valueOf);
        final var offset = params.lookup("offset", Integer::valueOf);
    ...
    private RequestResponse singleMeta(final URI targetUri, final HttpHeaders headers,
            final boolean withContent) {
        final var params = queryParams(targetUri);
        final var offset = params.lookup("offset", Integer::valueOf);
        final var limit = params.lookup("limit", Integer::valueOf);
    ...
    private static RequestResponse explorer(final ImplementedMethod method, final URI targetUri,
    ...
    // the /ui resource.
    private static RequestResponse ui(final ImplementedMethod method, final SegmentPeeler peeler) {
        return switch (method) {
