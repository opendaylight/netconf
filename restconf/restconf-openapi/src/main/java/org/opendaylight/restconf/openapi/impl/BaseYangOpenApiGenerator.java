...
public abstract class BaseYangOpenApiGenerator {
    ...
    public OpenApiObject getControllerModulesDoc(final UriInfo uriInfo, final DefinitionNames definitionNames) {
        ...
        getSortedModules(context).stream()
            .filter(module -> getListOrContainerConfigChildNode(module) != null)
            .findFirst()
    ...
    public static void addRootPostLink(final Module module, final String deviceName,
        ...
        final var childNode = getListOrContainerConfigChildNode(module);
    ...
    public static <T extends DataNodeContainer> DataSchemaNode getListOrContainerConfigChildNode(final T node) {
        return node.getChildNodes().stream()
