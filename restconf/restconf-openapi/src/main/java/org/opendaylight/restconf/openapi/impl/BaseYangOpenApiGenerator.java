...
public abstract class BaseYangOpenApiGenerator {
    ...
    public Collection<? extends Module> getPortionOfModels(final EffectiveModelContext context,
        final Integer offset, final Integer limit) {
            if (!isForAllModules(offset, limit)) {
                final var modules = modulesList(context);
                final var start = offset < 0 ? 0 : offset;
                final var end = limit <= 0 ? modules.size() : Math.min(modules.size(), start + limit);
                return modules.subList(start, end);
            }
        return context.getModules();
    }

    private static List<Module> modulesList(final EffectiveModelContext context) {
