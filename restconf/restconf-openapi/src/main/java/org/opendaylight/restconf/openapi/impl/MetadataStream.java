...
public final class MetadataStream extends InputStream {
    ...
    public MetadataStream(final Integer offset, final Integer limit, final long allModules, final long configModules)
            throws IOException {
        this.writer = new OpenApiBodyWriter(generator, stream);
        this.offset = requireNonNullElse(offset, 0);
        this.limit = requireNonNullElse(limit, 0);
    ...
    private static Map<String, ?> metadataForPagedModules(final Integer offset, final Integer limit,
            final long configModules, final long allModules) {
        return Map.of(
            TOTAL_MODULES, allModules,
            CONFIG_MODULES, configModules,
            NON_CONFIG_MODULES, allModules - configModules,
