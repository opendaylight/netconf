...
public final class SchemasStream extends InputStream {
    ...
    private Deque<SchemaEntity> toComponents(final Module module) {
        ...
        for (final var childNode : module.getChildNodes()) {
            processDataNodeContainers(childNode, moduleName, stack, definitionNames, result, moduleName);
    ...
    private static void processDataNodeContainers(final DataSchemaNode node, final String title,
        ...
        if (node instanceof ContainerSchemaNode || node instanceof ListSchemaNode) {
            ...
            final var child = new SchemaEntity(node, newTitle, OBJECT_TYPE, stack, parentName, true,
                definitionNames);
            result.add(child);
            stack.enterSchemaTree(node.getQName());
            for (final var childNode : ((DataNodeContainer) node).getChildNodes()) {
                processDataNodeContainers(childNode, newTitle, stack, definitionNames, result, newTitle);
            }
            stack.exit();
            ...
            final var caseTitle = parentName + "_" + caseNode.getQName().getLocalName();
            final var caseEntity = new SchemaEntity(caseNode, caseTitle,
                OBJECT_TYPE, stack, parentName, true, definitionNames);
            result.add(caseEntity);
            stack.enterSchemaTree(caseNode.getQName());
            for (final var childNode : caseNode.getChildNodes()) {
                processDataNodeContainers(childNode, caseTitle, stack, definitionNames, result, caseTitle);
            }
