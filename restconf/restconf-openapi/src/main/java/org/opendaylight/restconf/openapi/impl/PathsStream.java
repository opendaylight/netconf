...
public final class PathsStream extends InputStream {
        ...
        if (!isApplied(depth)) {
            if (!isApplied(width)) { // width and depth are ignored - creating full schema
                ...
                for (int i = 0; i < limit; i++) {
                    // Creating paths for exact amount on nodes as applied width
                    processChildren(module, result, (DataSchemaNode) childrenList[i], 0);
                }
            }
        } else {
            if (!isApplied(width)) { // width is ignored, depth is applied
                ...
                for (final var node : module.getChildNodes()) {
                    ...
                    if (!tooDeep && childrenCounter < width) {
                        // If child isn't too deep, and we have less children than applied width - adding it
                        result.addAll(nodeWithChildren);
                        childrenCounter++;
    ...
    private boolean processChildNode(final DataSchemaNode node, final List<ParameterEntity> pathParams,
        ...
        return false; // Node not deep enough to reject it
    }
