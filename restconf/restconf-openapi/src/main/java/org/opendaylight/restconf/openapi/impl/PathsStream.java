        ...
        for (final var node : module.getChildNodes()) {
            ...
            if (node instanceof ListSchemaNode || node instanceof ContainerSchemaNode) {
                if (isConfig && !hasRootPostLink && isForSingleModule) {
                    ...
                    if (getListOrContainerChildNode(((DataNodeContainer) node)) != null) {
                        result.add(new PathEntity(resolvedPath, new PostEntity(node, deviceName, moduleName, pathParams,
                            nodeLocalName, module), null, null, null, null));
