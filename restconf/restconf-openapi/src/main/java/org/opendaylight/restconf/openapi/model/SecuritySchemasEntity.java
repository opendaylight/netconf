...
public final class SecuritySchemasEntity extends OpenApiEntity {
    private final Map<String, SecuritySchemeObject> schemes;

    public SecuritySchemasEntity(final Map<String, SecuritySchemeObject> schemes) {
    ...
    public void generate(@NonNull final JsonGenerator generator) throws IOException {
        ...
        for (final var entry : schemes.entrySet()) {
            ...
            if (entry.getValue() instanceof Http http) {
                generator.writeStringField("schema", http.scheme());
