...
public final class OpenApiDataStream extends InputStream {
    ...
    public OpenApiDataStream(final EffectiveModelContext context, final String title, final String url,
            ...
            final String basePath, final Map<String, Object> mappedMetadata) throws IOException {
        final OpenApiBodyWriter writer = new OpenApiBodyWriter(generator, stream);
        stack.add(new OpenApiInputStream(context, title, url, security, deviceName, urlPrefix, isForSingleModule,
            includeDataStore, modules, basePath, writer, stream, generator));
        stack.add(new MetadataStream(mappedMetadata, writer));
    }
