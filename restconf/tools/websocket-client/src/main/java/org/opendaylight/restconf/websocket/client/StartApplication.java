...
public final class StartApplication {
    ...
    public static void main(String[] args) {
        final ApplicationSettings applicationSettings = ApplicationSettings.parseApplicationSettings(args);
    ...
    private static void printHandledStreamsOverview(final List<String> streams,
        ...
        LOG.info("Successfully created stream handlers ({}): {}.", successfullyHandledStreams.size(),
                successfullyHandledStreams);
        LOG.info("Unsuccessfully handled streams ({}): {}.", unsuccessfullyHandledStreams.size(),
     ...
     * @return Instance of {@link WebSocketClientHandler} or {@link WebSocketTlsClientHandler} in case of WSS.
     */
    private static WebSocketClientHandler getWebSocketClientHandler(final ApplicationSettings applicationSettings,
            final SslContextFactory sslContextFactory, final ScheduledExecutorService scheduledExecutorService,
        ...
        try {
            ...
            if (uri.getScheme() == null) {
                ...
                switch (uri.getScheme().toLowerCase(Locale.getDefault())) {
                    case WS_SCHEME:
                        return new WebSocketClientHandler(uri, applicationSettings.getPingInterval(),
                                applicationSettings.getPingMessage(), scheduledExecutorService);
                    case WSS_SCHEME:
                        return new WebSocketTlsClientHandler(uri, applicationSettings.getPingInterval(),
                                applicationSettings.getPingMessage(), scheduledExecutorService, sslContextFactory);
                    default:
