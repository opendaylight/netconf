...
public class CapabilitiesWriterTest {
    ...
    public void restconfStateCapabilitesTest() {
        ...
        assertEquals(CapabilitiesWriter.CAPABILITY, capability.getIdentifier());

        assertThat(capability.body().stream().map(LeafSetEntryNode::body).collect(Collectors.toUnmodifiableList()),
            containsInAnyOrder(
