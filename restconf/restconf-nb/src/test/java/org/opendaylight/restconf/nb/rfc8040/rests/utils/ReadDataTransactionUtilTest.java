...
public class ReadDataTransactionUtilTest {
    ...
    public void readLeafWithDefaultParameters() throws Exception {
        ...
        final ContainerNode content = Builders.containerBuilder()
                ...
                .withChild(leafNode)
                .build();
        EffectiveModelContext mySchemaContext = YangParserTestUtils.parseYangFiles(
                TestRestconfUtils.loadFiles("/modules"));

        final YangInstanceIdentifier path = YangInstanceIdentifier.builder().node(QName.create(DATA.base, "cont")
                ).build();
        ...
        doReturn(immediateFluentFuture(Optional.of(content))).when(netconfService).getConfig(path);
        doReturn(immediateFluentFuture(Optional.of(content))).when(netconfService).get(path);

        NormalizedNode normalizedNode = ReadDataTransactionUtil.readData(
                ContentParam.ALL, path, mdsalStrategy, WithDefaultsParam.TRIM, mySchemaContext);
        assertEquals(content, normalizedNode);

        normalizedNode =  ReadDataTransactionUtil.readData(
                ContentParam.ALL, path, netconfStrategy, WithDefaultsParam.TRIM, mySchemaContext);
        assertEquals(content, normalizedNode);
    }
    ...
    public void readContainerWithDefaultParameters() throws Exception {
        ...
        assertEquals(((MapNode) ((ContainerNode) normalizedNode).getChildByArg(
                        NodeIdentifier.create(QName.create(DATA.base, "exampleList")))).getChildByArg(
                        YangInstanceIdentifier.NodeIdentifierWithPredicates.of(QName.create(DATA.base, "exampleList"))),
                ((MapNode) content.getChildByArg(
                        NodeIdentifier.create(QName.create(DATA.base, "exampleList")))).childByArg(
                        YangInstanceIdentifier.NodeIdentifierWithPredicates.of(
                                QName.create(DATA.base, "exampleList"))));
