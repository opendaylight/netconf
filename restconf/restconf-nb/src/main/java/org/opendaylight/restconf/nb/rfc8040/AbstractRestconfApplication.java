...
abstract class AbstractRestconfApplication extends Application {
    ...
    AbstractRestconfApplication(final DatabindContextProvider databindContextProvider,
    ...
    public final Set<Object> getSingletons() {
        return ImmutableSet.<Object>builderWithExpectedSize(services.size() + 5)
            .addAll(services)
            .add(new JsonNormalizedNodeBodyReader(databindContextProvider, mountPointService))
            .add(new JsonPatchBodyReader(databindContextProvider, mountPointService))
            .add(new XmlNormalizedNodeBodyReader(databindContextProvider, mountPointService))
            .add(new XmlPatchBodyReader(databindContextProvider, mountPointService))
            .add(new RestconfDocumentedExceptionMapper(databindContextProvider))
            .build();
    }
}
