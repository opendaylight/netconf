...
public final class JaxRsRestconf implements ParamConverterProvider {
    ...
    @Path("/data")
    @SuppressWarnings("checkstyle:abbreviationAsWordInName")
    public void dataOPTIONS(@Context final UriInfo uriInfo, @Suspended final AsyncResponse ar) {
        completeOptions(server.dataOPTIONS(requestOf(uriInfo)), ar);
    }
    ...
    public void dataOPTIONS(@Encoded @PathParam("identifier") final ApiPath identifier, @Context final UriInfo uriInfo,
            @Suspended final AsyncResponse ar) {
        completeOptions(server.dataOPTIONS(requestOf(uriInfo), identifier), ar);
    }

    private static void completeOptions(final RestconfFuture<OptionsResult> future, final AsyncResponse ar) {
        future.addCallback(new JaxRsRestconfCallback<>(ar) {
    ...
    public void operationsOPTIONS(@Encoded @PathParam("operation") final ApiPath operation,
            @Context final UriInfo uriInfo, @Suspended final AsyncResponse ar) {
        completeOptions(server.operationsOPTIONS(requestOf(uriInfo), operation), ar);
    }
