        ...
        try (var writer = new StringWriter();
            ...
            for (final RestconfError error : exception.getErrors()) {
                ...
                if (error.getErrorPath() != null) {
                    ...
                    // Set NormalizedNodeStreamWriterStack tracker to "error-path" qName.
                    final var stack = SchemaInferenceStack.of(databindProvider.currentContext().modelContext());
                    stack.enterGrouping(
                        org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.restconf.rev170126.Errors.QNAME);
                    stack.enterSchemaTree(Errors.QNAME);
                    stack.enterSchemaTree(Error.QNAME);
                    final var nnStreamWriterStack = NormalizedNodeStreamWriterStack.of(stack.toInference());
                    nnStreamWriterStack.startLeafNode(NodeIdentifier.create(ERROR_PATH_QNAME));

                    // Create Nested writer with tracker pointing to "error-path" qName leaf
                    final var nodeNamespace = error.getErrorPath().getLastPathArgument().getNodeType().getNamespace();
                    final var nestedWriter = JSONNormalizedNodeStreamWriter.createNestedWriter(
                        currentDatabindContext.jsonCodecs(), nnStreamWriterStack, nodeNamespace, jsonWriter);

                    //Emit YangInstanceIdentifier to "error-path" value
