...
final class RootFormattableBody extends NormalizedFormattableBody<ContainerNode> {
    ...
    @Override
    protected void formatToJSON(final DatabindContext databind, final ContainerNode data, final JsonWriter writer)
            throws IOException {
        writer.beginObject();

        final var nnWriter = newWriter(JSONNormalizedNodeStreamWriter.createNestedWriter(databind.jsonCodecs(),
            writer, null));
    ...
    @Override
    protected void formatToXML(final DatabindContext databind, final ContainerNode data, final XMLStreamWriter writer)
        ...
        if (!data.isEmpty()) {
            final var nnWriter = newWriter(
                XMLStreamNormalizedNodeStreamWriter.create(writer, databind.modelContext()));
            nnWriter.write(data);
            nnWriter.flush();
        }
