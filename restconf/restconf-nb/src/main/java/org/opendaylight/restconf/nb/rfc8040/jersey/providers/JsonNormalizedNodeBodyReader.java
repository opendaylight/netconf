...
import org.opendaylight.yangtools.yang.data.util.DataSchemaContextTree.NodeAndStack;
...
public class JsonNormalizedNodeBodyReader extends AbstractNormalizedNodeBodyReader {
    ...
    public static NormalizedNodePayload readFrom(final InstanceIdentifierContext path, final InputStream entityStream)
        ...
        final var yangInstanceIdentifier = YangInstanceIdentifier.create(pathArguments);
        final var nodeAndStack = DataSchemaContextTree.from(path.getSchemaContext())
            .enterPath(yangInstanceIdentifier).orElseThrow();

        final var jsonParser = JsonParserStream.create(writer, jsonCodecFactory, nodeAndStack.stack().toInference());
