...
public sealed interface Insert {
    ...
    static @Nullable Insert forParams(final @Nullable InsertParam insert, final @Nullable PointParam point) {
        ...
        return switch (insert) {
            case BEFORE -> new BeforeOrAfter(true, checkNotNull(point));
            case AFTER -> new BeforeOrAfter(false, checkNotNull(point));
            case FIRST -> {
                checkNull(point);
                yield new FirstOrLast(true);
            }
            case LAST -> {
                checkNull(point);
                yield new FirstOrLast(false);
            }
        };
    }
