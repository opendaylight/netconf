...
public final class OSGiNorthbound {
    ...
    private final ComponentFactory<ListenersBroker> listenersBrokerFactory;
    private final ComponentFactory<DefaultRestconfStreamServletFactory> servletFactoryFactory;

    private ComponentInstance<ListenersBroker> listenersBroker;
    ...
    public OSGiNorthbound(
            ...
            @Reference(target = "(component.factory=" + ListenersBroker.FACTORY_NAME + ")")
            final ComponentFactory<ListenersBroker> listenersBrokerFactory,
            final Configuration configuration) {
        this.listenersBrokerFactory = requireNonNull(listenersBrokerFactory);
        this.servletFactoryFactory = requireNonNull(servletFactoryFactory);

        useSSE = configuration.use$_$sse();
        listenersBroker = listenersBrokerFactory.newInstance(FrameworkUtil.asDictionary(ListenersBroker.props(useSSE)));

        servletProps = DefaultRestconfStreamServletFactory.props(listenersBroker.getInstance(), useSSE,
    ...
    void modified(final Configuration configuration) {
        ...
        if (newUseSSE != useSSE) {
            useSSE = newUseSSE;
            listenersBroker.dispose();
            listenersBroker = listenersBrokerFactory.newInstance(FrameworkUtil.asDictionary(
                ListenersBroker.props(useSSE)));
            LOG.debug("ListenersBroker restarted with {}", newUseSSE ? "SSE" : "Websockets");
        }

        final var newServletProps = DefaultRestconfStreamServletFactory.props(listenersBroker.getInstance(), useSSE,
            new StreamsConfiguration(configuration.maximum$_$fragment$_$length(),
                configuration.idle$_$timeout(), configuration.heartbeat$_$interval()),
            configuration.ping$_$executor$_$name$_$prefix(), configuration.max$_$thread$_$count());
