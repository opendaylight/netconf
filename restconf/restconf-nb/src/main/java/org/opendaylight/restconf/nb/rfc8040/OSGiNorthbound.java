...
import org.opendaylight.restconf.nb.rfc8040.streams.MdsalRestconfStreamRegistry;
...
public final class OSGiNorthbound {
    ...
    public OSGiNorthbound(
            ...
            final ComponentFactory<MdsalRestconfStreamRegistry> listenersBrokerFactory,
            final Configuration configuration) {
        registryFactory = requireNonNull(listenersBrokerFactory);
        this.servletFactoryFactory = requireNonNull(servletFactoryFactory);

        useSSE = configuration.use$_$sse();
        registry = listenersBrokerFactory.newInstance(
            FrameworkUtil.asDictionary(MdsalRestconfStreamRegistry.props(useSSE)));
