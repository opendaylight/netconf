...
public final class CreateStreamUtil {
     ...
     * @return {@link DOMRpcResult} - Output of RPC - example in JSON
     */
    static DOMRpcResult createDeviceNotificationListener(final String baseUrl,
        ...
        if (mountPoint != null) {
            createDeviceNotificationListener(data, pathValue, refSchemaCtx, instanceIdentifier, mountPointService,
                    restconfStrategy);
            // building of output
            final QName outputQname = QName.create(qname, "output");
            ...
            final ContainerNode output = ImmutableContainerNodeBuilder.create()
                    .withNodeIdentifier(new NodeIdentifier(outputQname))
                    .withChild(ImmutableNodes.leafNode(streamNameQname, baseUrl + pathValue)).build();
            return new DefaultDOMRpcResult(output);
        } else {
    ...
    public static void createDeviceNotificationListener(final ContainerNode data, String pathValue,
                                                 final EffectiveModelContext refSchemaCtx,
                                                 final InstanceIdentifierContext instanceIdentifier,
                                                 final DOMMountPointService mountPointService,
                                                 final RestconfStrategy restconfStrategy) {
        final NotificationOutputType outputType = data == null ? NotificationOutputType.XML : prepareOutputType(data);
        ...
        final DeviceNotificationListenerAdaptor notificationListenerAdapter = ListenersBroker.getInstance()
                .registerDeviceNotificationListener(pathValue, outputType,
                        instanceIdentifier.getSchemaContext(), refSchemaCtx);
        notificationListenerAdapter.listen(
                instanceIdentifier.getMountPoint().getService(DOMNotificationService.class).get(),
                absolutes.toArray(Absolute[]::new));
        DeviceNotificationMountPointListener.addDeviceNotificationListener(
                instanceIdentifier.getMountPoint().getIdentifier(), notificationListenerAdapter, mountPointService,
                restconfStrategy, refSchemaCtx);
    }
