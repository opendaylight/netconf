 ...
 */
@NonNullByDefault
record MdsalMountPointResolver(DOMMountPointService mountPointService) implements MountPointResolver {
    MdsalMountPointResolver {
    ...
    private static MountPointResolver mountPointResolverOf(final DOMMountPoint mountPoint) {
        return mountPoint.getService(DOMMountPointService.class)
            .<MountPointResolver>map(MdsalMountPointResolver::new)
            .orElse(mountPath -> {
