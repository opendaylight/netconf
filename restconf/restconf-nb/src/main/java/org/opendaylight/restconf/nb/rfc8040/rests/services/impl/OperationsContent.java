...
enum OperationsContent {
    ...
    public final @NonNull String bodyFor(final @NonNull InstanceIdentifierContext identifierContext) {
        ...
        final var moduleRpcs = getModuleRpcs(context, context.getModuleStatements());
        final var schemaInferenceStack = identifierContext.inference().toSchemaInferenceStack();
        if (schemaInferenceStack.isEmpty()) {
            return createBody(moduleRpcs);
        }
        final var key = prefix(schemaInferenceStack.currentModule());
