...
enum OperationsContent {
    ...
    /**
     * Return the content for a particular {@link InstanceIdentifierContext}.
     *
    ...
    public final @NonNull String bodyFor(final @NonNull InstanceIdentifierContext identifierContext) {
        ...
        final var instanceIdentifier = identifierContext.getInstanceIdentifier();
        if (instanceIdentifier == null) {
            return emptyBody;
        }

        final var rpcQname = instanceIdentifier.getLastPathArgument().getNodeType();
        final var moduleRpcs = getModuleRpcs(context, context.getModuleStatements());
        final var filteredModuleRpcs = moduleRpcs.stream()
                .filter(entry -> entry.getKey().equals(prefix(context
                        .findModuleStatements(rpcQname.getNamespace()).iterator().next())))
                .map(entry -> Map.entry(entry.getKey(), entry.getValue()
                        .stream().filter(s -> s.equals(rpcQname.getLocalName()))
                        .toList()))
                .toList();
