...
public abstract class RestconfStrategy {
    @NonNullByDefault
    public record StrategyAndInstance(RestconfStrategy strategy, Inference inference, YangInstanceIdentifier instance) {
...
//    }
    ...
    public final @NonNull StrategyAndInstance resolveStrategyPath(final ApiPath path) {
        ...
        final var result = strategy.pathNormalizer.normalizePath(andTail.tail());
        return new StrategyAndInstance(strategy, result.stack.toInference(), result.path);
    }
    ...
    public final @NonNull StrategyAndTail resolveStrategy(final ApiPath path) {
        ...
        final var normalizedPath = pathNormalizer.normalizePath(path.subPath(0, mount)).path;
