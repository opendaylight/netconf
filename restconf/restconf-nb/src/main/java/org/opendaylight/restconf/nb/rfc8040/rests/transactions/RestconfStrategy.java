...
public abstract class RestconfStrategy extends AbstractServerStrategy implements ServerDataOperations {
    ...
    RestconfStrategy(final DatabindContext databind, final ImmutableMap<QName, RpcImplementation> localRpcs,
        ...
        rpc = rpcService != null ? new MdsalServerRpcOperations(localRpcs, rpcService)
            : new LocalServerRpcOperations(localRpcs);
        action = actionService != null ? new MdsalServerActionOperations(actionService) : null;
    }

    @Override
    protected final ServerActionOperations action() {
        final var local = action;
        return local != null ? local
            : (request, path, input) -> request.completeWith(new ServerException(
                ErrorType.PROTOCOL, ErrorTag.OPERATION_NOT_SUPPORTED, "Action invocation is not available"));
    }
