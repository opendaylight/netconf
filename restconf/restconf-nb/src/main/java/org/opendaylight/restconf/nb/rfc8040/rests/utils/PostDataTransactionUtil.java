...
public final class PostDataTransactionUtil {
    ...
    private static void insertWithPointPost(final YangInstanceIdentifier path, final NormalizedNode data,
        ...
        for (final NormalizedNode nodeChild : readList.body()) {
            ...
            final YangInstanceIdentifier childPath = path.node(nodeChild.getIdentifier());
    ...
    private static void checkListDataDoesNotExist(final YangInstanceIdentifier path, final NormalizedNode data,
        ...
        if (data instanceof NormalizedNodeContainer<?> dataNode) {
            for (final var node : dataNode.body()) {
                checkItemDoesNotExists(strategy.exists(LogicalDatastoreType.CONFIGURATION,
                    path.node(node.getIdentifier())), path.node(node.getIdentifier()));
            }
        } else {
