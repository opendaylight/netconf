...
public final class RestconfDataServiceImpl {
    ...
    public Response readData(@Context final UriInfo uriInfo) {
        ...
        return readData(server.bindRoot(), readParams).getValue();
    }
    ...
    private Entry<NormalizedNode, Response> readData(final DataMode reqPath, final ReadDataParams readParams) {
    ...
    private Response putData(final @Nullable String identifier, final UriInfo uriInfo, final ResourceBody body) {
        ...
        final var req = bindDataRequest(reqPath, body);
    ...
    private void plainPatchData(final ResourceBody body, final AsyncResponse ar) {
        plainPatchData(server.bindRoot(), body, ar);
    }
     ...
     * @param ar {@link AsyncResponse} which needs to be completed
     */
    private void plainPatchData(final DataMode reqPath, final ResourceBody body, final AsyncResponse ar) {
        final var req = bindDataRequest(reqPath, body);
    ...
    private @NonNull ResourceRequest bindDataRequest(final DataMode reqPath, final ResourceBody body) {
        final var inference = reqPath.inference();
    ...
    private PatchStatusContext yangPatchData(final @NonNull PatchBody body) {
        final var modelContext = server.bindRoot().inference().getEffectiveModelContext();
