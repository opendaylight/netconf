...
public final class RestconfDataServiceImpl {
    ...
    private Response postData(final InstanceIdentifierContext iid, final ChildBody body, final UriInfo uriInfo) {
        ...
        final var payload = body.toPayload(iid.inference());
        final var data = payload.body();
        var path = iid.getInstanceIdentifier();
        final var pathSuffix = payload.prefix();
        if (!pathSuffix.isEmpty()) {
            path = path.isEmpty() ? YangInstanceIdentifier.of(pathSuffix)
                : YangInstanceIdentifier.of(Iterables.concat(path.getPathArguments(), pathSuffix));
        }
