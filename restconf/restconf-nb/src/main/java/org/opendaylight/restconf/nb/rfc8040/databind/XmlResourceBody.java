...
public final class XmlResourceBody extends ResourceBody {
    ...
    @Override
    void streamTo(final InputStream inputStream, final Inference inference, final QName qname,
        ...
        try (var xmlParser = XmlParserStream.create(writer, inference)) {
            ...
            if (!docRootName.equals(qname.getLocalName()) || !docRootNs.equals(qname.getNamespace().toString())) {
                throw new RestconfDocumentedException("Not correct message root element \"" + docRootName + "\", "
                    + "should be \"" + qname + "\"", ErrorType.PROTOCOL, ErrorTag.MALFORMED_MESSAGE);
            }
