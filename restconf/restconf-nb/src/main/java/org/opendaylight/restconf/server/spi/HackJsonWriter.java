...
final class HackJsonWriter extends JsonWriter {
    ...
    public JsonWriter value(final Number value) throws IOException {
        setValue(new Value(value.toString(), Value.Kind.NUMBER));
        return super.value(value);
    }
    ...
    public JsonWriter value(final String value) throws IOException {
        ...
        setValue(new Value(value, Value.Kind.STRING));
        return super.value(value);
    }

    @NonNull Value acquireValue() throws IOException {
    ...
    private void setValue(final Value newValue) throws IOException {
        if (value != null) {
            throw new IOException("Value already set to " + value);
        }
        value = newValue;
    }
}
