...
public abstract sealed class ResourceBody extends AbstractBody permits JsonResourceBody, XmlResourceBody {
    ...
    public @NonNull NormalizedNode toNormalizedNode(final YangInstanceIdentifier path, final Inference inference,
            final SchemaNode schemaNode) {
        final var holder = new NormalizationResultHolder();
        try (var streamWriter = ImmutableNormalizedNodeStreamWriter.from(holder)) {
            streamTo(acquireStream(), inference,
                path.isEmpty() ? Data.QNAME : path.getLastPathArgument().getNodeType(), streamWriter);
        } catch (IOException e) {
    ...
    abstract void streamTo(InputStream inputStream, Inference inference, QName qname, NormalizedNodeStreamWriter writer)
        throws IOException;
     ...
     */
    @VisibleForTesting
    static final @NonNull YangInstanceIdentifier validTopLevelNodeName(final YangInstanceIdentifier path,
            final NormalizedNode data) {
        final var payloadName = data.name();
        if (path.isEmpty()) {
            if (!Data.QNAME.equals(payloadName.getNodeType())) {
                throw new RestconfDocumentedException("Instance identifier has to contain at least one path argument",
                    ErrorType.PROTOCOL, ErrorTag.MALFORMED_MESSAGE);
            }
        } else {
            final var identifierName = path.getLastPathArgument();
