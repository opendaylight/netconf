...
public abstract class PatchBody implements AutoCloseable {
    ...
    public final @NonNull PatchContext toPatchContext(final @NonNull Inference inference,
            final @NonNull YangInstanceIdentifier targetResource) throws IOException {
        final var is = acquireStream();
        if (is == null) {
        ...
        try {
            return toPatchContext(inference, targetResource, is);
        } finally {
            is.close();
        }
    }

    abstract @NonNull PatchContext toPatchContext(@NonNull Inference inference,
        @NonNull YangInstanceIdentifier targetResource, @NonNull InputStream inputStream) throws IOException;

    static final YangInstanceIdentifier parsePatchTarget(final EffectiveModelContext schemaContext,
