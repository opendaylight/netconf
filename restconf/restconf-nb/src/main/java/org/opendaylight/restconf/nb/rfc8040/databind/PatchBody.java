...
public abstract class PatchBody implements AutoCloseable {
    ...
    public final @NonNull PatchContext toPatchContext(final @NonNull Inference inference,
            final @NonNull YangInstanceIdentifier target) throws IOException {
        final var is = acquireStream();
        if (is == null) {
        ...
        try {
            return toPatchContext(inference, target, is);
        } finally {
            is.close();
        }
    }

    abstract @NonNull PatchContext toPatchContext(@NonNull Inference inference, @NonNull YangInstanceIdentifier target,
