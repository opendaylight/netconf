...
public final class RestconfInvokeOperationsServiceImpl {
    ...
    private void invokeRpc(final String identifier, final UriInfo uriInfo, final AsyncResponse ar,
        ...
        // FIXME: bad cast
        final var reqPath = (InvokeOperationMode) server.bindPOST(identifier);
        ...
        Futures.addCallback(hackInvokeRpc(reqPath, uriInfo, input), new FutureCallback<>() {
            @Override
            public void onSuccess(final Optional<ContainerNode> result) {
                if (result.isPresent()) {
                    final var output = result.orElseThrow();
                    if (!output.isEmpty()) {
                        ar.resume(Response.ok().entity(new NormalizedNodePayload(operation, output)).build());
                        return;
                    }
                }
    ...
    private RestconfFuture<Optional<ContainerNode>> hackInvokeRpc(final InvokeOperationMode reqPath,
            final UriInfo uriInfo, final ContainerNode input) {
        final var operation = reqPath.operation();
        ...
        if (mountPoint == null) {
            ...
            if (CreateDataChangeEventSubscription.QNAME.equals(type)) {
                return RestconfFuture.of(Optional.of(CreateStreamUtil.createDataChangeNotifiStream(
                    streamUtils.listenersBroker(), input, reqPath.localDatabind().modelContext())));
