...
final class DataFormattableBody<N extends NormalizedNode> extends NormalizedFormattableBody<N> {
    ...
    @Override
    protected void formatToJSON(final DatabindContext databind, final N data, final JsonWriter writer)
            throws IOException {
        writeTo(data,
            JSONNormalizedNodeStreamWriter.createExclusiveWriter(databind.jsonCodecs(), parent, null, writer));
    }

    @Override
    protected void formatToXML(final DatabindContext databind, final N data, final XMLStreamWriter xmlWriter)
            throws IOException {
