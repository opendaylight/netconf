...
public final class WriterFieldsTranslator {
    ...
    private static DataSchemaContext addChildToResult(final DataSchemaContext currentNode, final QName childQName,
        ...
        // resolve parent node
        final var parentNode = resolveMixinNode(currentNode, level,
            NetconfFieldsTranslator.contextPathArgument(currentNode).getNodeType());
        if (parentNode == null) {
            throw new RestconfDocumentedException(
        ...
        level.add(NetconfFieldsTranslator.contextPathArgument(childNode).getNodeType());
        return childNode;
    }
