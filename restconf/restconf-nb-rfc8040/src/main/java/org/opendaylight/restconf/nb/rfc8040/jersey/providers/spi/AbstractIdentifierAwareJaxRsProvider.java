...
public abstract class AbstractIdentifierAwareJaxRsProvider<T> implements MessageBodyReader<T> {
    ...
    public final T readFrom(final Class<T> type, final Type genericType, final Annotation[] annotations,
        ...
        final String identifier = uriInfo.getPathParameters(false).getFirst("identifier");
        final ApiPath apiPath = ApiPath.valueOf(identifier == null ? "" : identifier);
        final InstanceIdentifierContext<?> path = ParserIdentifier.toInstanceIdentifier(apiPath,
            getSchemaContext(), Optional.ofNullable(getMountPointService()));
