...
public final class ApiSegment implements Immutable {
    ...
    public static @NonNull ApiSegment of(final ApiPath path, final EffectiveModelContext context)
            throws InvalidPathException {
        final var steps = path.steps();
        if (steps.isEmpty()) {
            return new ApiSegment(SchemaInferenceStack.of(context).toInference(), YangInstanceIdentifier.empty(),
                ImmutableList.of());
        }
        return create(context, steps);
    }
    ...
    private static @NonNull ApiSegment create(final EffectiveModelContext modelContext, final ImmutableList<Step> steps)
            throws InvalidPathException {
        ...
        while (true) {
            ...
            if (schema instanceof ListEffectiveStatement) {
                final var schemaKeys = schema.findFirstEffectiveSubstatementArgument(KeyEffectiveStatement.class)
                    .orElse(Set.of());
                final var stepKeys = step instanceof ListInstance ? ((ListInstance) step).keyValues() : List.of();
                if (schemaKeys.size() != stepKeys.size()) {
                    throw new InvalidPathException("List " + qname + " takes " + schemaKeys.size() + " keys, but "
                        + stepKeys.size() + " values specified");
                }

                if (schemaKeys.isEmpty()) {
                    // FIXME: index predicates and create NodeIdentifierWithPredicates
                    arg = NodeIdentifierWithPredicates.of(qname, (Map) null);
                } else {
                    arg = new NodeIdentifier(qname);
                }
            } else {
    ...
    private static QNameModule resolveModule(final EffectiveModelContext modelContext, final String moduleName)
            throws InvalidPathException {
        final var modules = modelContext.findModuleStatements(moduleName);
        if (modules.isEmpty()) {
            throw new InvalidPathException("Cannot find module " + moduleName);
        }
        return modules.iterator().next().localQNameModule();
    }
}
