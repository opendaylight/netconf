...
public final class CreateStreamUtil {
     ...
     */
    @Nonnull
    public static DOMRpcResult createDataChangeNotifiStream(@Nonnull final NormalizedNodeContext payload,
            @Nonnull final SchemaContextRef refSchemaCtx) {
        // parsing out of container with settings and path
        final ContainerNode data = (ContainerNode) Preconditions.checkNotNull(payload).getData();
        ...
        final StringBuilder streamNameBuilder = new StringBuilder(
                prepareDataChangeNotifiStreamName(path, Preconditions.checkNotNull(refSchemaCtx).get(), data));
    ...
    /**
     * Create YANG notification stream with POST operation via RPC.
     *
     * @param notificationDefinition Input of RPC.
     * @param refSchemaCtx           Reference to {@link SchemaContext} - {@link SchemaContextRef}.
    ...
    public static NotificationListenerAdapter createYangNotifiStream(
            @Nonnull final NotificationDefinition notificationDefinition, @Nonnull final SchemaContextRef refSchemaCtx,
            @Nonnull final NotificationOutputType outputType) {
        final String streamName = parseNotificationStreamName(
                Preconditions.checkNotNull(notificationDefinition),
                Preconditions.checkNotNull(refSchemaCtx),
                Preconditions.checkNotNull(outputType.getName()));
    ...
    private static String parseNotificationStreamName(final NotificationDefinition notificationDefinition,
        ...
        Preconditions.checkNotNull(module, "Module for namespace "
                + notificationDefinitionQName.getModule().getNamespace()
                + " does not exist");
