...
public final class ReadDataTransactionUtil {
    ...
    public static @Nullable NormalizedNode readData(final @NonNull String valueOfContent,
        ...
        switch (valueOfContent) {
            case RestconfDataServiceConstant.ReadData.CONFIG:
                if (withDefa == null) {
                    return readDataViaTransaction(strategy, LogicalDatastoreType.CONFIGURATION, path, fields);
                } else {
                    return prepareDataByParamWithDef(
                            readDataViaTransaction(strategy, LogicalDatastoreType.CONFIGURATION, path, fields),
                            path, withDefa, ctx);
                }
            case RestconfDataServiceConstant.ReadData.NONCONFIG:
    ...
    private static @Nullable NormalizedNode readAllData(final @NonNull RestconfStrategy strategy,
        ...
        if (withDefa == null) {
            configDataNode = readDataViaTransaction(strategy, LogicalDatastoreType.CONFIGURATION, path, fields);
        } else {
