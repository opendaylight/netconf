...
public final class RestconfLoggingBroker {
    ...
    private synchronized BigInteger getAndIncrementMessageId() {
        final var currentMessageId = messageIdCounter;
    ...
    private void writeUserInfo(final SecurityContext securityContext, final StringBuilder logBuilder) {
    ...
    private void writeHttpHeaders(final MultivaluedMap<String, ?> parameters, final StringBuilder logBuilder) {
        ...
        final var hiddenHttpHeaders = loggingConfiguration.getHiddenHttpHeaders();
        if (hiddenHttpHeaders.isEmpty()) {
            writeMap(parameters, logBuilder);
        } else {
            final var filteredParameters = filterHttpHeaders(
