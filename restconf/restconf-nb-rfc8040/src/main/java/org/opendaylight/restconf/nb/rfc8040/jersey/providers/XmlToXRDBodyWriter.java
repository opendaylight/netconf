...
/**
 * XML writer for Root Resource Discovery
 * See: https://tools.ietf.org/html/rfc8040#section-3.1
 */
@Provider
@Produces({ Rfc8040.MediaTypes.XRD + RestconfConstants.XML})
public final class XmlToXRDBodyWriter implements MessageBodyWriter<String> {

    private static final Logger LOG = LoggerFactory.getLogger(XmlToXRDBodyWriter.class);
     ...
     * the serialized form of {@code type}. A non-negative return value has been used in a HTTP
    ...
    public void writeTo(String input, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType,
        ...
        try {
            ...
            LOG.debug("Error creating XMLStreamWriter", ex);
        }
    }
}
