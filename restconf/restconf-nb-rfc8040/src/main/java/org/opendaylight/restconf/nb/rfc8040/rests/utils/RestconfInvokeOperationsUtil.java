...
public final class RestconfInvokeOperationsUtil {
    ...
    public static DOMActionResult invokeActionViaMountPoint(final DOMMountPoint mountPoint,
        final ContainerNode data, final SchemaPath schemaPath) {
        final Optional<DOMActionService> mountPointService = mountPoint.getService(DOMActionService.class);
        if (mountPointService.isPresent()) {
            final DOMDataTreeIdentifier domDataTreeIdentifier = prepareDataTreeId(data);
     ...
     *
     * @param data
     *             {@link NormalizedNode} - normalized node according to the YANG schema
     * @return {@link DOMDataTreeIdentifier} domDataTreeIdentifier
     */
    private static DOMDataTreeIdentifier prepareDataTreeId(final NormalizedNode<?, ?> data) {
        YangInstanceIdentifier yangInstanceIdentifier = YangInstanceIdentifier
            .create(new YangInstanceIdentifier.NodeIdentifier(data.getIdentifier().getNodeType()));
