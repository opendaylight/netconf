...
final class CreateStreamUtil {
    ...
    private static YangInstanceIdentifier preparePath(final ContainerNode data, final QName qualifiedName) {
        final Optional<DataContainerChild> path = data.getChild(
    ...
    private static <T> T parseEnum(final ContainerNode data, final Class<T> clazz, final String paramName) {
        final Optional<DataContainerChild> optAugNode = data.getChild(
        ...
        final Optional<DataContainerChild> enumNode = ((AugmentationNode) augNode).getChild(
