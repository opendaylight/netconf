...
final class Utf8Buffer implements Mutable {
    ...
    void appendByte(final int value) {
        var buf = bos;
        if (buf == null) {
    ...
    void flushTo(final @NonNull StringBuilder sb, final int offset) throws ParseException {
        final var buf = bos;
        if (buf != null && buf.size() != 0) {
            flushTo(sb, buf, offset);
        }
    }

    // Split out to aid inlining
    private void flushTo(final StringBuilder sb, final ByteArrayOutputStream buf, final int offset)
    ...
    private static ParseException report(final int offset, final byte[] bytes, final CharacterCodingException cause) {
        final String str = new String(bytes, StandardCharsets.UTF_8);
        LOG.debug(Markers.confidential(), "Rejecting invalid UTF-8 sequence '{}'", str, cause);
        return new ParseException("Invalid UTF-8 sequence '" + str + "': " + cause.getMessage(), offset);
    }
}