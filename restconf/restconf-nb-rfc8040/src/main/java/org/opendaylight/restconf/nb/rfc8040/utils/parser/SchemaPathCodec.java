...
public final class SchemaPathCodec {
        ...
        for (int i = 0; i < slicedPath.length; i++) {
            ...
            if (i == 0) {
                if (pathPart.isEmpty()) {
                    ...
                    throw new IllegalArgumentException(String.format("Input schema path '%s' should start with dot "
                ...
                if (pathPart.isEmpty() || pathPart.equals(String.valueOf(RestconfConstants.DOT))) {
                    throw new IllegalArgumentException(String.format("Schema path '%s' contains dots or empty "
            ...
            if (moduleAndLocalName.length == 1) {
                if (parentModule == null) {
                    throw new IllegalArgumentException(String.format("The first path element '%s' must be specified "
                ...
                throw new IllegalArgumentException(String.format("Schema path part '%s' contains more "
                        + "than two colons.", pathPart.trim()));
            }
        }
    ...
    private static QNameModule findModuleByName(final String moduleName, final SchemaContext schemaContext) {
        ...
        if (modules.isEmpty()) {
            throw new IllegalArgumentException(String.format("Module with name '%s' cannot be found in "
                    + "schema context.", moduleName));
        } else {
