...
public final class RestconfDocumentedExceptionMapper implements ExceptionMapper<RestconfDocumentedException> {
    ...
    private static final ImmutableList<MediaType> JSON_MEDIA_TYPES = ImmutableList.of(MediaType.APPLICATION_JSON_TYPE,
        YANG_DATA_JSON_TYPE, YANG_PATCH_JSON_TYPE);
    private static final ImmutableList<MediaType> XML_MEDIA_TYPES = ImmutableList.of(MediaType.APPLICATION_XML_TYPE,
        YANG_DATA_XML_TYPE, YANG_PATCH_XML_TYPE);
    ...
    private static boolean isJsonCompatibleMediaType(final MediaType mediaType) {
        return JSON_MEDIA_TYPES.contains(replaceWildcardPartsWithDefaultLabels(mediaType, JSON_MEDIA_LABEL));
    }

    private static boolean isXmlCompatibleMediaType(final MediaType mediaType) {
        return XML_MEDIA_TYPES.contains(replaceWildcardPartsWithDefaultLabels(mediaType, XML_MEDIA_LABEL));
    }

    private static MediaType replaceWildcardPartsWithDefaultLabels(final MediaType mediaType, final String label) {
        final StringBuilder cleanMediaType = new StringBuilder();
        if (MediaType.MEDIA_TYPE_WILDCARD.equalsIgnoreCase(mediaType.getType())) {
            cleanMediaType.append(APPLICATION_MEDIA_LABEL);
        } else {
            cleanMediaType.append(mediaType.getType());
        }
        cleanMediaType.append('/');
        if (MediaType.MEDIA_TYPE_WILDCARD.equalsIgnoreCase(mediaType.getSubtype())) {
            cleanMediaType.append(label);
        } else {
            cleanMediaType.append(mediaType.getSubtype());
        }
        return MediaType.valueOf(cleanMediaType.toString());
    }
