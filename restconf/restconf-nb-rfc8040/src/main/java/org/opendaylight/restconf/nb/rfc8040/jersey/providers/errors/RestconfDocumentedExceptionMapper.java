...
public final class RestconfDocumentedExceptionMapper implements ExceptionMapper<RestconfDocumentedException> {
    ...
    private static final String APPLICATION_MEDIA_LABEL = "application";
    private static final String JSON_MEDIA_LABEL = "json";
    private static final String XML_MEDIA_LABEL = "xml";
    ...
    public Response toResponse(final RestconfDocumentedException exception) {
        ...
        if (!responseStatus.equals(Response.Status.FORBIDDEN)
                && responseStatus.getFamily().equals(Response.Status.Family.CLIENT_ERROR)) {
    ...
    private static Status getResponseStatusCode(final RestconfDocumentedException exception) {
        if (exception.getStatus() == null) {
            if (exception.getErrors().isEmpty()) {
                ...
                final Set<Integer> allStatusCodesOfErrorEntries = exception.getErrors().stream()
    ...
    private MediaType getSupportedMediaType() {
        ...
        final Set<MediaType> acceptableAndSupportedMediaTypes = acceptableMediaTypes.stream()
        ...
        if (acceptableAndSupportedMediaTypes.size() == 0) {
            ...
            if (requestMediaType == null) {
                ...
                return chooseMediaType(Collections.singletonList(requestMediaType))
                        .orElseGet(() -> {
            ...
            final List<MediaType> fullySpecifiedMediaTypes = acceptableAndSupportedMediaTypes.stream()
                    .filter(mediaType -> !mediaType.getType().contains(MediaType.MEDIA_TYPE_WILDCARD)
                            && !mediaType.getSubtype().contains(MediaType.MEDIA_TYPE_WILDCARD))
                    .collect(Collectors.toList());
            if (fullySpecifiedMediaTypes.isEmpty()) {
                ...
                final List<MediaType> mediaTypesWithSpecifiedSubtypes = acceptableAndSupportedMediaTypes.stream()
                        .filter(mediaType -> !mediaType.getSubtype().contains(MediaType.MEDIA_TYPE_WILDCARD))
                        .collect(Collectors.toList());
    ...
    private static MediaType chooseAndCheckMediaType(final List<MediaType> options) {
        ...
        Preconditions.checkState(mediaTypeOpt.isPresent());
    ...
    private static boolean isJsonCompatibleMediaType(MediaType mediaType) {
        mediaType = replaceWildcardPartsWithDefaultLabels(mediaType, JSON_MEDIA_LABEL);
        return mediaType.equals(MediaType.APPLICATION_JSON_TYPE) || mediaType.equals(YANG_DATA_JSON_TYPE)
                || mediaType.equals(YANG_PATCH_JSON_TYPE);
    }

    private static boolean isXmlCompatibleMediaType(MediaType mediaType) {
        mediaType = replaceWildcardPartsWithDefaultLabels(mediaType, XML_MEDIA_LABEL);
        return mediaType.equals(MediaType.APPLICATION_XML_TYPE) || mediaType.equals(YANG_DATA_XML_TYPE)
                || mediaType.equals(YANG_PATCH_XML_TYPE);
    }

    private static MediaType replaceWildcardPartsWithDefaultLabels(MediaType mediaType, final String label) {
        final StringBuilder cleanMediaType = new StringBuilder();
        if (mediaType.getType().equalsIgnoreCase(MediaType.MEDIA_TYPE_WILDCARD)) {
            cleanMediaType.append(APPLICATION_MEDIA_LABEL);
        } else {
            cleanMediaType.append(mediaType.getType());
        }
        cleanMediaType.append('/');
        if (mediaType.getSubtype().equalsIgnoreCase(MediaType.MEDIA_TYPE_WILDCARD)) {
            cleanMediaType.append(label);
        } else {
            cleanMediaType.append(mediaType.getSubtype());
        }
        mediaType = MediaType.valueOf(cleanMediaType.toString());
        return mediaType;
    }
