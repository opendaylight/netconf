...
public final class SalRemoteServiceImpl implements SalRemoteService {
    ...
    public ListenableFuture<RpcResult<CreateDataChangeEventSubscriptionOutput>> createDataChangeEventSubscription(
        ...
        final DataChangeScope legacyScope;
        final NotificationOutputType legacyType;
        ...
        if (aug != null) {
            ...
            final var scope = requireNonNullElse(aug.getScope(), Scope.BASE);
            switch (scope) {
                case BASE:
                    legacyScope = DataChangeScope.BASE;
                    break;
                case ONE:
                    legacyScope = DataChangeScope.ONE;
                    break;
                case SUBTREE:
                    legacyScope = DataChangeScope.SUBTREE;
                    break;
                default:
                    throw new IllegalStateException("Unhandled scope " + scope);
            }

            final var type = requireNonNullElse(aug.getNotificationOutputType(), NotificationOutputType.XML);
            switch (type) {
                case JSON:
                    legacyType = NotificationOutputType.JSON;
                    break;
                case XML:
                    legacyType = NotificationOutputType.XML;
                    break;
                default:
                    throw new IllegalStateException("Unhandled output type " + type);
            }
        } else {
            datastoreType = LogicalDatastoreType.CONFIGURATION;
            legacyScope = DataChangeScope.BASE;
