...
public final class FieldsParameter implements Immutable {
    ...
    public static FieldsParameter parse(final String str) throws ParseException {
        ...
        int idx = 0;
        while (true) {
            idx = parseNodeSelector(nodeSelectors, str, idx);
            if (idx == str.length()) {
            ...
            if (ch != ';') {
                throw new ParseException("Expecting ';', got '" + ch + "'", idx);
            }
        }
    ...
    private static int parseNodeSelector(final Builder<NodeSelector> list, final String str, final int offset)
