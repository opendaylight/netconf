...
public final class ParserIdentifier {
    ...
    public static InstanceIdentifierContext<?> toInstanceIdentifier(
        ...
        if (identifier != null && identifier.contains(RestconfConstants.MOUNT)) {
            ...
            if (child.isPresent()) {
                return new InstanceIdentifierContext<SchemaNode>(pathYangInstanceIdentifier, child.get().getDataSchemaNode(),
            ...
            if (schemaNode instanceof ActionDefinition) {
                return new InstanceIdentifierContext<>(pathYangInstanceIdentifier, (ActionDefinition) schemaNode, domMountPoint,
                    mountSchemaContext);
            }
            return new InstanceIdentifierContext<>(pathYangInstanceIdentifier, (RpcDefinition) schemaNode, domMountPoint,
                mountSchemaContext);
        } else {
            final YangInstanceIdentifier deserialize = IdentifierCodec.deserialize(identifier, schemaContext);
            final Optional<DataSchemaContextNode<?>> child = DataSchemaContextTree.from(schemaContext).findChild(deserialize);

            if (child.isPresent()) {
                return new InstanceIdentifierContext<SchemaNode>(
                            deserialize, child.get().getDataSchemaNode(), null, schemaContext);
            }
            ...
            if (schemaNode instanceof ActionDefinition) {
                return new InstanceIdentifierContext<>(deserialize, (ActionDefinition) schemaNode, null,
                    schemaContext);
            }
            return new InstanceIdentifierContext<>(deserialize, (RpcDefinition) schemaNode, null,
                schemaContext);
        }
    }
