...
public final class ParserIdentifier {
    ...
    public static InstanceIdentifierContext<?> toInstanceIdentifier(
        ...
        if (identifier != null && identifier.contains(RestconfConstants.MOUNT)) {
            ...
            YangInstanceIdentifier pathYangInstanceIdentifier = IdentifierCodec
                .deserialize(pathId, mountSchemaContext);
            return createInstanceIdentifierContext(pathYangInstanceIdentifier, mountSchemaContext, domMountPoint);
        } else {
            YangInstanceIdentifier pathYangInstanceIdentifier = IdentifierCodec.deserialize(identifier, schemaContext);
            return createInstanceIdentifierContext(pathYangInstanceIdentifier, schemaContext, null);
        }
    }
     ...
     * @return {@link InstanceIdentifierContext}
     */
    public static InstanceIdentifierContext<?> createInstanceIdentifierContext(
        final YangInstanceIdentifier instanceIdentifier, final SchemaContext schemaContext,
        final DOMMountPoint domMountPoint) {
        final Optional<DataSchemaContextNode<?>> child = DataSchemaContextTree.from(schemaContext)
            .findChild(instanceIdentifier);

        if (child.isPresent()) {
            return new InstanceIdentifierContext<SchemaNode>(instanceIdentifier, child.get().getDataSchemaNode(),
                domMountPoint != null ? domMountPoint : null, schemaContext);
        }

        List<QName> qnames = instanceIdentifier.getPathArguments().stream()
        ...
        if (schemaNode instanceof ActionDefinition) {
            return new InstanceIdentifierContext<>(instanceIdentifier, (ActionDefinition) schemaNode,
                domMountPoint != null ? domMountPoint : null, schemaContext);
        }
        return new InstanceIdentifierContext<>(instanceIdentifier, (RpcDefinition) schemaNode,
            domMountPoint != null ? domMountPoint : null, schemaContext);
    }
