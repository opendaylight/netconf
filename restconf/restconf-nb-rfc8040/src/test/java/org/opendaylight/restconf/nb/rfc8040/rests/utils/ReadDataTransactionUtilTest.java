...
public class ReadDataTransactionUtilTest {
    ...
    public void readDataConfigWithXPathTest() {
        ...
        when(netconfReadTx.read(Mockito.any(), ArgumentMatchers.<NetconfXPathContext<YangInstanceIdentifier>>any()))
        ...
        final List<Set<QName>> fieldsFilter = new ArrayList<>();
        fieldsFilter.add(Collections.singleton(QName.create("ns", "2020-01-01", "test")));
        transactionVarsWrapper.setFieldsFilter(fieldsFilter);
        doReturn(DATA.path).when(context).getInstanceIdentifier();
        final String valueOfContent = RestconfDataServiceConstant.ReadData.CONFIG;
    ...
    public void prepareXPathWithoutFieldsTest() {
        ...
        final NetconfXPathContext<YangInstanceIdentifier> xpathContext = ReadDataTransactionUtil
                .prepareXPathContext(transactionNode);
