...
public class YangInstanceIdentifierDeserializerTest {
    ...
    public void prepareQnameNotValidContainerNameNegativeTest() {
        RestconfDocumentedException e = assertThrows(RestconfDocumentedException.class,
            () -> YangInstanceIdentifierDeserializer.create(SCHEMA_CONTEXT, "deserializer-test:contA/leafB"));
        assertEquals("Not expected error type", RestconfError.ErrorType.PROTOCOL, e.getErrors().get(0).getErrorType());
        assertEquals("Not expected error tag", RestconfError.ErrorTag.DATA_MISSING, e.getErrors().get(0).getErrorTag());
    }
    ...
    public void notAllListKeysEncodedNegativeTest() {
        ...
        assertEquals("Not expected error status code", 400, e.getErrors().get(0).getErrorTag().getStatusCode());
    }
