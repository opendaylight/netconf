...
public class InvokeActionThroughRestconfDataServiceTest {
    ...
    private static final class DOMActionImpl implements DOMActionImplementation {
        ...
        DOMActionImpl(final AdapterContext adapterContext,
                final Class<? extends Action<InstanceIdentifier<Cont1>, Input, Output>> actionInterface,
                final Action<InstanceIdentifier<Cont1>, Input, Output> implementation) {
        ...
        public ListenableFuture<? extends DOMActionResult> invokeAction(final Absolute type,
            ...
            final var identifier = codec.fromYangInstanceIdentifier(path.getRootIdentifier());
            final var rpcInput = codec.fromNormalizedNodeActionInput(actionInterface, input);
            final ListenableFuture<RpcResult<Output>> invocation = implementation.invoke(identifier, rpcInput);
            return Futures.transformAsync(invocation, r -> {
                if (r.isSuccessful()) {
                    final var output = codec.toLazyNormalizedNodeActionOutput(actionInterface, r.getResult());
                    return Futures.immediateFuture(new SimpleDOMActionResult(output, r.getErrors()));
                } else {
                    return Futures.immediateFuture(new SimpleDOMActionResult(r.getErrors()));
                }
