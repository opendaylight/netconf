...
public class PutDataTransactionUtilTest {
    ...
    public void testValidTopLevelNodeNameWrongTopIdentifier() throws Exception {
        iidContext = new InstanceIdentifierContext<>(iid, schemaNode, null, schema);
        payload = new NormalizedNodeContext(iidContext, buildLeaf);
        final QName containerQname = QName.create("http://example.com/ns/example-jukebox", "2015-04-04", "player");
        final QName leafQname = QName.create(containerQname, "gap");
        final YangInstanceIdentifier iid = YangInstanceIdentifier.builder()
                .node(containerQname)
                .node(leafQname)
                .build();
        PutDataTransactionUtil.validTopLevelNodeName(iid, payload);
    }
