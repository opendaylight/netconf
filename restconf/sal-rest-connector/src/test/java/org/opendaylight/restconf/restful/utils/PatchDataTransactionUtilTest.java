...
public class PatchDataTransactionUtilTest {
    ...
    private YangInstanceIdentifier iIDCreateDelete;
    private YangInstanceIdentifier iIDMerge;
    private SchemaContext schema;
    private ContainerNode buildBaseContainerForCreateDelete;
    private YangInstanceIdentifier targetNodeCreateDelete;
    ...
    public void setUp() throws Exception {
        ...
        iIDCreateDelete = YangInstanceIdentifier.builder()
                .node(baseQName)
        ...
        buildBaseContainerForCreateDelete = Builders.containerBuilder()
                .withNodeIdentifier(new YangInstanceIdentifier.NodeIdentifier(baseQName))
                .withChild(buildPlayerContainer)
                .build();

        targetNodeCreateDelete = YangInstanceIdentifier.builder(iIDCreateDelete)
                .node(containerPlayerQName)
                .node(leafGapQName)
                .build();
        ...
        doReturn(Futures.immediateCheckedFuture(false)).doReturn(Futures.immediateCheckedFuture(true))
                .when(rWTransaction).exists(LogicalDatastoreType.CONFIGURATION, targetNodeCreateDelete);
    ...
    public void testPatchDataCreateAndDelete() throws Exception {
        final PATCHEntity entityCreate = new PATCHEntity("edit1", "create", targetNodeCreateDelete, buildBaseContainerForCreateDelete);
        final PATCHEntity entityDelete = new PATCHEntity("edit2", "delete", targetNodeCreateDelete);
        final List<PATCHEntity> entities = new ArrayList<>();

        entities.add(entityCreate);
        entities.add(entityDelete);

        final InstanceIdentifierContext<? extends SchemaNode> iidContext = new InstanceIdentifierContext<>(iIDCreateDelete, null, null, schema);
