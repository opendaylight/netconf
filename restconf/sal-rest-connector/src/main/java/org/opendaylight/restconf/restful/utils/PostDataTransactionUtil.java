...
public class PostDataTransactionUtil {
    ...
    public static URI resolveLocation(final UriInfo uriInfo, final DOMMountPoint mountPoint,
            final YangInstanceIdentifier normalizedII, final SchemaContextRef schemaContextRef) {
        if (uriInfo == null) {
            return null;
        }
        ...
        if(mountPoint == null){
            uriBuilder.path(ParserIdentifier.toString(normalizedII, schemaContextRef.get()));
        } else {
            uriBuilder.path(ParserIdentifier.toString(normalizedII, mountPoint.getSchemaContext()));
        }
