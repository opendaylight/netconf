...
public class ParameterAwareNormalizedNodeWriter implements RestconfNormalizedNodeWriter {
    ...
    private List<QName> parsedFields;
    ...
    public static ParameterAwareNormalizedNodeWriter forStreamWriter(
            final NormalizedNodeStreamWriter writer, final Optional<Integer> maxDepth, Optional<String> fields) {
     ...
     * {@link #forStreamWriter(NormalizedNodeStreamWriter, Optional, Optional)}
    ...
    public final ParameterAwareNormalizedNodeWriter write(final NormalizedNode<?, ?> node) throws IOException {
        if (fields.isPresent()) {
            parsedFields = ParserFieldsParameter.parseFieldsParameter(
                    fields.get(), node.getNodeType().getNamespace().toString());

            final QName parentQname = QName.create(
                    node.getNodeType().getNamespace().toString(), node.getNodeType().getLocalName());

            // FIXME if parent is not selected then write nothing
            if (!parsedFields.contains(parentQname)) {
                return this;
            }
        }
    ...
    protected final boolean writeChildren(final Iterable<? extends NormalizedNode<?, ?>> children) throws IOException {
        if (!maxDepth.isPresent() || currentDepth < maxDepth.get()) {
            for (NormalizedNode<?, ?> child : children) {
                if (!fields.isPresent() || fields.get().contains(child.getNodeType().getLocalName())) {
                    write(child);
                }
            }
        }
