...
public class ParameterAwareNormalizedNodeWriter implements RestconfNormalizedNodeWriter {
    ...
    public static ParameterAwareNormalizedNodeWriter forStreamWriter(
            final NormalizedNodeStreamWriter writer, final Integer maxDepth, List<Set<QName>> fields) {
    ...
    public static ParameterAwareNormalizedNodeWriter forStreamWriter(final NormalizedNodeStreamWriter writer,
                                                                     ...
                                                                     final List<Set<QName>> fields) {
        if (orderKeyLeaves) {
            return new OrderedDepthAwareNormalizedNodeWriter(writer, maxDepth, fields);
        } else {
     ...
     *
     * @param node Node
     * @param selectedChildNode true if node is child of selected node
     * @return
     * @throws IOException when thrown from the backing writer.
     */
    public final ParameterAwareNormalizedNodeWriter write(
            final NormalizedNode<?, ?> node, final boolean selectedChildNode) throws IOException {
        if (wasProcessedAsCompositeNode(node, selectedChildNode)) {
            return this;
        }

        if (wasProcessAsSimpleNode(node, selectedChildNode)) {
            return this;
        }
    ...
    private boolean wasProcessAsSimpleNode(final NormalizedNode<?, ?> node, boolean selectedNodeChild) throws IOException {
        if (node instanceof LeafSetEntryNode) {
            if (selectedByFields(node, selectedNodeChild, true) || checkDepth()) {
    ...
    protected boolean checkDepth() {
        if (fields == null && (maxDepth == null || currentDepth < maxDepth)) {
            return true;
        }
    ...
    protected boolean selectedByFields(final NormalizedNode<?, ?> node,
                                       final boolean selectedNodeChild,
                                       final boolean mixinParent) {
        if (fields == null) {
            return false;
        }
    ...
    protected final boolean writeChildren(final Iterable<? extends NormalizedNode<?, ?>> children,
        ...
        for (final NormalizedNode<?, ?> child : children) {
            final boolean stillSelected = selectedByFields(child, selectedNodeChild, mixinParent);
            if (stillSelected || checkDepth()) {
                write(child, true);
            }
        }
        writer.endNode();
    ...
    protected boolean writeMapEntryChildren(final MapEntryNode mapEntryNode, final boolean selectedNodeChild) throws IOException {
        final boolean stillSelected = selectedByFields(mapEntryNode, selectedNodeChild, true);
        if (stillSelected || checkDepth()) {
            writeChildren(mapEntryNode.getValue(), true, false);
        } else if (maxDepth != null && currentDepth == maxDepth) {
    ...
    protected boolean writeMapEntryNode(final MapEntryNode node, final boolean selectedNodeChild) throws IOException {
        if(writer instanceof NormalizedNodeStreamAttributeWriter) {
        ...
        currentDepth++;
        writeMapEntryChildren(node, selectedNodeChild);
        currentDepth--;
        return true;
    }

    private boolean wasProcessedAsCompositeNode(final NormalizedNode<?, ?> node, boolean selectedNodeChild) throws IOException {
        ...
        if (node instanceof ContainerNode) {
            ...
            currentDepth++;
            processedAsCompositeNode = writeChildren(n.getValue(), selectedNodeChild, false);
            currentDepth--;
        }
        else if (node instanceof MapEntryNode) {
            processedAsCompositeNode =  writeMapEntryNode((MapEntryNode) node, selectedNodeChild);
        }
            ...
            processedAsCompositeNode = writeChildren(n.getValue(), selectedNodeChild, true);
        }
    ...
    private static final class OrderedDepthAwareNormalizedNodeWriter extends ParameterAwareNormalizedNodeWriter {
        private static final Logger LOG = LoggerFactory.getLogger(OrderedDepthAwareNormalizedNodeWriter.class);

        OrderedDepthAwareNormalizedNodeWriter(final NormalizedNodeStreamWriter writer, final Integer maxDepth,
                                              final List<Set<QName>> fields) {
        ...
        protected boolean writeMapEntryNode(final MapEntryNode node, final boolean selectedNodeChild) throws IOException {
            ...
            for (QName qname : qnames) {
                ...
                if (child.isPresent()) {
                    final boolean stillSelected = selectedByFields(child.get(), selectedNodeChild, true);
                    if (stillSelected || checkDepth()) {
                        write(child.get(), true);
                    }
                } else {
