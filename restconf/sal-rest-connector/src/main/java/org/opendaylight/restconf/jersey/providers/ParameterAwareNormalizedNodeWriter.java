...
public class ParameterAwareNormalizedNodeWriter implements RestconfNormalizedNodeWriter {
     ...
     * @param mixinParent {@code true} if parent is mixin, false otherwise
     * @return {@code true} if node will be written, false otherwise
     */
    protected boolean selectedByParameters(final NormalizedNode<?, ?> node, final boolean mixinParent) {
        ...
        // mixin nodes are never selected in fields but must be written if they are first
        ...
        // after this depth only depth is used to determine when to write node
        return (maxDepth == null || currentDepth < maxDepth);
    }
    ...
    protected boolean writeMapEntryChildren(final MapEntryNode mapEntryNode) throws IOException {
        if (selectedByParameters(mapEntryNode, true)) {
    ...
    private static final class OrderedDepthAwareNormalizedNodeWriter extends ParameterAwareNormalizedNodeWriter {
        ...
        protected boolean writeMapEntryNode(final MapEntryNode node) throws IOException {
            ...
            for (QName qname : qnames) {
                ...
                if (child.isPresent()) {
                    if (selectedByParameters(child.get(), true)) {
                        write(child.get());
                    }
                } else {
