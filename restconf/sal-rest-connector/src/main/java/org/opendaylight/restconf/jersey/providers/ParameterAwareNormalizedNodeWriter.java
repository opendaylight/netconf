...
public class ParameterAwareNormalizedNodeWriter implements RestconfNormalizedNodeWriter {
    ...
    protected final List<YangInstanceIdentifier.PathArgument> fields;
    protected int currentDepth = 0;

    private ParameterAwareNormalizedNodeWriter(final NormalizedNodeStreamWriter writer, final Optional<Integer> maxDepth,
                                               final List<YangInstanceIdentifier.PathArgument> fields) {
    ...
    public static ParameterAwareNormalizedNodeWriter forStreamWriter(
            final NormalizedNodeStreamWriter writer, final Optional<Integer> maxDepth, List<YangInstanceIdentifier.PathArgument> fields) {
     ...
     * {@link #forStreamWriter(NormalizedNodeStreamWriter, Optional, List)}
    ...
    private boolean wasProcessAsSimpleNode(final NormalizedNode<?, ?> node) throws IOException {
        if (node instanceof LeafSetEntryNode) {
            if (!maxDepth.isPresent() || currentDepth < maxDepth.get()) {
                if (fields.isEmpty() || fields.contains(node.getIdentifier())) {
                    fields.remove(node.getIdentifier());
    ...
    protected final boolean writeChildren(final Iterable<? extends NormalizedNode<?, ?>> children) throws IOException {
        if (!maxDepth.isPresent() || currentDepth < maxDepth.get()) {
            for (NormalizedNode<?, ?> child : children) {
                if (fields.isEmpty() || fields.contains(child.getIdentifier())) {
                    fields.remove(child.getIdentifier());
                    write(child);
                }
            }
        }
    ...
    protected boolean writeMapEntryChildren(final MapEntryNode mapEntryNode) throws IOException {
        if (!maxDepth.isPresent() || currentDepth < maxDepth.get()) {
            if (fields.isEmpty() || fields.contains(mapEntryNode.getIdentifier())) {
                fields.remove(mapEntryNode.getIdentifier());
                writeChildren(mapEntryNode.getValue());
            }
        } else if (maxDepth.isPresent() && currentDepth == maxDepth.get()) {
    ...
    private static final class OrderedDepthAwareNormalizedNodeWriter extends ParameterAwareNormalizedNodeWriter {
        ...
        OrderedDepthAwareNormalizedNodeWriter(final NormalizedNodeStreamWriter writer, final Optional<Integer> maxDepth,
                                              final List<YangInstanceIdentifier.PathArgument> fields) {
