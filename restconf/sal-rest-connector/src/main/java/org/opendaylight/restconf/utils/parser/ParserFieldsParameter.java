...
public class ParserFieldsParameter {
    ...
    /**
     * Pasrse fields parameter and return completed list of child nodes organized into levels.
    ...
    private static @Nonnull DataSchemaContextNode<?> addChildToResult(
        ...
        // if parent node is mixin continue until not mixin found
        DataSchemaContextNode<?> parentNode = currentNode;
        while (parentNode != null && parentNode.isMixin()) {
            parentNode.getChild(parentNode.getIdentifier().getNodeType());
        }
        ...
        // if child is mixin continue until not mixin found
        DataSchemaContextNode<?> child = parentNode.getChild(childQName);
        while (child != null && child.isMixin()) {
            // also mixin nodes must be added
        ...
        // add final child node to level nodes
        level.add(child.getIdentifier().getNodeType());
        return child;
    }
