...
public class ParserFieldsParameter {
    ...
    private static @Nonnull DataSchemaContextNode<?> addChildToResult(
        ...
        // resolve parent node
        final DataSchemaContextNode<?> parentNode = resolveMixinNode(currentNode, level);
        if (parentNode == null) {
            throw new RestconfDocumentedException(
        ...
        // resolve child node
        final DataSchemaContextNode<?> childNode = resolveMixinNode(parentNode.getChild(childQName), level);
        if (childNode == null) {
            throw new RestconfDocumentedException(
