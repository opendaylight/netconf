...
public class BrokerFacade {
    ...
    private NormalizedNode<?, ?> readDataViaTransaction(final DOMDataReadTransaction transaction,
        ...
        try {
            ...
            final RpcError error = e.getErrorList().get(0);
            if (error.getErrorType().equals(RpcError.ErrorType.APPLICATION) &&
                    error.getTag().equals("operation-failed") &&
                    error.getSeverity().equals(RpcError.ErrorSeverity.ERROR)
                    ) {
                throw new RestconfDocumentedException("Master is down. Please try again.",
                        ErrorType.TRANSPORT,
