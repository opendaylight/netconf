...
public final class ModifySubscriptionRpc implements DOMRpcImplementation {
    ...
    public ModifySubscriptionRpc(final SubscriptionsHolder subscriptionsHolder, final DOMSchemaService domSchemaService,
            final TransactionChainHandler transactionChainHandler, final ListeningExecutorService executorService) {
    ...
    private DOMRpcResult processRpc(final NormalizedNode<?, ?> input) {
        ...
        final RegisteredNotificationWrapper notificationWrapper = subscriptionsHolder.getNotification(subscriptionId);
    ...
    private static DefaultDOMRpcResult createErrorResponse(final QName reason) {
        final RpcError rpcError = RpcResultBuilder.newError(ErrorType.PROTOCOL,
                ErrorTag.OPERATION_FAILED.getTagValue(), reason.getLocalName());
