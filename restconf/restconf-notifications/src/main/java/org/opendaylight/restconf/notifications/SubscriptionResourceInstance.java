...
import org.opendaylight.restconf.server.SubscribedEventStreamService;
...
final class SubscriptionResourceInstance extends WebHostResourceInstance {
    ...
    private static final Map<Long, EventStreamListener> LISTENERS = new HashMap<>();

    private final EventStreamService service = new SubscribedEventStreamService();
    ...
    public RequestResponse prepare(final ImplementedMethod method, final URI targetUri, final HttpHeaders headers,
        ...
        if (segment.equals("subscription")) {
            return prepareEventStream(method, targetUri, headers, peeler, xrd);
        }
    ...
    private RequestResponse prepareEventStream(final ImplementedMethod method, final URI targetUri,
        ...
        final var subscriptionId = peeler.remaining();
        final var listener = new StreamListener();
        LISTENERS.put(Long.parseLong(subscriptionId), listener);
        service.startEventStream("/subscription/" + subscriptionId, listener,
