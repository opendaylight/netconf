...
final class SubscriptionResourceInstance extends WebHostResourceInstance {
    ...
    @Override
    public Response prepare(final ChannelHandler channelHandler, final ImplementedMethod method, final URI targetUri,
        ...
        return switch (method) {
            case GET -> prepareEventStream(channelHandler, method, targetUri, headers, subscriptionId, xrd);
            case HEAD, OPTIONS -> EmptyResponse.NOT_FOUND; // TODO
            default -> EmptyResponse.NOT_FOUND;
        };
    }

    private Response prepareEventStream(final ChannelHandler channelHandler, final ImplementedMethod method,
            final URI targetUri, final HttpHeaders headers, final String subscriptionId, final XRD xrd) {
        ...
        // FIXME read subscription and get stream name and encoding
        try {
        ...
        final var stream = streamRegistry.lookupStream(NETCONF);
        if (stream == null) {
            LOG.debug("Stream '{}' not found", NETCONF);
            return EmptyResponse.NOT_FOUND;
        }
        ...
        // FIXME get encoding from subscription
        final var registration = registerSender(stream, RestconfStream.EncodingName.RFC8040_JSON, streamParams, sender);
        ...
        return new EventStreamResponse(HttpResponseStatus.OK, sender, channelHandler, sseHeartbeatIntervalMillis);
    }
