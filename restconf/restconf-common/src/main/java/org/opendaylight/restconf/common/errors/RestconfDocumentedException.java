...
public class RestconfDocumentedException extends WebApplicationException {
    ...
    public static RestconfDocumentedException decodeAndThrow(final String message, final Exception cause) {
        if (cause instanceof ExecutionException) {
            return decodeAndThrow(message, (ExecutionException) cause);
        }
        if (cause instanceof OperationFailedException) {
            return decodeAndThrow(message, (OperationFailedException) cause);
        }
        throw new RestconfDocumentedException(message, cause);
    }

    public static RestconfDocumentedException decodeAndThrow(final String message, final ExecutionException cause) {
        final Throwable wrapped = cause.getCause();
        if (wrapped instanceof OperationFailedException) {
            return decodeAndThrow(message, (OperationFailedException) wrapped);
        }
        throw new RestconfDocumentedException(message, wrapped);
    }
