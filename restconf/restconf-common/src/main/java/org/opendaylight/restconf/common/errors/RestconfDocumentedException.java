...
public class RestconfDocumentedException extends WebApplicationException {
    ...
    private static List<YangNetconfError> convertToRestconfErrors(final Collection<? extends RpcError> rpcErrors) {
        ...
        if (rpcErrors != null) {
            for (RpcError rpcError : rpcErrors) {
                errorList.add(toNetconfError(rpcError));
            }
        }
    ...
    private static YangNetconfError toNetconfError(final RpcError rpcError) {
        final String tag = rpcError.getTag();
        final RpcError.ErrorSeverity severity = rpcError.getSeverity();

        // FIXME: we need a carrier definition for this, and it needs to be shared somehow
        //        String localErrorInfo = rpcError.getInfo();
        //        if (localErrorInfo == null) {
        //            final Throwable cause = rpcError.getCause();
        //            if (rpcError.getCause() != null) {
        //                localErrorInfo = cause.getMessage();
        //            }
        //        }

        return ImmutableYangNetconfError.builder()
            .severity(severity != null ? severity.toNetconf() : ErrorSeverity.ERROR)
            .type(rpcError.getErrorType().toNetconf())
            .tag(tag == null ? ErrorTag.OPERATION_FAILED : new ErrorTag(tag))
            .message(rpcError.getMessage())
            .appTag(rpcError.getApplicationTag())
            .build();
    }
