...
public final class SubtreeEventStreamFilter implements AbstractRestconfStreamRegistry.EventStreamFilter {
    ...
    public boolean test(final YangInstanceIdentifier path, final ContainerNode body) {
        if (!path.isEmpty() && !permitsPath(filter, path)) {
            return false;
        }
    ...
    /**
     * Return {@code true} when every QName which appears in {@code path}
     * is accepted by this filter’s containment hierarchy.
     *
     * <p>The method walks the path top-down and descends into exactly one
     * containment node on each level; it stops as soon as a segment has
     * no matching containment.
     *
     * <p>**Note:** This validates only the ancestor path of an instance
     * notification (RFC7950). Content inside the notification
     * body is still evaluated later by {@link SubtreeMatcher}.
     */
