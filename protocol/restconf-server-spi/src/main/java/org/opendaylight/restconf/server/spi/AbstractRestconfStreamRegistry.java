...
public abstract class AbstractRestconfStreamRegistry implements RestconfStream.Registry {
    ...
    private final class DynSubscription extends AbstractRestconfStreamSubscription {
        ...
        private ListenableFuture<Void> updateReceiversState(final State newState) {
            ...
            return updateOperationalDatastore();
        }
        ...
        public void suspendSubscription(final QName reason) {
            Futures.addCallback(updateReceiversState(State.Suspended), new FutureCallback<>() {
                ...
                public void onSuccess(Void result) {
                    ...
                    publishSubscriptionSuspendedStateNotification(id(), reason);
                }
        ...
        public void resumeSubscription() {
            Futures.addCallback(updateReceiversState(State.Active), new FutureCallback<>() {
                ...
                public void onSuccess(Void result) {
                    ...
                    publishSubscriptionResumedStateNotification(id());
                }
    ...
    public void modifySubscription(final ServerRequest<Subscription> request, final Uint32 id,
        ...
        Futures.addCallback(modifySubscriptionParameters(id, filter, stopTime), new FutureCallback<>() {
            ...
            public void onSuccess(final Void result) {
                switch (filter) {
                    case SubscriptionFilter.Reference ref -> {
                        subscription.setInlineSubtreeSpec(null);
                        subscription.setFilterName(ref.filterName());
                    }
                    case SubscriptionFilter.SubtreeDefinition sub -> {
                        subscription.setInlineSubtreeSpec(sub.document());
                        subscription.setFilterName(null);
                    }
                    case null, default -> {
                    }
                }
    ...
    @NonNullByDefault
    protected abstract void publishSubscriptionSuspendedStateNotification(Uint32 subscriptionId, QName reason);

    @NonNullByDefault
    protected abstract void publishSubscriptionResumedStateNotification(Uint32 subscriptionId);
    ...
    protected final void recompileAllFilters() {
        filterLock.lock();
        try {
            final var recompilation = QName.create("Waiting for recompilation");

            // suspend everyone while we rebuild
            subscriptions.values().forEach(s -> s.suspendSubscription(recompilation));
        ...
        for (var subscription : subscriptions.values()) {
            ...
            if (subscriptionFilter != null && names.contains(subscriptionFilter)) {
                ...
                if (newFilter != null) {
                    ...
                    subscription.suspendSubscription(QName.create("Filter no longer valid"));
                }
            }
        }
    }
