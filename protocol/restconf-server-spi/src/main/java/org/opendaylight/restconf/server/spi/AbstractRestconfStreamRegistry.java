...
public abstract class AbstractRestconfStreamRegistry implements RestconfStream.Registry {
    ...
    private final class DynSubscription extends AbstractRestconfStreamSubscription {
        ...
        void suspend(final String reason) {
            if (state() == SubscriptionState.SUSPENDED) {
                return;
            }
            LOG.warn("Suspending subscription {} – {}", id(), reason);
            setState(SubscriptionState.SUSPENDED);
            // downstream receivers should ignore event data
            //receivers.forEach(Rfc8639Subscriber::suspend);
        }
    ...
    // --- Filter Management ---
    // filterSpecs: HashMap<String, ChoiceNode> recording known filter-spec definitions, access guarded by filterLock
    // filters: volatile ImmutableMap<String, EventStreamFilter>; reads via getAcquire(), writes via setRelease()
    // This enables safe re-compilation after every EffectiveModelContext change.
    // After every (re)compile, filtersOperationalViewUpdated(Set<String> workingNames) is called to update the operational datastore
    // with the list of filter names that compiled successfully (removing those that failed).
