...
public abstract class AbstractRestconfStreamRegistry implements RestconfStream.Registry {
    ...
    private final class DynSubscription extends AbstractRestconfStreamSubscription {
        ...
        public void addReceiver(final ServerRequest<Registration> request, final Sender sender) {
            ...
            final var receiverState = switch (state) {
                case ACTIVE -> State.Active;
                case SUSPENDED -> State.Suspended;
                default -> {
                    LOG.warn("Unexpected value of SubscriptionState: {}.", state);
                    throw new IllegalStateException("Unexpected state value: " + state);
                }
            };
            try {
        ...
        public void suspendSubscription(final QName reason) {
            if (!state().equals(SubscriptionState.SUSPENDED)) {
                ...
                LOG.debug("Attempted to suspend subscription that is already suspended state");
            }
        }
        ...
        public void resumeSubscription() {
            if (state().equals(SubscriptionState.SUSPENDED)) {
                ...
                LOG.debug("Attempted to resume subscription that is in not suspended state");
            }
        }
