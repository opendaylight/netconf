...
public final class SubtreeFilterRestconf {
    ...
    private static ContainerNode filterContainerNode(final ContainerNode filter, final ContainerNode data) {
        final var builder = ImmutableNodes.newContainerBuilder(filter.getIdentifier());
        for (var filterChild : filter.body().iterator()) {
            var matchingChild = data.findChildByArg(filterChild.getIdentifier());
            if (matchingChild.isPresent()) {
                var filteredChild = applyFilter(filterChild, matchingChild.get());
                if (filteredChild != null) {
                    builder.withChild(filteredChild);
                }
            }
        }
    ...
    private static MapNode filterMapNode(final MapNode filter, final MapNode data) {
        final var builder = ImmutableNodes.mapNodeBuilder(filter.getIdentifier());
        for (var filterEntry : filter.body().iterator()) {
            var matchingEntry = data.findChildByArg(filterEntry.getIdentifier());
            if (matchingEntry.isPresent()) {
                var filteredEntry = applyFilter(filterEntry, matchingEntry.get());
                if (filteredEntry != null) {
                    builder.withChild(filteredEntry);
                }
            }
        }
        return builder.build();
