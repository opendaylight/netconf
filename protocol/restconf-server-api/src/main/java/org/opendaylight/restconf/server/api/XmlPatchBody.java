...
public final class XmlPatchBody extends PatchBody {
    ...
    private static @NonNull PatchContext parse(final ResourceContext resource, final Document doc)
        ...
        final var patchId = requireNonNullValue(doc.getElementsByTagName("patch-id").item(0), PATCH_ID)
        ...
        for (int i = 0; i < editNodes.getLength(); i++) {
            ...
            final String editId = requireNonNullValue(element.getElementsByTagName("edit-id").item(0), EDIT_ID)
                .getFirstChild().getNodeValue();
