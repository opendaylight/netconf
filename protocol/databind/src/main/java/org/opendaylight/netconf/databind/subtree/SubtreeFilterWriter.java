...
final class SubtreeFilterWriter {
    ...
    private static void writeContentMatch(final XMLStreamWriter writer, final Prefixes prefixes,
        ...
        switch (content.selection()) {
            case Exact(var identifier) -> {
                final var prefix = prefixes.lookUpPrefix(identifier.getNodeType().getNamespace().toString());
                writer.writeStartElement(prefix, identifier.getNodeType().getLocalName(),
                    identifier.getNodeType().getNamespace().toString());
            }
    ...
    private static void writeSelection(final XMLStreamWriter writer, final Prefixes prefixes,
        ...
        for (var attr : selection.attributeMatches()) {
            ...
            final var prefix = prefixes.lookUpPrefix(sel.identifier().getNodeType().getNamespace().toString());
            writer.writeAttribute(prefix, sel.identifier().getNodeType().getNamespace().toString(),
                sel.identifier().getNodeType().getLocalName(),
                attr.value().toString());
        }
