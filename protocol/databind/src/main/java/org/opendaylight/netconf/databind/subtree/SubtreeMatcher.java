...
public final class SubtreeMatcher implements Immutable {
    ...
    private static boolean matchContent(final ContentMatchNode contentMatch, final ContainerNode node) {
        ...
        if (selection instanceof NamespaceSelection.Exact exact) {
            ...
            final var leaf = findLeaf(node, selection);
            if (leaf == null) {
                return false;
            }
            ...
            for (var candidate : wildcard.qnames()) {
                ...
                final var leaf = findLeaf(node, new NamespaceSelection.Exact(candidate));
                if (leaf != null) {
                    final var actualValue = leaf.body();
    ...
    private static @Nullable Object getAttributeValue(ContainerNode node, NamespaceSelection key) {
        ...
        final var expectedId = YangInstanceIdentifier.NodeIdentifier.create(attrQName);
    ...
    private static @Nullable LeafNode<?> findLeaf(final ContainerNode parent, final NamespaceSelection selection) {
        if (!(selection instanceof NamespaceSelection.Exact exact)) {
            return null;
        }
        final var expectedQName = exact.qname();
        final var expectedId = YangInstanceIdentifier.NodeIdentifier.create(expectedQName);
        for (var child : parent.body()) {
            if (child instanceof LeafNode<?> leaf && leaf.name().equals(expectedId)) {
                return leaf;
            }
