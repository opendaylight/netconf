...
final class SubtreeFilterReader {
    ...
    private static void fillElement(final XMLStreamReader reader, final SiblingSetBuilder builder,
        ...
        if (!elementNamespace.isEmpty()) {
            // assume there is only one parent if there is namespace and take it
            ...
            if (childModule.hasNext()) {
                ...
                if (child != null) {
                    ...
                    throw new XMLStreamException("Failed to lookup node with name %s and namespace %s under %s node."
                        .formatted(elementLocalName, elementNamespace, firstNode.getPathStep()));
                }
            } else {
