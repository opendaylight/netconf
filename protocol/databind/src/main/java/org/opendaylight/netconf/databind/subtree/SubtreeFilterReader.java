    ...
    private static Sibling fillBuilder(final DataContainerChild node) {
        ...
        switch (node) {
            ...
            case LeafNode leafNode: {
                ...
                if (leafNode.body().equals("")) {
                    return SelectionNode.builder(new Exact(identifier)).build();
                }
                // if leaf not empty - match content of the leaf
                return new ContentMatchNode(new Exact(identifier), leafNode.body());
            }
            case UnkeyedListNode listNode: {
                final var list = listNode.body().iterator().next();
                // if list is empty - select whole list
                if (list.isEmpty()) {
                    return SelectionNode.builder(new Exact(identifier)).build();
                }
                // if list not empty - select list with only specified children
                final var containment = ContainmentNode.builder(new Exact(identifier));
                for (final var child : list.body()) {
                    containment.add(fillBuilder(child));
                }
                return containment.build();
            }
