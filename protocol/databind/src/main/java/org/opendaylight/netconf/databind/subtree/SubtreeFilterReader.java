...
final class SubtreeFilterReader {
    ...
    private static void fillElement(final XMLStreamReader reader, final SiblingSetBuilder builder,
        ...
        if (!elementNamespace.isEmpty()) {
            ...
            if (firstNode instanceof DataSchemaContext.PathMixin mixin) {
                // Because of limited visibility we can't directly check either mixin list or choice, but we can try to
                // enter list - if result is null then mixin is choice and if there is some data then it is list
                final var data = mixin.enterChild(firstStack, firstNode.getPathStep());
            ...
            switch (reader.next()) {
                ...
                case XMLStreamConstants.CHARACTERS: {
                    ...
                    if (!text.trim().isEmpty()) {
                        ...
                        // get only first node as per rfc7950 we can't refine leaf into different type
                        final var entry = children.entrySet().stream().findFirst().orElseThrow();
                        final var node = entry.getValue().dataSchemaNode();
                        if (node instanceof TypedDataSchemaNode typed) {
                            try {
                                final var value = codec.codecFor(typed, entry.getKey()).parseValue(nsContext, text);
                                builder.addSibling(new ContentMatchNode(namespace, value));
                            } catch (IllegalArgumentException e) {
                                throw new XMLStreamException("Illegal value type for node %s and namespace %s."
                                    .formatted(node.getQName().getLocalName(), node.getQName().getNamespace()), e);
                            }
                        }
                        return;
                    }
