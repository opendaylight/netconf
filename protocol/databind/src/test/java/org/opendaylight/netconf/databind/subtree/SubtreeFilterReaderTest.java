...
class SubtreeFilterReaderTest {
    ...
    void testExamples(final String xml, final SubtreeFilter expected) throws Exception {
        ...
        final var reader = UntrustedXML.createXMLStreamReader(inputStream);
            ...
            Arguments.of("""
                <filter type="subtree">
                  <a:top xmlns:a="http://example.com/schema/1.2/config"
                    ...
                    .add(ContainmentNode.builder(exactNamespace(NAMESPACE, REVISION, "top"))
                        .add(SelectionNode.builder(exactNamespace(NAMESPACE2, REVISION, "users"))
                            .build()).build()).build())
        );
    }

    private static NamespaceSelection exactNamespace(final String namespace, final String revision,
            final String name) {
