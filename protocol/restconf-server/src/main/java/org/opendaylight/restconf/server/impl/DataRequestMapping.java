...
final class DataRequestMapping {
    ...
    static final List<RequestProcessor> PROCESSORS = List.of(
        ...
        new AbstractRequestProcessor(DATA_OPTIONAL_ID, HttpMethod.GET, ALL_TYPES) {
            @Override
            public void process(final RestconfServer service, final RequestParameters params) {
                final var apiPath = extractApiPath(pathPattern, params);
                ...
                final RestconfCallback<DataPostResult> callback = callback(params, result -> {
                    if (result instanceof CreateResourceResult createdRsrcResult) {
                        final var location = CREATED_DATA_RESOURCE
                            .formatted(params.basePath(), createdRsrcResult.createdPath().toString());
                        setResponse(params, createdRsrcResult, HttpResponseStatus.CREATED,
