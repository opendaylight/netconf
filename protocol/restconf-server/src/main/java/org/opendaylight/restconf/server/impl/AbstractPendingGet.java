 ...
 */
@NonNullByDefault
public abstract class AbstractPendingGet<T> extends PendingRequestWithoutBody<T> {
    private final boolean withContent;

    protected AbstractPendingGet(final EndpointInvariants invariants, final TransportSession session,
    ...
    @Override
    protected final Response transformResult(final NettyServerRequest<?> request, final T result) {
        if (withContent) {
    ...
    protected abstract Response transformResultImpl(NettyServerRequest<?> request, T result);

    protected abstract void fillHeaders(T result, HttpHeaders headers);
