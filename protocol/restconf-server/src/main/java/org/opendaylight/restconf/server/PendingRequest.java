 ...
 * has set up everything it needs, it will invoke {@link #execute(PendingRequestListener, Principal, InputStream)} to
 * kick off this request. Subclasses are required to notify of request completion by invoking the appropriate method
 * on the supplied {@link PendingRequestListener}.
 *
...
abstract non-sealed class PendingRequest<T> implements PreparedRequest {
     ...
     */
    abstract void execute(PendingRequestListener listener, @Nullable Principal principal, InputStream body);
