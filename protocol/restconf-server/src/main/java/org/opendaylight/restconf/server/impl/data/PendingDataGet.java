...
import org.opendaylight.restconf.server.impl.AbstractPendingGet;
...
final class PendingDataGet extends AbstractPendingGet<DataGetResult> {
    ...
    @Override
    protected void execute(final NettyServerRequest<DataGetResult> request) {
        if (apiPath.isEmpty()) {
    ...
    @Override
    protected Response transformResultImpl(final NettyServerRequest<?> request, final DataGetResult result) {
    ...
    @Override
    protected void fillHeaders(final DataGetResult result, final HttpHeaders headers) {
