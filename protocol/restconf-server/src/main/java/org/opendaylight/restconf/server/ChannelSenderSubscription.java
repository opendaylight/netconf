...
/**
 * A {@link RestconfStream.Sender} bound directly to the underlying transport channel. This is how event streams are delivered over
 ...
 */
public final class ChannelSenderSubscription extends SimpleChannelInboundHandler<FullHttpRequest>
    implements RestconfStream.Sender {
    private static final ByteBuf EMPTY_LINE = Unpooled.wrappedBuffer(new byte[]{'\r', '\n'}).asReadOnly();

    private final int sseMaximumFragmentLength;
    private final ReceiverImpl receiver;

    private Registration registration;
    private ChannelHandlerContext context;

    public ChannelSenderSubscription(final int sseMaximumFragmentLength, final ReceiverImpl receiver) {
    ...
    public void sendDataMessage(final String data) {
        if (!data.isEmpty() && context != null) {
            receiver.incrementSentEventCounter();
