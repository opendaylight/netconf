...
final class ModulesResource extends AbstractLeafResource {
    ...
    private PreparedRequest prepareModulesGet(final URI targetUri, final HttpHeaders headers,
        ...
        return doYin ? new PendingModulesGetYin(invariants, targetUri, principal, withContent, mountPath, decoded)
            : new PendingModulesGetYang(invariants, targetUri, principal, withContent, mountPath, decoded);
    }
}
