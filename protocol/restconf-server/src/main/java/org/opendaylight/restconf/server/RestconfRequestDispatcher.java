...
final class RestconfRequestDispatcher {
    ...
    static {
        ...
        NOT_ACCEPTABLE_DATA = new CompletedRequest(HttpResponseStatus.NOT_ACCEPTABLE, headers);
        UNSUPPORTED_MEDIA_TYPE_DATA = new CompletedRequest(HttpResponseStatus.UNSUPPORTED_MEDIA_TYPE, headers);
        UNSUPPORTED_MEDIA_TYPE_PATCH = new CompletedRequest(HttpResponseStatus.UNSUPPORTED_MEDIA_TYPE,
            ...
            case PendingRequest<?> pending -> {
                ...
                pending.execute(new RequestCompleter() {
                    ...
                    public void requestComplete(final PendingRequest<?> request, final Response reply) {
                        callback.onSuccess(switch (reply) {
                            case ByteSourceResponse byteSource -> {

                                throw new UnsupportedOperationException();
                            }
                            case CompletedRequest completed -> completed.toHttpResponse(version);
