...
final class RestconfRequestDispatcher {
    ...
    @NonNull PreparedRequest prepare(final URI targetUri, final SegmentPeeler peeler, final HttpRequest request) {
        ...
        return switch (peeler.next()) {
            ...
            case "operations" -> prepareOperations(targetUri, peeler, method, headers);
    ...
    private static @NonNull PreparedRequest prepareDataPatch(final SegmentPeeler peeler, final HttpHeaders headers) {
    ...
    // create resource -> https://www.rfc-editor.org/rfc/rfc8040#section-4.4.1
    // or invoke an action -> https://www.rfc-editor.org/rfc/rfc8040#section-3.6
    private static @NonNull PreparedRequest prepareDataPost(final SegmentPeeler peeler, final HttpHeaders headers) {
        final var contentTypeValue = headers.get(HttpHeaderNames.CONTENT_TYPE);
        if (contentTypeValue == null) {
            return CompletedRequest.badRequest(HttpHeaderNames.CONTENT_TYPE);
        }
    ...
    private static @NonNull PreparedRequest prepareDataPut(final SegmentPeeler peeler, final HttpHeaders headers) {
        final var contentTypeValue = headers.get(HttpHeaderNames.CONTENT_TYPE);
        if (contentTypeValue == null) {
            return CompletedRequest.badRequest(HttpHeaderNames.CONTENT_TYPE);
        }
    ...
    private @NonNull PreparedRequest prepareOperations(final URI targetUri, final SegmentPeeler peeler,
            final HttpMethod method, final HttpHeaders headers) {
        return switch (method.name()) {
     ...
     */
    private static @NonNull PreparedRequest prepareModules(final SegmentPeeler peeler, final HttpMethod method,
            final HttpHeaders headers) {
        return switch (method.name()) {
    ...
    private static @NonNull PreparedRequest prepareModulesGet(final SegmentPeeler peeler, final HttpHeaders headers,
            final boolean withContent) {
        ...
        return fileName.isEmpty() ? CompletedRequest.notFound()
            ...
            : new PendingModulesGet(mountPath, QueryStringDecoder.decodeComponent(fileName),
