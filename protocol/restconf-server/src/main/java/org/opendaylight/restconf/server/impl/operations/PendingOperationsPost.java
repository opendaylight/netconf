...
final class PendingOperationsPost extends PendingRequestWithOutput<InvokeResult, OperationInputBody> {
    ...
    @Override
    protected void execute(final NettyServerRequest<InvokeResult> request, final OperationInputBody body) {
        server().operationsPOST(request, restconfURI(), apiPath, body);
    }

    @Override
    protected Response transformResult(final NettyServerRequest<?> request, final InvokeResult result) {
        return transformInvoke(request, result, acceptEncoding);
    }

    @Override
    protected OperationInputBody wrapBody(final InputStream body) {
