 ...
 */
@Singleton
@Component(immediate = true, service = PrincipalService.class)
public final class AAAShiroPrincipalService implements PrincipalService {
    ...
    public @Nullable Principal acquirePrincipal(final FullHttpRequest request) {
        final var uuid = request.headers().get(PRINCIPAL_UUID_HEADER);
        if (uuid != null) {
            try {
                return principalMap.remove(UUID.fromString(uuid));
