...
import org.opendaylight.restconf.server.PendingRequestListener;
...
public abstract sealed class AbstractPendingRequest<T> extends PendingRequest<T>
    ...
    protected final @NonNull RestconfServer server() {
     ...
     */
    protected final @NonNull URI restconfURI() {
        return targetUri.resolve(invariants.restconfPath());
    }

    @Override
    @SuppressWarnings("checkstyle:illegalCatch")
    protected final void execute(final PendingRequestListener listener, final InputStream body) {
        try {
     ...
     */
    @NonNullByDefault
    protected abstract Response transformResult(NettyServerRequest<?> request, T result);
    ...
    @NonNullByDefault
    protected static final HttpHeaders metadataHeaders(final ConfigurationMetadata metadata) {
        return setMetadataHeaders(HEADERS_FACTORY.newEmptyHeaders(), metadata);
    }

    protected static final HttpHeaders setMetadataHeaders(final HttpHeaders headers,
