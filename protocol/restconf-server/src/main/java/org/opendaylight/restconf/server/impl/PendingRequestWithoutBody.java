 ...
 */
@NonNullByDefault
public abstract non-sealed class PendingRequestWithoutBody<T> extends AbstractPendingRequest<T> {
    private static final Logger LOG = LoggerFactory.getLogger(PendingRequestWithoutBody.class);

    protected PendingRequestWithoutBody(final EndpointInvariants invariants, final TransportSession session,
            final URI targetUri, final @Nullable Principal principal) {
    ...
    protected abstract void execute(NettyServerRequest<T> request);
