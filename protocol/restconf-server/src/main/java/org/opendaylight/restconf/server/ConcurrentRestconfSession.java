...
final class ConcurrentRestconfSession extends ConcurrentHTTPServerSession {
    ...
    // to read payload and it invocation of prepareResponse
    private final Map<Integer, RestconfHttp2Handler> aliveStreams = new ConcurrentHashMap<>();

    // there is only one delegate - just to overcome limitation that we cannot extends Http2EventAdapter in ConcurrentRestconfSession
    ...
    ConcurrentRestconfSession(final HTTPScheme scheme, final SocketAddress remoteAddress, final EndpointRoot root) {
        ...
        delegate = new RestconfHttp2Handler();
    }
    ...
    protected void channelRead0(final ChannelHandlerContext ctx, final FullHttpRequest msg) {
        // this should not be used - we are registered as frame listener
    }
    ...
    public void onStreamAdded(final Http2Stream stream) {
        ...
        aliveStreams.put(stream.id(), new RestconfHttp2Handler());
