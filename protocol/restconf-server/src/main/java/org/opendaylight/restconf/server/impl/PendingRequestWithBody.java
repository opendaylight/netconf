 ...
 */
@NonNullByDefault
public abstract non-sealed class PendingRequestWithBody<T, B extends ConsumableBody> extends AbstractPendingRequest<T> {
    // Note naming: derived from 'Content-Type'
    protected final MessageEncoding contentEncoding;

    protected PendingRequestWithBody(final EndpointInvariants invariants, final TransportSession session,
            final URI targetUri, final @Nullable Principal principal, final MessageEncoding contentEncoding) {
    ...
    protected abstract void execute(NettyServerRequest<T> request, B body);
     ...
     */
    protected abstract B wrapBody(InputStream body);

    protected final Response transformCreateResource(final CreateResourceResult result) {
    ...
    protected static final Response transformInvoke(final NettyServerRequest<?> request, final InvokeResult result,
