...
final class OperationsResource extends AbstractLeafResource {
    private static final EmptyRequestResponse METHOD_NOT_ALLOWED_RPC =
        new EmptyRequestResponse(HttpResponseStatus.METHOD_NOT_ALLOWED, AbstractPendingOptions.HEADERS_RPC);
    ...
    private PreparedRequest prepareOptions(final TransportSession session, final URI targetUri,
        ...
        return path.isEmpty() ? AbstractPendingOptions.READ_ONLY : requiredApiPath(path,
            apiPath -> new PendingOperationsOptions(invariants, session, targetUri, principal, apiPath, encoding));
    }
