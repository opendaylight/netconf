...
final class DataRequestProcessor {
    ...
    private static final String ALLOW_METHODS_ROOT = allowHeaderValue(OPTIONS, HEAD, GET, POST, PUT, PATCH);
    private static final String ALLOW_METHODS = allowHeaderValue(OPTIONS, HEAD, GET, POST, PUT, PATCH, DELETE);
    ...
    static void processDataRequest(final RequestParameters params, final RestconfServer service,
        ...
        switch (params.method()) {
            ...
            case PATCH -> {
                if (RESTCONF_TYPES.contains(contentType)) {
                    // merge target resource content -> https://datatracker.ietf.org/doc/html/rfc8040#section-4.6.1
                    patchData(params, service, callback, apiPath);
                } else if (YANG_PATCH_TYPES.contains(contentType)) {
                    // yang-patch -> https://datatracker.ietf.org/doc/html/draft-ietf-netconf-yang-patch-14#section-2
