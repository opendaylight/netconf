...
final class DataRequestProcessor {
    ...
    static void processDataRequest(final RequestParameters params, final RestconfServer service,
        ...
        if (HttpMethod.GET.equals(method)) {
            ...
            switch (params.mediaTypeGroup()) {
                case RESTCONF -> postData(params, service, callback, apiPath);
                default -> callback.onSuccess(unsupportedMediaTypeErrorResponse(params));
            }
        } else if (HttpMethod.PUT.equals(method)) {
            switch (params.mediaTypeGroup()) {
                case RESTCONF -> putData(params, service, callback, apiPath);
                default -> callback.onSuccess(unsupportedMediaTypeErrorResponse(params));
            }
        } else if (HttpMethod.PATCH.equals(method)) {
            switch (params.mediaTypeGroup()) {
                case RESTCONF -> patchData(params, service, callback, apiPath);
                case YANG_PATCH -> yangPatchData(params, service, callback, apiPath);
                default -> callback.onSuccess(unsupportedMediaTypeErrorResponse(params));
            }
