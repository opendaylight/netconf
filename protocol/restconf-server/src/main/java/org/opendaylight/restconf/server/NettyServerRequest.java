...
final class NettyServerRequest<T> extends MappingServerRequest<T> {
    ...
    private final @NonNull URI requestUri;
    private final Principal principal;

    private NettyServerRequest(final EndpointInvariants invariants, final PendingRequest<T> request,
            final RequestCompleter completer, final Principal principal, final URI requestUri) {
        super(QueryParameters.ofMultiValue(new QueryStringDecoder(requestUri).parameters()),
    ...
    NettyServerRequest(final PendingRequest<@NonNull T> request, final RequestCompleter completer,
            final Principal principal, final URI requestUri) {
        this(request.invariants, request, completer, principal, requestUri);
    }
