...
abstract class AbstractPendingOptions extends AbstractPendingRequest<OptionsResult> {
    ...
    static final HttpHeaders HEADERS_RESOURCE =
    ...
    private static HttpHeaders headers(final String... allowValues) {
        return HEADERS_FACTORY.newEmptyHeaders().set(HttpHeaderNames.ALLOW, Arrays.asList(allowValues));
    }

    private static HttpHeaders patchHeaders(final String... allowValues) {
        return headers(allowValues).set(HttpHeaderNames.ACCEPT_PATCH, List.of(
            // Plain patch
