...
final class RequestParameters {
    ...
    RequestParameters(final String basePath, final FullHttpRequest request, final @Nullable Principal principal,
        ...
        contentType = extractMimeType(request, defaultAcceptType);
    ...
    private static AsciiString extractMimeType(final FullHttpRequest request, final AsciiString defaultType) {
        final var mimeType = HttpUtil.getMimeType(request);
        if (mimeType != null) {
            return AsciiString.of(mimeType);
        }
