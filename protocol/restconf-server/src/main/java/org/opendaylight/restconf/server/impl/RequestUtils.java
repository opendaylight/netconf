...
final class RequestUtils {
    ...
    static ApiPath extractApiPath(final Pattern pattern, final RequestParameters params) {
        final var matcher = pattern.matcher(params.contextPath());
        String identifier;
        if (matcher.matches() && matcher.groupCount() > 0 && (identifier = matcher.group(1)) != null) {
            try {
                return ApiPath.parse(identifier);
    ...
    static ServerRequest buildServerRequest(final RequestParameters params) {
        try {
    ...
    private static <T extends ConsumableBody> T requestBody(final RequestParameters params, Class<T> type,
        ...
        return ContentTypes.isJson(params.contentType())
