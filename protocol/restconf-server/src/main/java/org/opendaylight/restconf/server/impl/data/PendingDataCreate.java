...
import org.opendaylight.restconf.server.impl.PendingRequestWithBody;
...
final class PendingDataCreate extends PendingRequestWithBody<CreateResourceResult, ChildBody> {
    ...
    @Override
    protected void execute(final NettyServerRequest<CreateResourceResult> request, final ChildBody body) {
        server().dataPOST(request, body);
    }

    @Override
    protected Response transformResult(final CreateResourceResult result) {
        return transformCreateResource(result);
    }

    @Override
    protected ChildBody wrapBody(final InputStream body) {
