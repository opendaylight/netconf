...
class AbstractRequestProcessorTest {
    ...
    @BeforeEach
    void beforeEach() {
        session = new RestconfSession(HTTPScheme.HTTP, new EndpointRoot(principalService, WELL_KNOWN,
            Map.of(BASE_PATH.substring(1),
                new APIResource(
                    new EndpointInvariants(server, PRETTY_PRINT, ERROR_TAG_MAPPING, MessageEncoding.JSON,
                        URI.create("/rests/")),
