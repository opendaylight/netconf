...
class DataE2ETest extends AbstractDataBrokerTest {
    ...
    @Override
    protected void setupWithSchema(final EffectiveModelContext modelContext) {
        this.modelContext = modelContext;
        super.setupWithSchema(modelContext);
    }

    @BeforeEach
    public void setUp() {
        dataBroker = getDataBroker();
        dispatcher = new RestconfRequestDispatcher(restconfService, principalService, baseUri, errorTagMapping,
    ...
    void crudOperationsTest() {
        dispatcher.dispatch(postRequest, callback);
        dispatcher.dispatch(getRequest, callback);
        dispatcher.dispatch(putRequest, callback);
        dispatcher.dispatch(deleteRequest, callback);
    }
