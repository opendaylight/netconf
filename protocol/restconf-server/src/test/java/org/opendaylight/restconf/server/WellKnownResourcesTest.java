...
class WellKnownResourcesTest {
    ...
    void options(final String uri) {
        assertOptionsResponse(RESOURCES.request(HttpVersion.HTTP_1_1, Method.OPTIONS, uri), "GET, HEAD, OPTIONS");
    }

    @ParameterizedTest
    @MethodSource
    void getHostMeta(final String uri, final AsciiString contentType, final String content) {
        assertResponse(RESOURCES.request(HttpVersion.HTTP_1_1, Method.GET, uri),
