...
public class RestconfSessionTest {
    @Mock
    ...
    @Test
    void closeRestconfSessionResourcesTest() {
        // setup
        final var sessionCaptor = ArgumentCaptor.forClass(RestconfSession.class);
        ...
        doReturn(sessionCaptor.getValue()).when(pipeline).remove(RestconfSession.class);
        // register resource
        sessionCaptor.getValue().registerResource(registration);
        listener.onTransportChannelClosed(transportChannel);
