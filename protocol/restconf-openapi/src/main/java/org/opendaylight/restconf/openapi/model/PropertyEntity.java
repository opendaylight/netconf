...
public class PropertyEntity {
    ...
    private void processRef(final String name, final SchemaNode schemaNode, final String discriminator,
        ...
        if (schemaNode instanceof ListSchemaNode listNode) {
            ...
            final var optMinElements = stmt.findFirstEffectiveSubstatementArgument(MinElementsEffectiveStatement.class);
            if (optMinElements.isPresent()) {
                final var minElements = optMinElements.orElseThrow();
            ...
            final var optMaxElements = stmt.findFirstEffectiveSubstatementArgument(MaxElementsEffectiveStatement.class);
            if (optMaxElements.isPresent()) {
                writeMaxItems(optMaxElements.orElseThrow());
            }
        } else {
             /*
