...
public final class RestconfLoggingBroker {
    ...
    private RestconfLoggingBroker(final boolean loggingHeadersEnabled, final boolean loggingQueryParametersEnabled,
            final boolean loggingBodyEnabled, final @NonNull Set<String> hiddenHttpHeaders) {
        this.loggingHeadersEnabled = loggingBodyEnabled;
     ...
     * @return JAX-RS singletons
     */
    public static Set<Object> singletons(final JaxRsEndpointConfiguration configuration) {
        if (configuration.restconfLoggingEnabled()) {
            final var broker = new RestconfLoggingBroker(configuration.loggingHeadersEnabled(),
                configuration.loggingQueryParametersEnabled(), configuration.loggingBodyEnabled(),
                configuration.hiddenHttpHeaders());
            return Set.of(new RequestLogger(broker), new ResponseWithoutBodyLogger(broker),
                new ResponseWithBodyLogger(broker));
        } else {
            return Set.of();
        }
    }
     ...
     */
    public void logRequest(final SecurityContext securityContext, final ContainerRequestContext requestContext,
                           final HttpServletRequest servletRequest, final byte[] requestBody) {
        final var messageId = getAndIncrementMessageId();
        ...
        writeUserInfo(securityContext, logBuilder);
    ...
    private long getAndIncrementMessageId() {
    ...
    private static void writeMessageId(final long messageId, final StringBuilder logBuilder) {
        logBuilder.append(LOG_ENTRIES_SEPARATOR).append(messageId);
    }
    ...
    private static void writeRemoteHost(final HttpServletRequest servletRequest, final StringBuilder logBuilder) {
        logBuilder.append(LOG_ENTRIES_SEPARATOR)
    ...
    private static void writeUserInfo(final SecurityContext securityContext, final StringBuilder logBuilder) {
        final var principal = securityContext.getUserPrincipal();
        logBuilder.append(LOG_ENTRIES_SEPARATOR);
        if (principal instanceof ODLPrincipal) {
            logBuilder.append(((ODLPrincipal) principal).getUserId());
        } else {
            logBuilder.append(principal.getName());
        }
    }

    private void writeQueryParameters(final MultivaluedMap<String, ?> parameters, final StringBuilder logBuilder) {
        if (loggingQueryParametersEnabled) {
    ...
    private void writeHttpHeaders(final MultivaluedMap<String, ?> parameters, final StringBuilder logBuilder) {
        if (loggingHeadersEnabled) {
            if (hiddenHttpHeaders.isEmpty()) {
                ...
                writeMap(filterHttpHeaders(parameters, hiddenHttpHeaders), logBuilder);
            }
        }
    }
    ...
    private static MultivaluedHashMap<String, Object> filterHttpHeaders(final MultivaluedMap<String, ?> parameters,
        ...
        final var filteredHeaders = new MultivaluedHashMap<String, Object>();
    ...
    private static void writeMap(final MultivaluedMap<String, ?> parameters, final StringBuilder logBuilder) {
        logBuilder.append(LOG_ENTRIES_SEPARATOR)
                .append("{");
        final var parametersIterator = parameters.entrySet().iterator();
        while (parametersIterator.hasNext()) {
            final var queryParameter = parametersIterator.next();
            logBuilder.append(queryParameter.getKey()).append(":").append(queryParameter.getValue());
