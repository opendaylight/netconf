    ...
    private record NamespaceName(String name, @Nullable String namespace) implements Comparable<NamespaceName> {
        ...
        static NamespaceName of(final Sibling sibling) {
            return switch (sibling.selection()) {
                case NamespaceSelection.Exact(var namespace, var name) -> new NamespaceName(name, namespace);
                case NamespaceSelection.Wildcard(var name) -> new NamespaceName(name, null);
            };
        }
