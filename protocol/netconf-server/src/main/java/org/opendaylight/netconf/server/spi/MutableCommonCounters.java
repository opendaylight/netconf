...
public final class MutableCommonCounters extends AbstractJavaCommonCounters implements MutableJavaCommonCounters {
    ...
    public static MutableCommonCounters copyOf(final CommonCounters values) {
        return switch (values) {
            ...
            case JavaCommonCounters from -> new MutableCommonCounters(checkedCast(from.inRpcs()),
                checkedCast(from.inBadRpcs()), checkedCast(from.outRpcErrors()), checkedCast(from.outNotifications()));
            default -> new MutableCommonCounters(unwrap(values.requireInRpcs()), unwrap(values.requireInBadRpcs()),
                unwrap(values.requireOutRpcErrors()), unwrap(values.requireOutNotifications()));
        };
    }
