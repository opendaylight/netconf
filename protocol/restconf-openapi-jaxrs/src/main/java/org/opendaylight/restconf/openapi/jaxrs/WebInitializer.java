...
public final class WebInitializer implements AutoCloseable {
    ...
    private static OpenApiOauth2Configuration oauth2Configuration(final Configuration configuration) {
        final var authUrl = configuration.oauth2$_$authorization$_$url().isBlank()
            ? null : configuration.oauth2$_$authorization$_$url();
        final var tokenUrl = configuration.oauth2$_$token$_$url().isBlank()
            ? null : configuration.oauth2$_$token$_$url();
        final var refreshUrl = configuration.oauth2$_$refresh$_$url().isBlank()
            ? null : configuration.oauth2$_$refresh$_$url();

        return authUrl == null || tokenUrl == null
            ? null : new OpenApiOauth2Configuration(authUrl, tokenUrl, refreshUrl);
    }
}
