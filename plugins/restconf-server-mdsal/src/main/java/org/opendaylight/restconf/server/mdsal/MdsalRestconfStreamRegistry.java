...
//                    .build())
    ...
    private static EventFormatter<DOMNotification> getEventFormatter(final QName encoding) {
     ...
     */
    private static ContainerNode subscriptionTerminated(final Uint32 id) {
        // FIXME probably should be SubscriptionTerminatedReason
        return buildErrorStateNotification(id, NoSuchSubscription.QNAME, State.TERMINATED);
    }
    ...
    private static ContainerNode subscriptionSuspended(final Uint32 id) {
        return buildErrorStateNotification(id, SubscriptionSuspendedReason.QNAME, State.TERMINATED);
    }
     ...
     */
    @SuppressFBWarnings("UPM")
    private static ContainerNode subscriptionCompleted(final Uint32 id) {
        return buildStateNotification(id, State.COMPLETED);
    }
