...
public final class MdsalRestconfStreamRegistry extends AbstractRestconfStreamRegistry implements AutoCloseable {
    ...
    public ListenableFuture<Void> updateReceiver(final String receiverName, final Uint32 subscriptionId,
        ...
        final var sentEventIid = YangInstanceIdentifier.builder()
            ...
            .node(NodeIdentifierWithPredicates.of(Subscription.QNAME, SubscriptionUtil.QNAME_RECEIVER_NAME, receiverName));

        final LeafNode<Uint64> counterValue;
        switch (recordType) {
            case SENT_EVENT_RECORDS -> {
                sentEventIid.node(SENT_EVENT_RECORDS_NODEID);
                counterValue = ImmutableNodes.leafNode(SENT_EVENT_RECORDS_NODEID, counter);
            }
            case EXCLUDED_EVENT_RECORDS -> {
                sentEventIid.node(EXCLUDED_EVENT_RECORDS_NODEID);
                counterValue = ImmutableNodes.leafNode(EXCLUDED_EVENT_RECORDS_NODEID, counter);
            }
