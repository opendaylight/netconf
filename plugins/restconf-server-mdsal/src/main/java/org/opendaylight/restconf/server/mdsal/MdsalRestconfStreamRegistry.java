...
//                        .build())
    ...
    private EventFormatter<DOMNotification> getEventFormatter(final QName encoding) {
     ...
     */
    private ContainerNode buildErrorStateNotification(final Uint32 id,
     ...
     */
    private ContainerNode buildStateNotification(final Uint32 id, final State state) {
        return ImmutableNodes.newContainerBuilder()
            .withNodeIdentifier(state.nodeId)
            .withChild(ImmutableNodes.leafNode(ID_NODEID, id))
            .build();
