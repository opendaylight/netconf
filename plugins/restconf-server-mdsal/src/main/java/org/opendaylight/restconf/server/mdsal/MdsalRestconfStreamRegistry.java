...
public final class MdsalRestconfStreamRegistry extends AbstractRestconfStreamRegistry
    ...
    private DOMTransactionChain filterTxChain;
    ...
    public MdsalRestconfStreamRegistry(@Reference final DOMDataBroker dataBroker,
        ...
        leaderReg = cssProvider.registerClusterSingletonService(this);
        filterTxChain = dataBroker.createMergingTransactionChain();
...
//                    .build())
    ...
    @Override
    protected synchronized void filtersOperationalViewUpdated(final Set<String> workingNames) {
        if (!isLeader.get()) {
            return;
        }

        final var tx = filterTxChain.newWriteOnlyTransaction();
        for (final var entry : filterSpecs.entrySet()) {
            final var name = entry.getKey();
            final var spec = entry.getValue();
            final var path = FILTERS_OPER_PATH.node(NodeIdentifierWithPredicates.of(StreamFilter.QNAME, NAME_QNAME,
                    name))
        ...
        Futures.addCallback(tx.commit(),
            new FutureCallback<CommitInfo>() {
                @Override public void onSuccess(final CommitInfo info) {
                    LOG.debug("Filter operational view updated");
                }

                @Override public void onFailure(final Throwable cause) {
                    LOG.warn("Failed to update filter operational view", cause);
                }
            }, MoreExecutors.directExecutor());
    }
