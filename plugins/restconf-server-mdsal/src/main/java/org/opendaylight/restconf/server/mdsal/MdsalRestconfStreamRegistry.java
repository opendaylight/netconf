...
//                        .build())
    ...
    protected EventStreamFilter parseSubtreeFilter(final AnydataNode<?> filter) {
        // alternative
        final var body = (DOMSourceAnyxmlNode) filter.body();
        final var element = UntrustedXML.newDocumentBuilder().newDocument().createElement();
        final var apiFilter = SubtreeFilter.readFrom(element.getDocumentElement());
