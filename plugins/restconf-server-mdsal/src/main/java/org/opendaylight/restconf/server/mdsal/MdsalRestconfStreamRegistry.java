...
public final class MdsalRestconfStreamRegistry extends AbstractRestconfStreamRegistry {
            ...
            for (var change : changes) {
                ...
                switch (node.modificationType()) {
                    ...
                    case SUBTREE_MODIFIED, APPEARED, WRITE -> {
                        final var data = (DataContainerNode) change.getRootNode().dataAfter();
                        ...
                        // FIXME: finish this, extract to method, fix casts
                        final var subtree = (AnydataNode<?>) filterSpec.childByArg(new NodeIdentifier(StreamSubtreeFilter.QNAME));
                        final var xpath = (String) filterSpec.childByArg(new NodeIdentifier(QName.create(FilterSpec.QNAME, "stream-xpath-filter"))).body();
                        final EventStreamFilter filter;
                        try {
                            filter = resolveFilter(new RestconfStream.SubscriptionFilter.SubtreeDefinition(subtree));
                    ...
                    case DELETE, DISAPPEARED -> {
                        final var data = (DataContainerNode) change.getRootNode().dataBefore();
                        final var name = (String) data.childByArg(FILTER_NAME).body();
    ...
    public MdsalRestconfStreamRegistry(@Reference final DOMDataBroker dataBroker,
        ...
        if (changeService != null) {
            changeService.registerTreeChangeListener(DOMDataTreeIdentifier.of(LogicalDatastoreType.OPERATIONAL,
