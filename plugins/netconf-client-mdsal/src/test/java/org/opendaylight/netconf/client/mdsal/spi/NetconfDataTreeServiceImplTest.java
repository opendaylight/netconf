...
class NetconfDataTreeServiceImplTest extends AbstractTestModelTest {
    ...
    private DataStoreService netconService;
    ...
    @Test
    void get() {
        netconService.read(LogicalDatastoreType.OPERATIONAL, YangInstanceIdentifier.of(), List.of());
        verify(rpcService).invokeNetconf(eq(Get.QNAME), any());
    }

    @Test
    void getConfig() {
        netconService.read(LogicalDatastoreType.CONFIGURATION, YangInstanceIdentifier.of(), List.of());
        verify(rpcService).invokeNetconf(eq(GetConfig.QNAME), any());
    }

    @Test
    void merge() {
        netconService.merge(TxTestUtils.getLeafId(), TxTestUtils.getLeafNode());
    ...
    @Test
    void replace() {
        netconService.replace(TxTestUtils.getLeafId(), TxTestUtils.getLeafNode());
        verify(rpcService, timeout(2000)).invokeNetconf(eq(EditConfig.QNAME), captor.capture());
        verify(rpcService).invokeNetconf(eq(Lock.QNAME), any());
    ...
    @Test
    void create() {
        netconService.create(TxTestUtils.getLeafId(), TxTestUtils.getLeafNode());
        verify(rpcService, timeout(2000)).invokeNetconf(eq(EditConfig.QNAME), captor.capture());
        verify(rpcService).invokeNetconf(eq(Lock.QNAME), any());
    ...
    @Test
    void delete() {
        netconService.delete(TxTestUtils.getLeafId().getParent());
        verify(rpcService, timeout(2000)).invokeNetconf(eq(EditConfig.QNAME), captor.capture());
        verify(rpcService).invokeNetconf(eq(Lock.QNAME), any());
    ...
    @Test
    void remove() {
        netconService.remove(TxTestUtils.getLeafId().getParent());
        verify(rpcService, timeout(2000)).invokeNetconf(eq(EditConfig.QNAME), captor.capture());
        verify(rpcService).invokeNetconf(eq(Lock.QNAME), any());
    ...
    private DataStoreService getNetconService() {
