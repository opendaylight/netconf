...
final class DefaultDeviceNetconfSchemaProvider implements DeviceNetconfSchemaProvider {
    ...
    public ListenableFuture<DeviceNetconfSchema> deviceNetconfSchemaFor(final RemoteDeviceId deviceId,
        ...
        return Futures.transformAsync(sourceResolverFuture, deviceSources -> {
            ...
            final var registrations = deviceSources.register(repository);
    ...
    @Override
    public <T extends @NonNull SourceRepresentation> Registration registerSchemaSource(
            final SchemaSourceProvider<T> provider, final PotentialSchemaSource<T> source) {
        return repository.registerSchemaSource(provider, source);
    }
}
