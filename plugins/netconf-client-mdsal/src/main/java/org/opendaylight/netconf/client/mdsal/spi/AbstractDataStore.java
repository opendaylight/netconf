...
public abstract class AbstractDataStore implements DataStoreService, DataOperationService {
    ...
    @Override
    public ListenableFuture<? extends DOMRpcResult> createData(Data path, NormalizedNode data) {
        if (isNonEmptyListPath(data)) {
            ...
            for (var child : ((NormalizedNodeContainer<?>) data).body()) {
                ...
                addIntoFutureChain(futureChain, () -> create(childPath, child), false);
            }
            return futureChain;
        }
        return create(path.instance(), data);
    }

    @Override
    public ListenableFuture<? extends DOMRpcResult> deleteData(Data path) {
        return eraseData(path, this::delete);
    }

    @Override
    public ListenableFuture<? extends DOMRpcResult> removeData(Data path) {
        return eraseData(path, this::remove);
    }

    @Override
    public ListenableFuture<? extends DOMRpcResult> mergeData(final Data path, NormalizedNode data) {;
    ...
    public ListenableFuture<? extends DOMRpcResult> putData(final Data path, final NormalizedNode data) {
        if (isNonEmptyListPath(data)) {
            ...
            for (var child : ((NormalizedNodeContainer<?>) data).body()) {
                final var childPath = path.instance().node(child.name());
                addIntoFutureChain(futureChain, () -> put(childPath, child), false);
            }
            return futureChain;
        } else {
            return put(path.instance(), data);
        }
    }
    ...
    private ListenableFuture<Optional<NormalizedNode>> readDataViaTransaction(final @NonNull LogicalDatastoreType store,
        ...
        final var readFuture = read(store, path.instance(), fieldPaths);
    ...
    private @NonNull Collection<? extends NormalizedNode> getListItemsForRemove(final Data path)
        ...
        if (path.schema().dataSchemaNode() instanceof ListSchemaNode listSchemaNode) {
            ...
            future = read(CONFIGURATION, path.instance(), List.of());
        }
