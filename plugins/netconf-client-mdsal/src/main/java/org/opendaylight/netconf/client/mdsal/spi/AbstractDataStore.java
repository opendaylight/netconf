...
public abstract class AbstractDataStore implements DataStoreService {
    ...
    ListenableFuture<Optional<NormalizedNode>> getData(final YangInstanceIdentifier path,
    ...
    ListenableFuture<? extends DOMRpcResult> addIntoFutureChain(
        ...
        return Futures.transformAsync(lockFuture, result -> {
    ...
    ListenableFuture<? extends DOMRpcResult> addCancelIfFails(final ListenableFuture<? extends DOMRpcResult> future) {
        final var handelRpcResult = Futures.transformAsync(future, result -> {
            if (result != null && !result.errors().isEmpty() && !allWarnings(result.errors())) {
                return Futures.transformAsync(cancel(), ignored ->
                    Futures.immediateFuture(result), MoreExecutors.directExecutor());
            }
            return Futures.transform(RPC_SUCCESS, resultB -> result, MoreExecutors.directExecutor());
        }, Executors.newSingleThreadExecutor());

        return Futures.catchingAsync(handelRpcResult, Throwable.class, t ->
