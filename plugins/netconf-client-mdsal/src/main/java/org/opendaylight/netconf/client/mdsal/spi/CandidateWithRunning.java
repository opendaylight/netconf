...
public class CandidateWithRunning extends AbstractDataStore {
    ...
    ListenableFuture<? extends DOMRpcResult> unlock() {
        if (lockDatastore && lock.getAndSet(false)) {
            ...
            return Futures.whenAllComplete(candidateUnlock, runningUnlock).call(() -> {
                ...
                if (!runningResult.errors().isEmpty() && !allWarnings(runningResult.errors())) {
                    return runningResult;
                }
                if (!candidateResult.errors().isEmpty() && !allWarnings(candidateResult.errors())) {
    ...
    private ListenableFuture<? extends DOMRpcResult> editConfig(final EffectiveOperation operation,
        ...
        try {
            ...
            LOG.error("Failed to create edit-config structure node for child {} with path {}, with RPC operation {}",
