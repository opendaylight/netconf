...
public class DataOperationImpl implements DataOperationService {
    ...
    public ListenableFuture<? extends DOMRpcResult> putData(final Data path, final NormalizedNode data) {
        if (isNonEmptyListPath(data)) {
            ...
            for (var child : ((NormalizedNodeContainer<?>) data).body()) {
                final var childPath = path.instance().node(child.name());
                addIntoFutureChain(futureChain, () -> dataStoreService.replace(childPath, child));
            }
            return futureChain;
        } else {
    ...
    public ListenableFuture<Optional<NormalizedNode>> getData(final Data path, final DataGetParams params) {
        ...
        final var fieldsParser = new FieldsParamParser(path.inference().modelContext(), fields);
        return switch (content) {
            case ALL -> {
                // PREPARE STATE DATA NODE
                final var stateDataFuture = readDataViaTransaction(OPERATIONAL, path, fieldsParser);
                // PREPARE CONFIG DATA NODE
                final var configDataFuture = readDataViaTransaction(CONFIGURATION, path, fieldsParser);
                addCallbackToAddList(configDataFuture, path);
            ...
            case NONCONFIG -> processor.nonConfig(
                readDataViaTransaction(OPERATIONAL, path, fieldsParser));
        };
    }
        ...
        if (isListPath(path)) {
            ...
            if (items.isEmpty()) {
                ...
                for (final var childElement : items) {
                    final var childPath = path.enterPath(path.instance().node(childElement.name()));
