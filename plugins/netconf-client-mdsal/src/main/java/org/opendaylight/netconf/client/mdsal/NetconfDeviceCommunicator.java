...
public class NetconfDeviceCommunicator implements NetconfClientSessionListener, RemoteDeviceCommunicator {
    ...
    private void processMessage(final NetconfMessage message) {
        ...
        if (message instanceof RpcMessage rpcMessage) {
            outputMsgId = rpcMessage.getDocument().getDocumentElement().getAttribute(XmlNetconfConstants.MESSAGE_ID);
        } else if (message instanceof RpcReplyMessage replyMessage) {
            outputMsgId = replyMessage.getDocument().getDocumentElement().getAttribute(XmlNetconfConstants.MESSAGE_ID);
        } else {
