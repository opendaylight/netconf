...
public final class NetconfStateSchemasResolverImpl implements NetconfDeviceSchemasResolver {
    ...
    public ListenableFuture<NetconfDeviceSchemas> resolve(final RemoteDeviceId deviceId,
        ...
        if (sessionPreferences.containsNonModuleCapability(CapabilityURN.YANG_LIBRARY)) {
            ...
            function = sources -> resolveYang11(deviceId, sessionPreferences, deviceRpc, baseModelContext, sources);
        } else {
    ...
    private static ListenableFuture<List<ProvidedSources<?>>> resolveMonitoringSources(
        ...
        return Futures.transform(deviceRpc.invokeNetconf(Get.QNAME, GET_SCHEMAS_RPC),
            result -> resolveMonitoringSources(deviceId, result, baseModelContext),
            MoreExecutors.directExecutor());
    }

    private static List<ProvidedSources<?>> resolveMonitoringSources(final RemoteDeviceId deviceId,
            final DOMRpcResult rpcResult, final EffectiveModelContext baseModelContext) {
        ...
        return resolveMonitoringSources(deviceId, schemasNode);
    }
    ...
    static List<ProvidedSources<?>> resolveMonitoringSources(final RemoteDeviceId deviceId,
        ...
        final var builder = ImmutableSet.<SourceIdentifier>builderWithExpectedSize(schemaMap.size());
        for (var schemaNode : schemaMap.body()) {
            final var sourceId = createFromNormalizedNode(deviceId, schemaNode);
            if (sourceId != null) {
    ...
    private static @Nullable SourceIdentifier createFromNormalizedNode(final RemoteDeviceId id,
        ...
        try {
            return new SourceIdentifier(identifier, version.isEmpty() ? null : Revision.of(version));
    ...
    private static ListenableFuture<NetconfDeviceSchemas> resolveYang11(final RemoteDeviceId deviceId,
            final NetconfSessionPreferences sessionPreferences, final NetconfRpcService deviceRpc,
            final EffectiveModelContext baseModelContext, final List<ProvidedSources<?>> monitoringSources) {
//      return LibraryModulesSchemas.forDevice(deviceRpc, deviceId);
        throw new UnsupportedOperationException("Not implemeneted yet");
    }
}
