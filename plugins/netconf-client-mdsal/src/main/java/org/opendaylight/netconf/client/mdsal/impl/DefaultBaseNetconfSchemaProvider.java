...
public final class DefaultBaseNetconfSchemaProvider implements BaseNetconfSchemaProvider {
    ...
    @Override
    public DefaultBaseNetconfSchema baseSchemaForCapabilities(final Collection<String> capabilityURNs) {
        return baseSchemas.getUnchecked(new Capabilities(
            capabilityURNs.contains(CapabilityURN.WRITABLE_RUNNING),
            capabilityURNs.contains(CapabilityURN.CANDIDATE),
            capabilityURNs.contains(CapabilityURN.CONFIRMED_COMMIT_1_1)
                || capabilityURNs.contains(CapabilityURN.CONFIRMED_COMMIT),
            capabilityURNs.contains(CapabilityURN.ROLLBACK_ON_ERROR),
            capabilityURNs.contains(CapabilityURN.VALIDATE_1_1) || capabilityURNs.contains(CapabilityURN.VALIDATE),
            capabilityURNs.contains(CapabilityURN.STARTUP),
            capabilityURNs.contains(CapabilityURN.URL),
            capabilityURNs.contains(CapabilityURN.XPATH),
            capabilityURNs.contains(CapabilityURN.NOTIFICATION),
            capabilityURNs.contains(CapabilityURN.YANG_LIBRARY)
                || capabilityURNs.contains(CapabilityURN.YANG_LIBRARY_1_1),
            capabilityURNs.contains("urn:ietf:params:xml:ns:yang:ietf-netconf-monitoring?module=ietf-netconf-monitoring"
                + "&revision=2010-10-04")));
    }
}
