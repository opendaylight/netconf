 ...
 */
@NonNullByDefault
public final class RpcsTimeoutAndRecoveryHandler implements RpcsHandler {
    ...
    private <T> ListenableFuture<T> scheduleTimeout(final ListenableFuture<T> listenableFuture) {
        // Create the task responsible for cancelling provided listenableFuture and resolving the user future.
        // It also handles device disconnection upon RPC failure (timeout or exception).
        final var timeoutTask = new RequestTimeoutTask<>(listenableFuture);
