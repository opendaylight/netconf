 ...
 *
 * <p>This class sends get-schema RPCs to a device to fetch all device models. If during this process, RPCs handled by
 * {@link org.opendaylight.netconf.client.mdsal.api.RpcsTimeoutAndRecoveryHandler} fail, it triggers device
 * reconnection. This sets {@code reconnectingDevice} flag to {@code true} Setting this flag prevents the building of
 * the {@link EffectiveModelContext} and interrupts further RPC requests. This is expected, as this class will be
 * reinitialized upon the next device connection.
 */
final class SchemaSetup implements FutureCallback<EffectiveModelContext> {
    ...
    public void onFailure(final Throwable cause) {
        ...
        if (cause instanceof SchemaSourceException schemaSourceException) {
            requiredSources = handleSchemaSourceException(schemaSourceException);
        } else {
    ...
    private void trySetupSchema() {
        if (reconnectingDevice.get()) {
            LOG.debug("{}: Device is reconnecting. Abort schema context build.", deviceId);
    ...
    private List<SourceIdentifier> handleSchemaSourceException(final SchemaSourceException exception) {
        ...
        LOG.debug("{}: Unable to build schema context, failed to resolve source {}, will reattempt without it",
            deviceId, sourceId, exception);
        final var qNameOfMissingSource = getQNameFromSourceIdentifiers(List.of(sourceId));
        addUnresolvedCapabilities(qNameOfMissingSource, FailureReason.UnableToResolve);
