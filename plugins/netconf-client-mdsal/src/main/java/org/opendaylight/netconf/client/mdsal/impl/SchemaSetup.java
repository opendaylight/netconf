...
final class SchemaSetup implements FutureCallback<EffectiveModelContext> {
    ...
    public void onSuccess(final EffectiveModelContext result) {
        ...
        resultFuture.set(new DeviceNetconfSchema(new NetconfDeviceCapabilities(ImmutableMap.copyOf(unresolvedCapabilites),
    ...
    private void trySetupSchema() {
        if (!requiredSources.isEmpty()) {
            ...
            resultFuture.setException(new EmptySchemaContextException(deviceId + ": No more sources for schema context"));
        }
    }
