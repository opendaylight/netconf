...
final class SchemaSetup implements FutureCallback<EffectiveModelContext> {
        ...
        return origSources.parallelStream()
            .filter(sourceId -> {
                try {
                    ...
                    if (!reconnectingDevice.get() && isDeviceReconnectingException(e)) {
    ...
    private static boolean isDeviceReconnectingException(final Exception ex) {
        return ex instanceof ExecutionException executionExc
            ...
            && (missExc.getCause() instanceof MonitoringSchemaSourceException
