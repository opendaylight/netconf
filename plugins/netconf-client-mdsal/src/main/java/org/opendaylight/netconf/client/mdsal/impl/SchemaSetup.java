...
final class SchemaSetup implements FutureCallback<EffectiveModelContext> {
    ...
    private static void normalizeNetconfBaseTo2011(final Set<QName> req) {
        final boolean has2013 = req.stream().anyMatch(qn -> isNetconf(qn)
            && qn.getRevision().map(NETCONF_BASE_REVISION_2013::equals).orElse(false));

        // Remove any NETCONF base entries that are not 2011 (e.g., 2013)
        req.removeIf(qn -> isNetconf(qn) && !qn.getRevision().orElseThrow().equals(NETCONF_BASE_REVISION_2011));

        // Add 2011 only if 2013 was present, and 2011 is not already present
        final boolean has2011 = req.stream().anyMatch(qn -> isNetconf(qn)
            && qn.getRevision().map(NETCONF_BASE_REVISION_2011::equals).orElse(false));
