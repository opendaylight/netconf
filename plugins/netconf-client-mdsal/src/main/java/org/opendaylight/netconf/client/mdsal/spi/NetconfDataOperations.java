...
public class NetconfDataOperations extends AbstractServerDataOperations {
    ...
    protected void createData(final ServerRequest<? super CreateResourceResult> request, final Data path,
        ...
        var futureChain = dataStoreService.createData(path.instance(), data);
    ...
    public void mergeData(final ServerRequest<DataPatchResult> request, final Data path, final NormalizedNode data) {
        var futureChain = dataStoreService.mergeData(path.instance(), data);
        futureChain = addIntoFutureChain(futureChain, dataStoreService::commit);
    ...
    public void putData(final ServerRequest<DataPutResult> request, final Data path, final NormalizedNode data) {
        ...
        var futureChain = dataStoreService.putData(path.instance(), data);
        futureChain = addIntoFutureChain(futureChain, dataStoreService::commit);
    ...
    private ListenableFuture<? extends DOMRpcResult> patchRequest(final Data path, final NormalizedNode data,
        ...
        try {
            return switch (operation) {
                case Create -> dataStoreService.createData(path.instance(), data);
                case Merge -> dataStoreService.mergeData(path.instance(), data);
                case Replace -> dataStoreService.putData(path.instance(), data);
        ...
        return switch (insert.insert()) {
            case FIRST -> {
                ...
                yield Futures.transformAsync(readData, readNode -> {
                    ...
                    if (readNode.isEmpty()) {
                        result = dataStoreService.putData(instance, data);
                    } else {
                        var futureChain = dataStoreService.removeData(parentPath);
                        futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.putData(instance, data));
                        result = addIntoFutureChain(futureChain,
                            () -> dataStoreService.putData(parentPath.instance(), readNode.orElseThrow()));
                    }
                    return result;
                }, MoreExecutors.directExecutor());
            }
            case LAST -> dataStoreService.putData(instance, data);
            case BEFORE -> {
                final var readData = dataStoreService.getData(parentPath, CONFIG_PARAM);
    ...
    private ListenableFuture<? extends DOMRpcResult> insertWithPointPut(final Data path, final Data parentPath,
        ...
        futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.mergeData(parentPath.instance(),
            emptySubtree));

        for (var nodeChild : readList.body()) {
            if (lastInsertedPosition == lastItemPosition) {
                futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.putData(path.instance(), data));
            }
            final var conChild = childPath(parentPath, List.of(nodeChild.name()));
            futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.putData(conChild.instance(),
                nodeChild));
                ...
                yield Futures.transformAsync(readData,
                    readNode -> {
                        ...
                        if (readNode.isEmpty()) {
                            ...
                            futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.putData(instance,
                                data));
                            result = addIntoFutureChain(futureChain, () -> dataStoreService.putData(instance,
                                readNode.orElseThrow()));
                        }
                        return result;
                    }, MoreExecutors.directExecutor());
            }
            case LAST -> dataStoreService.createData(instance, data);
            case BEFORE -> {
                final var readData = dataStoreService.getData(path, CONFIG_PARAM);
                yield Futures.transformAsync(readData,
                    readNode -> {
    ...
    private ListenableFuture<? extends DOMRpcResult> insertWithPointPost(final Data path, final NormalizedNode data,
        ...
        for (var nodeChild : readList.body()) {
            ...
            futureChain = addIntoFutureChain(futureChain, () -> dataStoreService.putData(childPath.instance(),
                nodeChild));
            lastInsertedPosition++;
        }
