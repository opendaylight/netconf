...
public class NetconfDataOperations extends AbstractServerDataOperations {
    ...
    private @Nullable NormalizedNodeContainer<?> readList(final Data path) throws RequestException {
        ...
        final var result =  RestconfStrategy.syncAccess(read(CONFIGURATION, path),
            path.instance());
    ...
    public ListenableFuture<Optional<NormalizedNode>> readData(final @NonNull Data path,
        ...
        final var fieldsParser = new FieldsParamParser(path.inference().modelContext(), params.fields());
        return switch (content) {
            case ALL -> {
                // PREPARE STATE DATA NODE
                final var stateDataFuture = readDataViaTransaction(LogicalDatastoreType.OPERATIONAL, path,
                    fieldsParser);
                // PREPARE CONFIG DATA NODE
                final var configDataFuture = readDataViaTransaction(CONFIGURATION, path, fieldsParser);
            ...
            case NONCONFIG -> processor.nonConfig(
                readDataViaTransaction(LogicalDatastoreType.OPERATIONAL, path, fieldsParser));
        };
    }
     ...
     * @param fieldsParser          FieldsParser prepared to read from starting node
     * @return {@link NormalizedNode}
     * @throws RequestException when an error occurs
     */
    private ListenableFuture<Optional<NormalizedNode>> readDataViaTransaction(final @NonNull LogicalDatastoreType store,
            final @NonNull Data path, final FieldsParamParser fieldsParser) throws RequestException {
        // Paths to selected subtrees which should be read, relative to the parent path
        final var fieldPaths = fieldsParser.toPaths(path.schema());
