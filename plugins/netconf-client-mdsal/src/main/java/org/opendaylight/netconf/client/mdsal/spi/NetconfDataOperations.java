...
public final class NetconfDataOperations extends AbstractServerDataOperations {
    ...
    private static final DataPutResult PUT_CREATED = new DataPutResult(true);
    private static final DataPutResult PUT_REPLACED = new DataPutResult(false);
    private static final DataPatchResult PATCH_EMPTY = new DataPatchResult();
    ...
    public NetconfDataOperations(final NetconfDataTreeService dataTreeService) {
    ...
    protected void createData(final ServerRequest<? super CreateResourceResult> request, final Data path,
        ...
        Futures.addCallback(futureChain, new FutureCallback<DOMRpcResult>() {
            @Override
            public void onSuccess(final @Nullable DOMRpcResult result) {
     ...
     */
    @VisibleForTesting
    public void readData(final ServerRequest<Optional<NormalizedNode>> getRequest, final Data path,
            final DataGetParams params) throws RequestException {
        ...
        Futures.addCallback(getFuture, new FutureCallback<>() {
            @Override
            public void onSuccess(final @Nullable Optional<NormalizedNode> result) {
    ...
    private Collection<? extends NormalizedNode> getListItemsForRemove(final Data path) throws RequestException {
    ...
    private ListenableFuture<? extends DOMRpcResult> insertPut(final Data path, final NormalizedNode data,
            final Insert insert, final Data parentPath) throws RequestException {
    ...
    private ListenableFuture<? extends DOMRpcResult> insertCreate(final Data path, final NormalizedNode data,
            final Insert insert) throws RequestException {
        return switch (insert.insert()) {
            case FIRST -> {
    ...
    private ListenableFuture<Optional<NormalizedNode>> readDataViaTransaction(final LogicalDatastoreType store,
            final Data path, final @Nullable FieldsParam fields) throws RequestException {
    ...
    private static void executeWithLogging(final Supplier<ListenableFuture<? extends DOMRpcResult>> operation) {
        ...
        Futures.addCallback(operationResult, new FutureCallback<DOMRpcResult>() {
            @Override
            public void onSuccess(final @Nullable DOMRpcResult rpcResult) {
    ...
    private static boolean allWarnings(final Collection<? extends RpcError> errors) {
        return errors.stream().allMatch(error -> error.getSeverity() == ErrorSeverity.WARNING);
    }

    /**
     * Chain result of previous future stored in {@code futureChain} and following operation stored
     * in {@code nextFuture}.
     *
     * <p>In case of error (in the previous operation) we use {@code previousEntity} to show to the user parameters
    ...
    private static ListenableFuture<? extends DOMRpcResult> chainPatchEditTransaction(
        ...
        return Futures.transformAsync(futureChain,
            result -> {
                if (!result.errors().isEmpty() && !allWarnings(result.errors())) {
    ...
    private static RequestException decodeException(final Throwable ex, final String txType, final Data dataPath) {
