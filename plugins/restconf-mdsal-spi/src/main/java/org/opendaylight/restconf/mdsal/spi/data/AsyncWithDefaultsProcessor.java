...
public final class AsyncWithDefaultsProcessor {
    private static final Logger LOG = LoggerFactory.getLogger(AsyncWithDefaultsProcessor.class);

    private final Data path;
    private final WithDefaultsParam defaultsMode;

    public AsyncWithDefaultsProcessor(final Data path, final WithDefaultsParam defaultsMode) {
        this.path = path;
    ...
    public ListenableFuture<Optional<NormalizedNode>> config(final ListenableFuture<Optional<NormalizedNode>> config) {
        return Futures.whenAllComplete(config).call(() -> {
            ...
            final var result = defaultsMode == null ? configDataNode
    ...
    public ListenableFuture<Optional<NormalizedNode>> all(final ListenableFuture<Optional<NormalizedNode>> config,
        ...
        return Futures.whenAllComplete(config, nonConfig).call(() -> {
            ...
            final var configParsedDataNode = defaultsMode == null ? configDataNode
                : prepareDataByParamWithDef(configDataNode, path, defaultsMode.mode());
            final var result = mergeConfigAndSTateDataIfNeeded(stateDataNode, configParsedDataNode);
            return Optional.ofNullable(result);
        }, MoreExecutors.directExecutor());
    }
    ...
    private static <T extends NormalizedNode> void mapValueToBuilder(
            ...
            final @NonNull NormalizedNodeContainerBuilder<?, YangInstanceIdentifier.PathArgument, T, ?> builder) {
    ...
    private static <T extends NormalizedNode> void mapDataToBuilder(
            final @NonNull Map<YangInstanceIdentifier.PathArgument, T> configMap,
            final @NonNull Map<YangInstanceIdentifier.PathArgument, T> stateMap,
            final @NonNull NormalizedNodeContainerBuilder<?, YangInstanceIdentifier.PathArgument, T, ?> builder) {
